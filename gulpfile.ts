...
async function test(): Promise<void> {
	...
	result = cp.spawnSync("node", ["./out/test/runTest.js"], {
		encoding: "utf-8",
		stdio: "inherit",
		env,
	});
...
function buildTLEGrammar(): void {
	...
	const builtinFunctions: string[] =
		expressionMetadata.functionSignatures.map((sig) => sig.name);
	...
	for (const key of replacementKeys) {
		...
		const value = JSON.parse(grammar).preprocess[key];
...
function copyFolder(
	...
	fse.readdirSync(sourceFolder).forEach((fn) => {
		const src = path.join(sourceFolder, fn);
...
async function packageVsix(): Promise<void> {
	...
	const stagingFolder = getTempFilePath("staging", "");
	console.log(`Staging folder: ${stagingFolder}`);
	fse.mkdirpSync(stagingFolder);

	// Copy files/folders to staging
	for (const p of filesAndFoldersToPackage) {
		copyToStagingFolder(p, p);
	}
	const filesInStaging = fse.readdirSync(stagingFolder);
	...
	const packageContents = fse.readJsonSync(
...
async function verifyTestsReferenceOnlyExtensionBundle(
	...
	for (const filePath of await recursiveReadDir(testFolder)) {
