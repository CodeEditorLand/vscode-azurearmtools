...
const tleGrammarSourcePath: string = path.join(
	__dirname,
...
async function assertUnchangedTokens(
	...
	try {
		...
		let shouldBeExpression =
			shouldBeArmTemplate && !testPath.match(/\.NOT-EXPR\./i);
		...
		if (shouldBeArmTemplate) {
			...
			filePathForReadingTokens = getTempFilePath(
				path.basename(testPath),
				".azrm"
			);
			fs.writeFileSync(
				filePathForReadingTokens,
				fs.readFileSync(testPath)
			);
		}

		let rawData: { c: string; t: string; r: unknown[] }[] | undefined =
			await commands.executeCommand(
				"_workbench.captureSyntaxTokens",
				Uri.file(filePathForReadingTokens)
			);
		if (!rawData) {
		...
		let data: ITokenInfo[] = rawData
			.map(
				(d) =>
					<ITokenInfo>{ text: d.c.trim(), scopes: d.t, colors: d.r }
			)
			.filter((d) => d.text !== "");
		...
		for (let iData = 0; iData < data.length; ++iData) {
			...
			let nBegin = data.findIndex(
				(t, i) => i >= iData && t.text.match(testStartToken) !== null
			);
			if (nBegin < 0) {
				break;
			}
			...
			let dictionaryNestingLevel = getDictionaryNestingLevel(
				data[nBegin].scopes
			);
			while (
				getDictionaryNestingLevel(data[nBegin].scopes) ===
				dictionaryNestingLevel
			) {
				nBegin++;
			}
			...
			let nEnd = data.findIndex(
				(t, i) =>
					i >= nBegin &&
					// end of the dictionary value item
					getDictionaryNestingLevel(t.scopes) ===
						dictionaryNestingLevel
			);
			if (nEnd < 0) {
				let { fullScopeString, text } = getTestcaseResults([
					{ testString: "", data: data.slice(nBegin) },
				]);
				assert(
					false,
					`Couldn't find end of test string starting here:\\n${text}\n${fullScopeString}`
				);
			}
			nEnd -= 1;
			...
			let testcase: ITestcase = {
				testString: `TEST STRING: ${testData
					.map((d) => d.text)
					.join("")}`,
				data: testData,
			};
		...
		let {
			...
			fullScopeString: resultsFullString,
			shortScopeString: resultsShortString,
		} = getTestcaseResults(testCases);
		let resultsString = shouldCompareFullScope
			? resultsFullString
			: resultsShortString;
		...
		if (fs.existsSync(resultPath)) {
			let previousResult = normalizeString(
				fs.readFileSync(resultPath).toString().trim()
			);
			...
			try {
				for (let testcaseResult of testcaseResults) {
					...
					if (shouldBeArmTemplate) {
						...
						assert(
							!testcaseResult.includes(
								"source.json.arm-template"
							),
					...
					let isExpression = testcaseResult.includes(
						"meta.expression.tle.arm-template"
					);
				...
				let nonDiffError = isJustDiff
					? ""
					: `${parseError(e).message}${os.EOL}`;
				writeToResultsFile(
					resultPathToWriteTo,
					resultsString,
					shouldCompareFullScope,
					resultsFullString
				);
				...
				if (OVERWRITE) {
					...
					// tslint:disable-next-line: max-line-length
					throw new Error(
						`${nonDiffError}*** MODIFIED THE RESULTS FILE (${resultPathToWriteTo}). VERIFY THE CHANGES BEFORE CHECKING IN!`
					);
				} else {
					fs.writeFileSync(resultPathToWriteTo, resultsString);
					throw new Error(
						`${nonDiffError}*** ACTUAL RESULTS ARE IN (${resultPathToWriteTo}).`
					);
				}
			}
		} else {
	...
	function writeToResultsFile(
		resultPathToWriteTo: string,
		resultsString: string,
		shouldCompareFullScope: boolean,
		resultsFullString: string
	): void {
		fs.writeFileSync(resultPathToWriteTo, resultsString);
		if (!shouldCompareFullScope) {
			fs.writeFileSync(
				resultPathToWriteTo + ".full-scope-result.txt",
				resultsFullString
			);
		}
	}
...
function getTestcaseResults(testCases: ITestcase[]): {
	text: string;
	results: string[];
	fullScopeString: string;
	shortScopeString: string;
} {
	let results = testCases.map(
		(testcase: ITestcase): { short: string; full: string } => {
			let prefix = testcase.testString
				? `${testcase.testString}${os.EOL}`
				: "";

			function getLastScope(scopes: string): string {
				let lastSpaceIndex = scopes.lastIndexOf(" ");
				return lastSpaceIndex > 0
					? scopes.slice(lastSpaceIndex + 1)
					: scopes;
			}
			...
			function getTestCaseString(full: boolean): string {
				return testcase.data
					.map((td) => {
						...
						let scopes = unpreprocessScopes(
							full ? td.scopes : getLastScope(td.scopes)
						);
						if (padding > 0) {
							return `${theText}${" ".repeat(padding)}${scopes}`;
						} else {
							return `${theText}${os.EOL}${" ".repeat(
								tabSize
							)}${scopes}`;
						}
					})
			...
			return {
				short: prefix + shortTestCaseString,
				full: prefix + fullTestCaseString,
			};
		}
	);
	...
	let text = normalizeString(
		testCases
			.map((tc) => tc.data)
			.map((tis: ITokenInfo[]) => tis.map((ti) => ti.text).join(""))
			.join("")
	);

	return {
		text,
		results: results.map((r) => r.full),
		fullScopeString,
		shortScopeString,
	};
}
...
suite("TLE colorization", function (this: Suite): void {
	...
	for (let resultFile of resultFiles) {
		if (
			resultFile.endsWith(".actual") ||
			resultFile.endsWith(".full-scope-result.txt")
		) {
	...
	orphanedResultFiles.forEach((orphanedFile) => {
		test(`ORPHANED: ${orphanedFile}`, () => {
			throw new Error(`Orphaned result file ${orphanedFile}`);
		});
	});
		...
		if (testFile.startsWith("TO" + "DO")) {
			...
			test(testFile, async function (this: Context): Promise<void> {
				...
				await assertUnchangedTokens(
					path.join(testFolder, testFile),
					path.join(resultsFolder, testToResultFileMap.get(testFile)!)
				);
			});
		}
	});
	...
	test("Check longest test duration", () => {
		writeToLog(
			`Longest colorization test duration: ${longestTestDuration / 1000}s`
		);
	});
});
