...
suite("ResourceInfo", () => {
	suite("split names", () => {
		function createSplitNameTest(
			nameAsJsonString: string,
			expected: string[]
		): void {
			test(nameAsJsonString, async () => {
				...
				const info = getResourceInfo(
					dt.topLevelScope.rootObject!.getPropertyValue("resources")!
						.asArrayValue!.elements[0].asObjectValue!
				)!;
		...
		createSplitNameTest("simple string/separated/again", [
			"'simple string'",
			"'separated'",
			"'again'",
		]);
		...
		createSplitNameTest("[concat(variables('a'), variables('b'))]", [
		...
		createSplitNameTest("[concat(variables('a'), '/b')]", [
			"variables('a')",
			"'b'",
		]);
		createSplitNameTest(
			"[concat(variables('a'), '/b', '/', variables('c'))]",
			["variables('a')", "'b'", "variables('c')"]
		);
	});

	suite("split types", () => {
		function createSplitTypeTest(
			typeAsJsonString: string,
			expected: string[]
		): void {
		...
		suite("invalid types", () => {
			...
			createSplitTypeTest("simple string/separated", [
				"'simple string/separated'",
			]);
			createSplitTypeTest("[]", [""]);
			createSplitTypeTest("[variables('a')]", ["variables('a')"]);
			createSplitTypeTest("[concat(variables('a'), variables('b'))]", [
				"concat(variables('a'), variables('b'))",
			]);
		});

		suite("simple strings", () => {
			createSplitTypeTest("Microsoft.Sql/servers", [
				"'Microsoft.Sql/servers'",
			]);
			createSplitTypeTest("Microsoft.Sql/servers/firewallRules", [
				"'Microsoft.Sql/servers'",
				"'firewallRules'",
			]);
			createSplitTypeTest("Microsoft.Sql/servers/firewallRules/another", [
				"'Microsoft.Sql/servers'",
				"'firewallRules'",
				"'another'",
			]);
		...
		suite("expressions", () => {
			createSplitTypeTest("[variables('sqlservers')]", [
				"variables('sqlservers')",
			]);
			createSplitTypeTest("[concat('Microsoft.Sql/servers')]", [
				"'Microsoft.Sql/servers'",
			]);
			createSplitTypeTest("[concat('Microsoft.Sql', '/servers')]", [
				"'Microsoft.Sql/servers'",
			...
			createSplitTypeTest(
				"[concat('Microsoft.Sql', '/', variables('servers'))]",
				["concat('Microsoft.Sql/', variables('servers'))"]
			);
			...
			createSplitTypeTest(
				"[concat('Microsoft.Sql', '/', variables('a'), variables('b'))]",
				[
					"concat('Microsoft.Sql/', concat(variables('a'), variables('b')))",
				]
			);

			createSplitTypeTest(
				"[concat('Microsoft.Sql', '/', variables('servers'),'/', variables('grandchild'))]",
				[
					"concat('Microsoft.Sql/', variables('servers'))",
					"variables('grandchild')",
				]
			);
			createSplitTypeTest(
				"[concat('Microsoft.Sql', '/', variables('servers'),'/', 'grandchild')]",
				[
					"concat('Microsoft.Sql/', variables('servers'))",
					"'grandchild'",
				]
			...
			createSplitTypeTest(
				"[concat('Microsoft.Sql', '/servers', variables('other'),'/', 'grandchild')]",
				[
					"concat('Microsoft.Sql/', concat('servers', variables('other')))",
					"'grandchild'",
				]
			);

			createSplitTypeTest(
				"[concat('Microsoft.Sql', variables('slashservers'),'/', 'grandchild')]",
				[
					"concat('Microsoft.Sql', variables('slashservers'))",
					"'grandchild'",
				]
			);
			createSplitTypeTest(
				"[concat('Microsoft.Sql/servers/', variables('servers'))]",
				["'Microsoft.Sql/servers'", "variables('servers')"]
			);
			createSplitTypeTest(
				"[concat('Microsoft.Sql/servers/other2', variables('servers'))]",
				[
					"'Microsoft.Sql/servers'",
					"concat('other2', variables('servers'))",
				]
			);
			createSplitTypeTest(
				"[concat('Microsoft.Sql/servers/other2/', variables('servers'))]",
				["'Microsoft.Sql/servers'", "'other2'", "variables('servers')"]
			);
		...
		createSplitTypeTest("[concat(variables('a'), '/b')]", [
			"variables('a')",
			"'b'",
		]);
		createSplitTypeTest(
		...
		createSplitTypeTest("[concat('ms.abc', '/', parameters('b'))]", [
			"concat('ms.abc/', parameters('b'))",
		]);
		createSplitTypeTest("[concat('ms.abc', '/', parameters('b'))]", [
			"concat('ms.abc/', parameters('b'))",
		]);
		...
		createSplitTypeTest(
			"[concat(parameters('a'), '/def/', parameters('b'))]",
			["parameters('a')", "'def'", "parameters('b')"]
		);
	});
	...
	suite("fullTypeExpression", () => {
		...
		suite("expressions", () => {
			...
			createFullTypeTest(
				[`parameters('abc')`, `'def'`],
				`concat(parameters('abc'), '/def')`
			);
			createFullTypeTest(
				[`'abc'`, `parameters('def')`],
				`concat('abc/', parameters('def'))`
			);
			createFullTypeTest(
				[`parameters('abc')`, `parameters('def')`],
				`concat(parameters('abc'), '/', parameters('def'))`
			);
			createFullTypeTest(
				[`parameters('abc')`, `'def'`, `parameters('ghi')`],
				`concat(parameters('abc'), '/def/', parameters('ghi'))`
			);
		});
	...
	suite("IResource getFriendlyName", () => {
		function createFriendlyNameTest(
			...
			const testName = JSON.stringify(
				`${resource.getFullTypeExpression()}: ${resource.getFullNameExpression()}`
			);
			test(`${testName} (short name)`, () => {
				const actualShort = resource.getFriendlyResourceLabel({});
				assert.deepStrictEqual(
					actualShort,
					expectedWithShortResourceName
				);
			});
			test(`${testName} (full name)`, () => {
				const actualFull = resource.getFriendlyResourceLabel({
					fullName: true,
				});
				assert.deepStrictEqual(
					actualFull,
		...
		createFriendlyNameTest(
			new ResourceInfo([`'my name'`], [`variables('typeName')`]),
			// tslint:disable-next-line: no-invalid-template-strings
			"my name (${typeName})",
			// tslint:disable-next-line: no-invalid-template-strings
			"my name (${typeName})"
		...
		createFriendlyNameTest(
			new ResourceInfo([`'my name'`], [`add(1, variables('typeName'))`]),
			// tslint:disable-next-line: no-invalid-template-strings
	...
	suite("getResourceIdExpression", () => {
		...
		createResourceIdTest(
			...
			new ResourceInfo([`'a'`], [`parameters('a')`, `'b'`]),
		...
		test("101-azure-database-migration-service", async () => {
			...
			const expected = [
				...
				},
				{
					...
					"name": "'CustomScriptExtension'",
					resourceId:
						"resourceId('Microsoft.Compute/virtualMachines/extensions', variables('sourceServerName'), 'CustomScriptExtension')",
					parent: "variables('sourceServerName')",
				},
				{
					"type": "'Microsoft.DataMigration/services'",
				...
				},
				{
					...
					"name": "'SqlToSqlDbMigrationProject'",
					resourceId:
						"resourceId('Microsoft.DataMigration/services/projects', variables('DMSServiceName'), 'SqlToSqlDbMigrationProject')",
				...
				},
				{
					...
					"name": "'default'",
					resourceId:
						"resourceId('Microsoft.Network/virtualNetworks/subnets', variables('adVNet'), 'default')",
					parent: "variables('adVNet')",
				},
				{
					"type": "'Microsoft.Storage/storageAccounts'",
				...
				},
				{
					...
					"name": "variables('databaseName')",
					resourceId:
						"resourceId('Microsoft.Sql/servers/databases', concat(variables('targetServerName')), variables('databaseName'))",
				...
				},
				{
					...
					"name": "'Import'",
					resourceId:
						"resourceId('Microsoft.Sql/servers/databases/extensions', concat(variables('targetServerName')), variables('databaseName'), 'Import')",
				...
				},
				{
					...
					"name": "'AllowAllWindowsAzureIps'",
					resourceId:
						"resourceId('Microsoft.Sql/servers/firewallrules', concat(variables('targetServerName')), 'AllowAllWindowsAzureIps')",
					parent: "concat(variables('targetServerName'))",
				},
			];
