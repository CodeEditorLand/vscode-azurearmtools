	...
	suite("List", () => {
		...
		suite("constructor(Reference.Type, Span[])", () => {
			...
			test("with undefined type", () => {
				// tslint:disable-next-line:no-any
				assert.throws(() => {
					new ReferenceList(<any>undefined);
				});
			});
			...
			test("with null spans", () => {
				// tslint:disable-next-line:no-any
				assert.throws(() => {
					new ReferenceList(DefinitionKind.Parameter, <any>null);
				});
			});
		...
		suite("addAll(Reference.List)", () => {
			...
			test("with empty list of a different type", () => {
				const list = new ReferenceList(DefinitionKind.Variable);
				assert.throws(() => {
					list.addAll(new ReferenceList(DefinitionKind.Parameter));
				});
			});
			...
			test("with non-empty list", () => {
				const list = new ReferenceList(DefinitionKind.Variable);
				list.addAll(
					new ReferenceList(DefinitionKind.Variable, [
						{ document, span: new Span(10, 20) },
					])
				);
				assert.deepStrictEqual(
					list.references.map((r) => r.span),
