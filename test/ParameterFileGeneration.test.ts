...
suite("parameterFileGeneration tests", () => {
	suite("createParameterFileContents", () => {
		function testCreateFile(
			...
			test(`${testName} (${
				onlyRequiredParams ? "all params" : "required params"
			})`, async () => {
				...
				const paramFile = createParameterFileContents(
					dt.topLevelScope,
					4,
					onlyRequiredParams
				);
		...
		function testSinglePropertyWithIndent(
			...
			test(testName, async () => {
				...
				const foundDefinition =
					dt.topLevelScope.getParameterDefinition(parameterName);
				assert(foundDefinition);
				// tslint:disable-next-line:no-non-null-assertion
				const param = createParameterFromTemplateParameter(
					dt.topLevelScope,
					foundDefinition!,
					undefined,
					spacesPerIndent
