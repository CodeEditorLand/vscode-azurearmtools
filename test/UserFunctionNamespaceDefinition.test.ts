...
suite("UserFunctionNamespaceDefinition", () => {
	suite("constructor(Json.Property)", () => {
		...
		test("no namespace name (not valid)", () => {
			const namespaceName = createStringProperty(
				"namespaceMisspelled",
				"Contoso"
			);
			const namespaceObject = new Json.ObjectValue(fakeSpan, [
				namespaceName,
			]);

			let pd = UserFunctionNamespaceDefinition.createIfValid(
				dt.topLevelScope,
				dt,
				namespaceObject
			);
		...
		suite("with members", () => {
			...
			test("getMemberDefinition", () => {
				let userNs = UserFunctionNamespaceDefinition.createIfValid(
					dt.topLevelScope,
					dt,
					namespace1
				);
				...
				assert.equal(
					userNs.getMemberDefinition("function1")!.nameValue
						.unquotedValue,
					"function1"
				);
			});
			...
			test("getMemberDefinition case insensitive", () => {
				...
				assert.equal(
					userNs.getMemberDefinition("function2")!.nameValue
						.unquotedValue,
					"Function2"
				);
			});
