	...
	suite("Span", () => {
			...
			function constructorTest(startIndex: number, length: number): void {
				test(`with [${startIndex},${length})`, () => {
					...
					assert.deepStrictEqual(
						span.endIndex,
						startIndex + (length > 0 ? length - 1 : 0),
						"Wrong endIndex"
					);
					assert.deepStrictEqual(
						span.afterEndIndex,
		...
		suite("contains()", () => {
			suite("ContainsBehavior.strict", () => {
				test("With index less than startIndex", () => {
					assert.deepStrictEqual(
						false,
						new Span(3, 4).contains(2, ContainsBehavior.strict)
					);
				});
				...
				test("With index directly after end index", () => {
					assert.deepStrictEqual(
						false,
						new Span(3, 4).contains(7, ContainsBehavior.strict)
					);
				});
			});
			...
			suite("ContainsBehavior.extended", () => {
				test("With index less than startIndex", () => {
					assert.deepStrictEqual(
						false,
						new Span(3, 4).contains(2, ContainsBehavior.extended)
					);
				});

				test("With index equal to startIndex", () => {
					assert(
						new Span(3, 4).contains(3, ContainsBehavior.extended)
					);
				});

				test("With index between the start and end indexes", () => {
					assert(
						new Span(3, 4).contains(5, ContainsBehavior.extended)
					);
				});

				test("With index equal to endIndex", () => {
					assert(
						new Span(3, 4).contains(6, ContainsBehavior.extended)
					);
				});
				...
				test("With index directly after end index", () => {
					...
					assert.deepStrictEqual(
						true,
						new Span(3, 4).contains(7, ContainsBehavior.extended)
					);
				});
				...
				test("With index two after end index", () => {
					assert.deepStrictEqual(
						false,
						new Span(3, 4).contains(8, ContainsBehavior.extended)
					);
				});
			});
			...
			suite("ContainsBehavior.enclosed", () => {
				test("With index less than startIndex", () => {
					assert.deepStrictEqual(
						false,
						new Span(3, 4).contains(2, ContainsBehavior.enclosed)
					);
				});
				...
				test("With index equal to startIndex", () => {
					...
					assert.equal(
						new Span(3, 4).contains(3, ContainsBehavior.enclosed),
						false
					);
				});
				...
				test("With index between the start and end indexes", () => {
					assert(
						new Span(3, 4).contains(5, ContainsBehavior.enclosed)
					);
				});

				test("With index equal to endIndex", () => {
					assert(
						new Span(3, 4).contains(6, ContainsBehavior.enclosed)
					);
				});
				...
				test("With index directly after end index", () => {
					assert.deepStrictEqual(
						false,
						new Span(3, 4).contains(7, ContainsBehavior.enclosed)
					);
				});
			});
		});
		...
		suite("intersect()", () => {
			...
			test("second span to left", () => {
				assert.deepEqual(
					Span.fromStartAndAfterEnd(10, 20).intersect(
						Span.fromStartAndAfterEnd(0, 9)
					),
					undefined
				);
			});
			...
			test("second touches the left", () => {
				assert.deepEqual(
					Span.fromStartAndAfterEnd(10, 20).intersect(
						Span.fromStartAndAfterEnd(0, 10)
					),
			...
			test("second span is superset", () => {
				assert.deepEqual(
					Span.fromStartAndAfterEnd(10, 20).intersect(
						Span.fromStartAndAfterEnd(0, 21)
					),
	...
	suite("Position", () => {
		suite("constructor(number,number)", () => {
			...
			test("With undefined _line", () => {
				// tslint:disable-next-line:no-any
				assert.throws(() => {
					new LineColPos(<any>undefined, 3);
				});
			});
			...
			test("With undefined _column", () => {
				// tslint:disable-next-line:no-any
				assert.throws(() => {
					new LineColPos(2, <any>undefined);
				});
			});
	...
	suite("Issue", () => {
		suite("constructor(Span,string)", () => {
			test("With null span", () => {
				// tslint:disable-next-line:no-any
				assert.throws(() => {
					new Issue(<any>null, "error message", IssueKind.tleSyntax);
				});
			});
			...
			test("With null message", () => {
				// tslint:disable-next-line:no-any
				assert.throws(() => {
					new Issue(new Span(4, 1), <any>null, IssueKind.tleSyntax);
				});
			});
			...
			test("With empty message", () => {
				assert.throws(() => {
					new Issue(new Span(3, 2), "", IssueKind.tleSyntax);
				});
			});
