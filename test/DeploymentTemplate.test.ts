...
import {
	...
	Span,
	TemplateScope,
	UnrecognizedUserFunctionIssue,
	UnrecognizedUserNamespaceIssue,
...
suite("DeploymentTemplate", () => {
	...
	suite("constructor(string)", () => {
		test("Null stringValue", () => {
			...
			assert.throws(() => {
				new DeploymentTemplateDoc(<any>undefined, fakeId, 0);
			});
		});
		...
		test("JSON stringValue with one parameter definition", () => {
			...
			const parameterDefinitions: IParameterDefinition[] =
				dt.topLevelScope.parameterDefinitions;
		...
		test("JSON stringValue with one parameter definition with undefined description", () => {
			const dt = new DeploymentTemplateDoc(
				"{ 'parameters': { 'num': { 'type': 'number', 'metadata': { 'description': null } } } }",
				fakeId,
				0
			);
			assert.deepStrictEqual(fakeId.fsPath, dt.documentUri.fsPath);
		...
		test("JSON stringValue with one parameter definition with empty description", () => {
			const dt = new DeploymentTemplateDoc(
				"{ 'parameters': { 'num': { 'type': 'number', 'metadata': { 'description': '' } } } }",
				fakeId,
				0
			);
			assert.deepStrictEqual(fakeId.fsPath, dt.documentUri.fsPath);
		...
		test("JSON stringValue with one parameter definition with non-empty description", () => {
			const dt = new DeploymentTemplateDoc(
				"{ 'parameters': { 'num': { 'type': 'number', 'metadata': { 'description': 'num description' } } } }",
				fakeId,
				0
			);
			assert.deepStrictEqual(fakeId.fsPath, dt.documentUri.fsPath);
		...
		test("JSON stringValue with one variable definition", () => {
			...
			assert.deepStrictEqual(
				dt.topLevelScope.variableDefinitions.length,
				1
			);
			assert.deepStrictEqual(
				dt.topLevelScope.variableDefinitions[0].nameValue.toString(),
				"a"
			);

			const variableDefinition: Json.StringValue | undefined =
				Json.asStringValue(
					dt.topLevelScope.variableDefinitions[0].value
				);
		...
		test("JSON stringValue with two variable definitions", () => {
			...
			const a: Json.StringValue | undefined = Json.asStringValue(
				dt.topLevelScope.variableDefinitions[0].value
			);
			if (!a) {
				throw new Error("failed");
			...
			assert.deepStrictEqual(
				dt.topLevelScope.variableDefinitions[1].nameValue.toString(),
				"b"
			);
			const b: Json.NumberValue | undefined = Json.asNumberValue(
				dt.topLevelScope.variableDefinitions[1].value
			);
			if (!b) {
				throw new Error("failed");
			}
	...
	suite("errors", () => {
		...
		test("with one undefined parameter error deployment template", () => {
			...
			const expectedErrors = [
				new Issue(
					new Span(23, 6),
					'Undefined parameter reference: "test"',
		...
		test("with one undefined variable error deployment template", () => {
			...
			const expectedErrors = [
				new Issue(
					new Span(22, 6),
					'Undefined variable reference: "test"',
		...
		test("with reference() call inside a different expression in a variable definition", () => {
			const dt = new DeploymentTemplateDoc(
				`{ "variables": { "a": "[concat(reference('test'))]" } }`,
				fakeId,
				0
			);
			const errors = dt.getErrors(undefined);
		...
		test("with property access on variable reference without variable name", () => {
			...
			assert.deepStrictEqual(errors, [
				new IncorrectArgumentsCountIssue(
					new Span(35, 11),
					"The function 'variables' takes 1 argument.",
					"variables",
					0,
					1,
					1
		...
		test("with property access on string variable reference", () => {
			...
			assert.deepStrictEqual(errors, [
				new Issue(
					new Span(51, 1),
					`Property "b" is not a defined property of "variables('a')".`,
		...
		test("with undefined variable reference grandchild property", () => {
			const dt = new DeploymentTemplateDoc(
				`{ "variables": { "a": { "b": {} } }, "z": "[variables('a').b.c]" }`,
				fakeId,
				0
			);
			const errors = dt.getErrors(undefined);
			assert.deepStrictEqual(errors, [
				new Issue(
					new Span(61, 1),
					`Property "c" is not a defined property of "variables('a').b".`,
					IssueKind.undefinedVarProp
				),
			]);
		});
		...
		test("with undefined variable reference child and grandchild properties", () => {
			const dt = new DeploymentTemplateDoc(
				`{ "variables": { "a": { "d": {} } }, "z": "[variables('a').b.c]" }`,
				fakeId,
				0
			);
			const errors = dt.getErrors(undefined);
	...
	suite("get functionCounts()", () => {
		...
		test("with one TLE function used multiple times in deployment template", () => {
			const dt = new DeploymentTemplateDoc(
				"{ 'variables': { 'name': '[concat()]', 'name2': '[concat(1, 2)]', 'name3': '[concat(2, 3)]' } }",
				fakeId,
				0
			);
			const expectedHistogram = new Histogram();
		...
		test("with the same string repeated in multiple places (each use should get counted once, even though the strings are the exact same and may be cached)", () => {
			const dt = new DeploymentTemplateDoc(
				"{ 'name': '[concat()]', 'height': '[concat()]', 'width': \"[concat()]\" }",
				fakeId,
				0
			);
			assert.deepStrictEqual(
				3,
				dt.getFunctionCounts().getCount("concat(0)")
			);
			assert.deepStrictEqual(
				3,
				dt.getFunctionCounts().getCount("concat")
			);
		});
	});
	...
	suite("get jsonParseResult()", () => {
		...
		test("With comments", () => {
			...
			assert.notEqual(
				dt.topLevelScope.getParameterDefinition("storageAccountName"),
				null
			);
		});
	});
	...
	suite("get parameterDefinitions()", () => {
		...
		test("with parameter with metadata and description", () => {
			const dt = new DeploymentTemplateDoc(
				"{ 'parameters': { 'a': { 'metadata': { 'description': 'b' } } } }",
				fakeId,
				0
			);
			const parameterDefinitions: IParameterDefinition[] =
	...
	suite("getParameterDefinition(string)", () => {
		test("with empty", () => {
			const dt = new DeploymentTemplateDoc(
				"{ 'parameters': { 'apples': { 'type': 'string' }, 'bananas': { 'type': 'integer' } } }",
				fakeId,
				0
			);
			assert.deepStrictEqual(
				undefined,
				dt.topLevelScope.getParameterDefinition("''")
			);
		});
		...
		test("with no parameters definition", () => {
			...
			assert.deepStrictEqual(
				undefined,
				dt.topLevelScope.getParameterDefinition("spam")
			);
		});
		...
		test("with unquoted match", () => {
			...
			const apples: IParameterDefinition | undefined =
				dt.topLevelScope.getParameterDefinition("apples");
			if (!apples) {
				throw new Error("failed");
			}
			...
			assert.deepStrictEqual(
				apples.nameValue.span,
				new Span(18, 8),
				"Wrong name.span"
			);
			assert.deepStrictEqual(
				apples.nameValue.unquotedSpan,
				new Span(19, 6),
				"Wrong name.unquotedSpan"
			);
		});
	...
	suite("getVariableDefinition(string)", () => {
		test("with empty", () => {
			const dt = new DeploymentTemplateDoc(
				"{ 'variables': { 'apples': 'yum', 'bananas': 'good' } }",
				fakeId,
				0
			);
			assert.deepStrictEqual(
				undefined,
				dt.topLevelScope.getVariableDefinition("")
			);
		});
		...
		test("with no variables definition", () => {
			...
			assert.deepStrictEqual(
				undefined,
				dt.topLevelScope.getVariableDefinition("spam")
			);
		});
		...
		test("with unquoted match", () => {
			...
			const apples: IVariableDefinition | undefined =
				dt.topLevelScope.getVariableDefinition("apples");
			if (!apples) {
				throw new Error("failed");
			}
			assert.deepStrictEqual(apples.nameValue.toString(), "apples");

			const value: Json.Value | undefined = Json.asStringValue(
				apples.value
			);
			if (!value) {
				throw new Error("failed");
		...
		test("with one-sided-quote match", () => {
			...
			const value: Json.StringValue | undefined = Json.asStringValue(
				apples.value
			);
			if (!value) {
				throw new Error("failed");
		...
		test("with multiple case insensitive matches", () => {
			const dt = new DeploymentTemplateDoc(
				"{ 'variables': { 'apples': 'yum', 'APPLES': 'good' } }",
				fakeId,
				0
			);
			...
			const applesValue: Json.StringValue | undefined =
				Json.asStringValue(APPLES.value);
			if (!applesValue) {
				throw new Error("failed");
			}
	...
	suite("getContextFromDocumentLineAndColumnIndexes(number, number)", () => {
		test("with empty deployment template", () => {
			const dt = new DeploymentTemplateDoc("", fakeId, 0);
			const context = dt.getContextFromDocumentLineAndColumnIndexes(
				0,
				0,
				undefined
			);
	...
	suite("findReferences(Reference.Type, string)", () => {
		test("with parameter type and no matching parameter definition", () => {
			...
			const list: ReferenceList = findReferences(
				dt,
	...
	suite("ReferenceInVariableDefinitionJSONVisitor", () => {
		suite("constructor(DeploymentTemplate)", () => {
			...
			testWithLanguageServer(
				"expecting error: reference in variable definition",
			...
			testWithLanguageServer(
				"expecting error: reference in variable definition inside user function",
				async function (this: Context): Promise<void> {
					await testDiagnostics(
						{
							"variables": {
		...
		suite("visitStringValue(Json.StringValue)", () => {
			test("with non-TLE string", () => {
				...
				const visitor = new ReferenceInVariableDefinitionsVisitor(dt);
				const variables: Json.StringValue = Json.asObjectValue(
					dt.jsonParseResult.value
				)!.properties[0].nameValue;
			...
			test("with TLE string with reference() call", () => {
				...
				const visitor = new ReferenceInVariableDefinitionsVisitor(dt);
				const dtObject: Json.ObjectValue | undefined =
					Json.asObjectValue(dt.jsonParseResult.value);
				const variablesObject: Json.ObjectValue | undefined =
					Json.asObjectValue(dtObject!.getPropertyValue("variables"));
				const tle: Json.StringValue | undefined = Json.asStringValue(
					variablesObject!.getPropertyValue("a")
				);
	...
	suite(
		...
		function (this: Suite): void {
			this.timeout(60000);

			async function exercisePositionContextAtEveryPointInTheDoc(
				json: string
			): Promise<void> {
				await exercisePositionContextAtRandomPointsInTheDoc(
					json,
					json.length + 1
				); // length+1 so we include past the last character as a position
			}

			async function exercisePositionContextAtRandomPointsInTheDoc(
				json: string,
				numberOfIndicesToTest: number
			): Promise<void> {
				...
				for (let i = 0; i < numberOfIndicesToTest; ++i) {
					...
					try {
						...
						let pc = dt.getContextFromDocumentCharacterIndex(
							index,
							undefined
						);
						pc.getReferences();
						...
						dt.getCodeActions(
							undefined,
							getVSCodeRangeFromSpan(dt, new Span(index, 0)),
							getEmptyCodeActionContext()
						);
						/*const items =*/ await pc.getCompletionItems(
							undefined,
							4
						);
				...
				const json = `
        {
			...
			test("try parsing the document with a single character deleted (repeat through the whole document)", async function (this: Context): Promise<void> {
				...
				for (let i = 0; i < template.length; ++i) {
					...
					let partialTemplate =
						template.slice(0, i) + template.slice(i + 1);
					let dt = parseTemplate(partialTemplate);
					findReferences(
						dt,
						DefinitionKind.Parameter,
			...
			test("Random modifications", async function (this: Context): Promise<void> {
				...
				for (let i = 0; i < 5000; ++i) {
					...
					if (modifiedTemplate.length > 0 && Math.random() < 0.5) {
						...
						let position =
							Math.random() * (modifiedTemplate.length - 1);
						let length =
							Math.random() *
							Math.max(5, modifiedTemplate.length);
						modifiedTemplate = modifiedTemplate.slice(
							position,
							position + length
						);
					} else {
						...
						modifiedTemplate =
							modifiedTemplate.slice(0, position) +
							s +
							modifiedTemplate.slice(position);
					}
					...
					try {
						...
						if (
							parseError(err).message.includes("Malformed marker")
						) {
...
//345
	...
	suite("getDocumentPosition", () => {
		function createGetDocumentPositionTest(
			json: string,
			index: number,
			expectedPosition: LineColPos
		): void {
