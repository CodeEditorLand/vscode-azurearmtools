...
suite("Linked templates regressions", () => {
	suite(
		"variables and parameters inside templateLink object refer to the parent's scope",
		() => {
			testWithLanguageServer(
				"Regress #792: Regression from 0.10.0: top-level parameters not recognized in nested template properties",
				async () => {
			...
			suite(
				"Regress #773: Regression from 0.10.0: top-level parameters not recognized in nested template properties",
				() => {
					testWithLanguageServer(
						"linked-templates-scope.json",
						async () => {
							await testDiagnosticsFromFile(
								"templates/linked-templates-scope.json",
								{
	...
	suite("Error location inside linked and `nested tem`plates", async () => {
		...
		if (!isWin32) {
			testWithLanguageServer(
				"Nested template wrong param type",
				async () => {
			...
			testWithLanguageServer(
				"Nested template missing properties",
				async () => {
					await testDiagnostics(
						{
							"$schema":
