...
suite("TreeView", async (): Promise<void> => {
	...
	suite("JsonOutlineProvider", async (): Promise<void> => {
		...
		function treeMap<T extends INode<T>, U extends INode<U>>(
			tree: T[],
			visit: (node: T) => U
		): INode<U>[] {
		...
		async function testLabels(
			template: string,
			expected: Partial<ITestTreeItem>[]
		): Promise<void> {
				...
				[
					...
					},
					{
						...
						children: [
							...
							getIconObject(
								"trafficmanagerprofiles.svg",
								undefined
							),
							getIconObject(
								"virtualnetworkgateways.svg",
								undefined
							),
