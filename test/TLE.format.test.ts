...
suite("TLE format", () => {
	...
	function createTleFormatTest(
		...
		test(`${testName} (${unquotedValue})`, () => {
			...
			const result = parseResult.expression?.format({
				multiline: { tabSize: 4 },
			});
	...
	suite("funcs with single literal arg - one line", () => {
		...
		createTleFormatTest("single number arg", `json(-1.23)`, `json(-1.23)`);

		createTleFormatTest("no args", `json()`, `json()`);
		...
		createTleFormatTest("", `copyIndex(1)`, `copyIndex(1)`);
	...
	suite("array access", () => {
		...
		createTleFormatTest("no index", `variables('a')[]`, `variables('a')[]`);
