...
suite("ParentAndChildCodeLenses", () => {
	type Expected = [
		/*resourceName:*/ string,
		/*lensTitle:*/ string,
		/*targetStartLines:*/ number[] | undefined,
	];
	...
	function createTest(
		...
		test(name, async () => {
			const dt = parseTemplate(template, [], { ignoreInfos: true });
			const lenses: ParentOrChildCodeLens[] = dt
				.getCodeLenses(undefined)
				.filter(
					(cl) => cl instanceof ParentOrChildCodeLens
				) as ParentOrChildCodeLens[];
			...
			for (const lens of lenses) {
				...
				const enclosingResources: IJsonResourceInfo[] = allInfos.filter(
					(info) =>
						info.resourceObject.span.startIndex <=
							lens.span.startIndex &&
						info.resourceObject.span.endIndex >= lens.span.endIndex
				);
				// If more than one, take the one that starts last, which will be the innermost
				const res = sortArray(
					enclosingResources,
					(info) => info.resourceObject.startIndex,
					{ descending: true }
				)[0];
				assert(
					res,
					"Could not find a resource at the location of the code lens"
				);
				const targetStartLines = lens.command?.arguments
					? (<IGotoResourcesArgs>(
							lens.command.arguments[0]
					  )).targets.map((loc) => loc.range.start.line)
					: undefined;
				actual.push([
	...
	createTest(
		...
		{
			resources: [{ name: "self", type: "microsoft.abc/def" }],
		},
		[
			// Not showing if not applicable (#1009) ["self", "No parent", undefined],
			// Not showing if not applicable (#1009) ["self", "No children", undefined]
	...
	createTest(
		"nested template",
		{
			resources: [
				...
				{
					condition:
						"[equals(parameters('vnet-new-or-existing'), 'new')]",
					apiVersion: "2019-11-01",
		...
		[
			...
			["Subnet-1", "Parent: virtualNetwork1 (virtualNetworks)", [13]],
			/* Not showing if not applicable (#1009)
            [
                "Subnet-1",
                "No children",
			...
			["Subnet-2", "Parent: virtualNetwork1 (virtualNetworks)", [13]],
			/* Not showing if not applicable (#1009)
            [
                "Subnet-2",
                "No children",
			...
			["Subnet-1", "Parent: virtualNetwork1 (virtualNetworks)", [45]],
			/* Not showing if not applicable (#1009)
            [
                "Subnet-1",
                "No children",
			...
			["Subnet-2", "Parent: virtualNetwork1 (virtualNetworks)", [45]],
			/* Not showing if not applicable (#1009)
            [
                "Subnet-2",
                "No children",
