...
suite("Format document", function (this: Suite): void {
		...
		testWithLanguageServer(
			...
			async function (this: Context): Promise<void> {
				...
				if (source.match(/\.jsonc?$/)) {
					sourceIsFile = true;
					jsonUnformatted = fs
						.readFileSync(resolveInTestFolder(source))
						.toString()
						.trim();
				}
				...
				if (jsonExpected.match(/\.jsonc?$/)) {
					jsonExpected = fs
						.readFileSync(resolveInTestFolder(expected))
						.toString()
						.trim();
				}
				...
				if (source === "format-me.json") {
					assert(
						!jsonUnformatted.match(/[\n\r]/),
						"The input file to format should only be one line of JSON. Did it get corrupted?"
					);
				}

				let filePath = getTempFilePath(
					sourceIsFile ? `temp.${path.basename(source)}` : undefined
				);
				...
				if (sourceIsFile && jsonUnformatted !== jsonExpected) {
					assert.notEqual(
						jsonUnformatted,
						output,
						"The format command did not modify the editor."
					);
				}
	...
	suite("Format entire document", () => {
		testFormat(
			"templates/format-me.jsonc",
			"templates/format-me.jsonc",
			"templates/format-me.expected.full.jsonc"
		);
		testFormat(
			"format twice",
			"templates/format-me.expected.full.jsonc",
