...
suite("Backend validation", () => {
	...
	testWithLanguageServer(
		"reference not valid inside a function definition",
		async () => {
			await testDiagnostics(
				{
					"$schema":
							...
							"members": {
								"storageUri": {
									...
									"output": {
										"type": "string",
										"value":
											"[reference(concat('Microsoft.Storage/storageAccounts/', parameters('storageAccountName')), '2018-02-01').primaryEndpoints.blob]",
									},
								},
							},
						},
	...
	suite(
		"#695 https://docs.microsoft.com/en-us/azure/azure-resource-manager/templates/copy-outputs",
		() => {
			testWithLanguageServer(
				"copy loop in outputs 1 - resource group deployment",
				async () => {
					await testDiagnostics(
						"templates/regression/695a-rg.json",
						{
							parametersFile:
								"templates/regression/695a-rg.parameters.json",
						},
						[]
					);
				}
	...
	testWithLanguageServer("param-with-keyvault-reference.json", async () => {
		await testDiagnosticsFromFile(
			...
			{
				parametersFile:
					"templates/param-with-keyvault-reference.params.json",
			},
			[
