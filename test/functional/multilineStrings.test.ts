...
for (let eolIndex of [0, 1]) {
	...
	suite(`multilineStrings (EOL=${eolLabel})`, () => {
		...
		suite("unindentMultilineString", () => {
			function testUnindentMultilineString(
				testName: string,
				input: string,
				expected: string,
				...
				test(`${testName}`, async () => {
					try {
						...
						const indented = unindentMultilineString(
							input,
							ignoreFirstLine
						);
			...
			testUnindentMultilineString(
				"single line indented 1",
				" a b  c",
				"a b  c"
			);
			testUnindentMultilineString(
				"single line indented 5",
				"     a b  c",
				"a b  c"
			);
			...
			testUnindentMultilineString(
				"two lines just spaces",
				`   ${EOL}   `,
				`${EOL}`
			);
			testUnindentMultilineString(
				"two lines just spaces #2",
				`   ${EOL} `,
				`  ${EOL}`
			);
			testUnindentMultilineString(
				"two lines just spaces #3",
				` ${EOL}   `,
				`${EOL}  `
			);

			testUnindentMultilineString(
				"two lines no indentation",
				`a${EOL}a`,
				`a${EOL}a`
			);
			testUnindentMultilineString(
				"two lines with indentation #1",
				`  a${EOL}  a`,
				`a${EOL}a`
			);
			testUnindentMultilineString(
				"two lines with indentation #2",
				`      abc${EOL}  def`,
				`    abc${EOL}def`
			);
			testUnindentMultilineString(
				"two lines with indentation #3",
				`  a b c${EOL}      def`,
				`a b c${EOL}    def`
			);
