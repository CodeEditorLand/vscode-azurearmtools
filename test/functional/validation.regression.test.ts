...
} from "../support/testWithLanguageServer";
import { isWin32 } from "../testConstants";

suite("Validation regression tests", () => {
	testWithLanguageServer(
		"Template validation error for evaluated variables (https://github.com/microsoft/vscode-azurearmtools/issues/380)",
		async () =>
							...
							"WEBNSGRule": [
								{
									...
									"properties": {
										...
										"destinationPortRange":
											"[variables(parameters('Environment')).WebPort[1]]",
									},
								},
							],
						},
			...
			await testDiagnostics(
				{
					...
					"parameters": {
						"workbookDisplayName": {
							...
							"metadata": {
								"description":
									"The friendly name for the workbook that is used in the Gallery or Saved List.  This name must be unique within a resource group.",
							},
						},
						"workbookType": {
							"type": "string",
							...
							"metadata": {
								"description":
									"The gallery that the workbook will been shown under. Supported values include workbook, tsg, etc. Usually, this is 'workbook'",
							},
						},
						"workbookSourceId": {
							"type": "string",
							...
							"metadata": {
								"description":
									"The id of resource instance to which the workbook will be associated",
							},
						},
						"workbookId": {
							"type": "string",
							...
							"metadata": {
								"description":
									"The unique guid for this workbook instance",
							},
						},
					},
					"resources": [
					...
					"outputs": {
						"workbookId": {
							"type": "string",
							"value":
								"[resourceId( 'microsoft.insights/workbooks', parameters('workbookId'))]",
						},
					},
					"$schema":
								...
								"template": {
									...
									"outputs": {
										"paramFromOuter": {
											"type": "string",
											"value":
												"[parameters('paramFromOuter')]",
										},
										"variableFromOuter": {
											"type": "string",
											"value":
												"[parameters('variableFromOuter')]",
										},
										"variableFromInner": {
											"type": "string",
											"value":
												"[variables('innerVariable')]",
										},
									},
								},
								"parameters": {
	...
	suite(
		...
		() => {
			testWithLanguageServer(
				`#730 scenario a: Template validation failed: Value cannot be null. (Parameter 'o')`,
				async () => {
					await testDiagnostics(
						"templates/regression/730a.json",
						{
							parametersFile:
								"templates/regression/730a.params.json",
						},
						[]
					);
				}
			...
			if (!isWin32) {
				testWithLanguageServer(
					`#730 scenario b: Template validation failed: The template resource '' at line '<null>' and column '<null>' is not valid. The name property cannot be null or empty.`,
					async () => {
						await testDiagnostics(
							"templates/regression/730b.json",
							{
								parametersFile:
									"templates/regression/730b.parameters.json",
							},
							[
			...
			testWithLanguageServer(
				`#730 scenario c: Template function copyIndex not expected at this location`,
				async () => {
					await testDiagnostics(
						"templates/regression/730c.json",
						{
							parametersFile:
								"templates/regression/730c.params.json",
						},
						[
	...
	testWithLanguageServer(
		`#904 Validation giving false positives for top-level params used in outer-scoped nested template`,
		async () => {
			await testDiagnostics(
	...
	testWithLanguageServer(
		`#1056 dateTimeAdd() in certain scenarios gives a template validation error`,
		async () => {
			await testDiagnostics(
				"templates/regression/1056-dateTimeAdd.json",
				{
					parametersFile:
						"templates/regression/1056-dateTimeAdd.parameters.json",
				},
				[]
			);
		}
	...
	testWithLanguageServer(
		`#831 Validation error with resourceGroup() tags when using parameter file`,
		async () => {
			await testDiagnostics(
	...
	if (!isWin32) {
		testWithLanguageServer(
			`#1060-rg Validation of template function "deployment" - RG scope - should give error`,
			async () => {
				await testDiagnostics(
					"templates/regression/1060-rg.json",
					{
						parametersFile:
							"templates/regression/1060-rg.parameters.json",
	...
	testWithLanguageServer(
		`#1060-mg Validation of template function "deployment" - MG scope - should succeed`,
		async () => {
			await testDiagnostics(
				"templates/regression/1060-mg.json",
				{
					parametersFile:
						"templates/regression/1060-mg.parameters.json",
				},
				[]
			);
		}
	...
	testWithLanguageServer(
		`#967 Validation needs to recognize new top-level "scope" property `,
		async () => {
			await testDiagnostics(
				"templates/regression/967-nested-with-scope-property.json",
				{
					parametersFile:
						"templates/regression/967-nested-with-scope-property.parameters.json",
