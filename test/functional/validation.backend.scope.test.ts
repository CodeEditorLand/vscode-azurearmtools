...
suite("Backend validation deployment scope", () => {
	suite("Subscription deployment scope", () => {
		testWithLanguageServer(
			"subscription() with subscription schema works",
			() =>
				testDiagnostics(
					{
						"$schema":
	...
	suite("Resource Group deployment scope", () => {
		testWithLanguageServer(
			"resourceGroup() with resource group schema works",
			() =>
				testDiagnostics(
					{
						"$schema":
	...
	testWithLanguageServer("management group deployment scope", async () => {
		await testDiagnostics("templates/scopes/1055-mg.json", {}, []);
	});
	...
	testWithLanguageServer("subscription deployment scope", async () => {
		await testDiagnostics("templates/scopes/1055-sub.json", {}, []);
	});
