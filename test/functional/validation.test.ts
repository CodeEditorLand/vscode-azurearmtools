...
suite("General validation tests (all diagnostic sources)", () => {
	suite("scoped deployments", () => {
		...
		testWithLanguageServerAndRealFunctionMetadata(
			"resource group deployment - old root schema",
			async () => {
				await testDiagnostics(
		...
		testWithLanguageServerAndRealFunctionMetadata(
			"resource group deployment - new root schema",
			async () => {
				await testDiagnostics(
		...
		testWithLanguageServerAndRealFunctionMetadata(
			"subscription deployment with nested resource group deployment",
			async () => {
				await testDiagnostics(
					...
					[
						`${testMessages.nestedTemplateNoValidation(
							"storageDeployment"
						)} (arm-template (expressions)) [30,21-30,40]`,
					]
				);
			}
		);
				...
				await testDiagnostics(
					...
					[
						`${testMessages.linkedTemplateNoValidation(
							"linkedDeployment1"
						)} (arm-template (expressions)) [32,15-32,34]`,
						`${testMessages.linkedTemplateNoValidation(
							"linkedDeployment2"
						)} (arm-template (expressions)) [15,15-15,34]`,
					]
				);
			}
		);
