...
suite("Acceptance validation tests (all sources)", () => {
	function createAcceptanceTest(
		templatePath: string,
		options: ITestDiagnosticsOptions,
		expected: string[]
	): void {
		testWithLanguageServer(
			`validation acceptance test ${templatePath}`,
			async () => {
				await testDiagnosticsFromFile(templatePath, options, expected);
			}
		);
	}

	testWithLanguageServer(
		"minimal deployment template - no errors",
		async () => {
			await testDiagnostics(minimalDeploymentTemplate, {}, []);
		}
	);
	...
	createAcceptanceTest(
		...
		[
			...
			testMessages.linkedTemplateNoValidation(
				"Acronis.acronis-backup-lin-20180305162104",
				"[197,21-197,64]"
			),
			testMessages.nestedTemplateNoValidation(
				"[concat('BackupVaultPolicy', '-', parameters('backupVaultName'), '-', parameters('backupPolicyName'))]",
				"[271,21-271,125]"
			),
			testMessages.nestedTemplateNoValidation(
				"[concat(parameters('virtualMachineName'), '-' , 'BackupIntent')]",
				"[312,21-312,87]"
			),
	...
	createAcceptanceTest("templates/udf-xmas-tree2.jsonc", {}, []);
});
