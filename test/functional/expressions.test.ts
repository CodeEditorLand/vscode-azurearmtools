...
import {
	diagnosticSources,
	IDeploymentParameterDefinition,
	IDeploymentTemplate,
	testDiagnostics,
...
suite("Expressions functional tests", () => {
	...
	function testExpression(
		...
		testWithLanguageServer(testName || expression, async () => {
			...
			function addParamIfUsed(
				paramName: string,
				definition: IDeploymentParameterDefinition
			): void {
				if (
					expression.match(
						new RegExp(
							`parameters\\s*\\(\\s*'${paramName}'\\s*\\)`,
							"i"
						)
					)
				) {
	...
	function testLiteralExpression(
		...
		testExpression(
			`testLiteralExpression("${literalExpression}")`,
			`[concat(${literalExpression})]`,
			expected
		);
	}
	...
	suite("Plain strings vs expressions", () => {
		...
		suite(
			"1: Starts with [ but doesn't end with ] -> consider a string, don't change",
			() => {
		...
		suite(
			"2: Starts with [[ -> replace first [[ with [, consider a string",
			() => {
				// "[[one]" // -> string: "[one]"
				testExpression("", "[[one]", []);
				// "[[one]two]" // -> string: "[one]two]"
	...
	suite("Variables/Parameters", () => {
        ...
        */
		...
		suite("Undefined parameters/variables", () => {
			...
			testExpression(
				"with child property access from undefined variable reference",
				"[variables('undefVar').apples]",
				[
			...
			// No errors should be reported for a property access to an undefined variable, because the top priority error for the developer to address is the undefined variable reference.
			testExpression(
				"with grandchild property access from undefined variable reference",
				"[variables('undefVar').apples.bananas]",
				[
			...
			testExpression(
				"with child property access from variable reference to non-object variable",
				"[variables('intVar').apples]",
				[
			...
			testExpression(
				"with grandchild property access from variable reference to non-object variable",
				"[variables('stringVar').apples.bananas]",
				[
	...
	suite("Array/Object access", () => {
		...
		testExpression(
			"",
			"[variables('arrayVar')[parameters('intParam')]]",
			[]
		);
