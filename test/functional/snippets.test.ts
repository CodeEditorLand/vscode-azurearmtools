...
suite("Snippets functional tests", () => {
	...
	function createSnippetTests(snippetsFile: string): void {
		suite(snippetsFile, () => {
			const snippetsFilePath = resolveInTestFolder(
				path.join("..", "assets", snippetsFile)
			);
			const resourceSnippetsFolderPath = resolveInTestFolder(
				path.join("..", "assets", "resourceSnippets")
			);
			const snippetsFromMainFile = <{ [name: string]: ISnippet }>(
				JSON.parse(
					stripJsonComments(
						fse.readFileSync(snippetsFilePath).toString()
					)
				)
			);
			const resourceSnippets = fse
				.readdirSync(resourceSnippetsFolderPath)
				.filter((f) => f !== "README.jsonc")
				.map((file) => file.replace(/(.*)\.snippet\.json$/, "$1"));
			const snippetExpectedResultsNames = fse
				.readdirSync(resolveInTestFolder("snippets/expected"))
				.map((file) => file.replace(/\.snippetresult\.json$/, ""));
			let snippetNames = Object.getOwnPropertyNames(snippetsFromMainFile)
				.concat(resourceSnippets)
				.concat(snippetExpectedResultsNames);
					...
					for (let i = 0; i < 1; ++i) {
						testWithPrep(
							...
							async function (this: Context): Promise<void> {
								...
								assert(
									snippet !== undefined,
									`Couldn't find snippet ${snippetName}`
								);
								await testSnippet(
									this,
									snippetsFilePath,
	...
	async function testSnippet(
		...
		const template =
			overrideTemplateForSnippet[snippetName] !== undefined
				? overrideTemplateForSnippet[snippetName]
				: defaultTemplate;

		// tslint:disable-next-line: strict-boolean-expressions
		const expectedDiagnostics = (
			overrideExpectedDiagnostics[snippetName] || []
		).sort();
		// tslint:disable-next-line: strict-boolean-expressions
		const snippetInsertComment: string =
			overrideInsertPosition[snippetName] || "//Insert here: resource";
		const snippetInsertIndex: number =
			template.indexOf(snippetInsertComment);
		const snippetInsertLength: number = snippetInsertComment.length;
		assert(
			snippetInsertIndex >= 0,
			`Couldn't find location to insert snippet (looking for "${snippetInsertComment}")`
		);
		const fakeDt = new DeploymentTemplateDoc(
			template,
			Uri.file("fake template"),
			0
		);
		const snippetInsertPos = getVSCodePositionFromPosition(
			fakeDt.getContextFromDocumentCharacterIndex(
				snippetInsertIndex,
				undefined
			).documentPosition
		);
		const snippetInsertEndPos = getVSCodePositionFromPosition(
			fakeDt.getContextFromDocumentCharacterIndex(
				snippetInsertIndex + snippetInsertLength,
				undefined
			).documentPosition
		);
		...
		const diagnosticOptions: IGetDiagnosticsOptions = {
			ignoreSources: overrideIgnoreSchemaValidation[snippetName]
				? [diagnosticSources.schema]
				: [],
		};
		...
		await simulateCompletion(editor, snippet.prefix, undefined);
		...
		const outputPath = path.join(
			resultsFolder,
			`${snippetName}.snippetresult.json`
		);
		...
		messages = messages.map((m) =>
			m.replace(
				/and column '[0-9]+' has incorrect segment lengths/,
				"and column '{COL}' has incorrect segment lengths"
			)
		);
		...
		const expectedNormalized = stringify(
			JSON.parse(removeComments(expected))
		);
		const actualNormalized = stringify(
			JSON.parse(removeComments(docTextAfterInsertion))
		);
		assertEx.strictEqual(
			actualNormalized,
			expectedNormalized,
			{},
			`Actual result from ${outputPath} did not match expected result in ${expectedPath}`
		);
		...
		function validateSnippet(): void {
			...
			errorIfTextMatches(
				snippetText,
				/\$\$/,
				`Instead of $$ in snippet, use \\$`
			);
			errorIfTextMatches(
				snippetText,
				/\${[^0-9]/,
				"Snippet placeholder is missing the numeric id, makes it look like a variable to vscode"
			);
		}

		function validateDocumentWithSnippet(): void {
			assert(
				docTextBeforeInsertion !== docTextAfterInsertion,
				"No insertion happened?  Document didn't change."
			);
		}

		function errorIfTextMatches(
			text: string,
			regex: RegExp,
			errorMessage: string
		): void {
