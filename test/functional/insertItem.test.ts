...
suite("InsertItem", async (): Promise<void> => {
	function assertTemplate(
		...
		options?: { ignoreWhiteSpace?: boolean; ignoreApiVersions?: boolean }
	): void {
	...
	function testInsertItem(
		...
		testWithRealSnippets("Tabs CRLF", async () => {
			await testInsertItemWithSettings(
				template,
				expected,
				false,
				4,
	...
	async function testInsertItemWithSettings(
		template: string,
		expected: string,
		...
		let deploymentTemplate = new DeploymentTemplateDoc(
			document.getText(),
			document.uri,
			document.version
		);
	...
	function createInsertItemTests(
		...
		testInsertItem(
			startTemplate,
			expectedTemplate,
			async (insertItem, template, editor) =>
				await insertItem.insertItem(
					template,
					sectionType,
					editor,
					getActionContext()
    ...
    "resources": [
		...
		suite(
			"Insert one resource (KeyVault Secret) into totally empty template",
			async () => {
				createInsertItemTests(
					totallyEmptyTemplate,
					oneResourceTemplate,
		...
		suite(
			"Insert one more resource (Application Security Group)",
			async () => {
				createInsertItemTests(
					oneResourceTemplate,
					twoResourcesTemplate,
					TemplateSectionType.Resources,
    ...
    "functions": [
		...
		suite("Insert function", async () => {
			createInsertItemTests(
				...
				["ns", "function1", "String", "parameter1", "String", ""],
				"resourceGroup()"
			);
		});
		...
		suite("Insert function in namespace", async () => {
			createInsertItemTests(
				...
				oneFunctionTemplate,
				TemplateSectionType.Functions,
				["function1", "String", "parameter1", "String", ""],
				"resourceGroup()"
			);
		});
    ...
    "parameters": {
		...
		suite("Insert one parameter", async () => {
			createInsertItemTests(
				emptyTemplate,
				oneParameterTemplate,
				TemplateSectionType.Parameters,
				["parameter1", "String", "default", "description"]
			);
		});
		suite("Insert one more parameter", async () => {
			createInsertItemTests(
				oneParameterTemplate,
				twoParametersTemplate,
				TemplateSectionType.Parameters,
				["parameter2", "String", "", ""]
			);
		});
		suite("Insert even one more parameter", async () => {
			createInsertItemTests(
				twoParametersTemplate,
				threeParametersTemplate,
				TemplateSectionType.Parameters,
				["parameter3", "Secure string", "", "description3"]
			);
		});
		...
		suite(
			"Insert one int parameter in totally empty template",
			async () => {
				createInsertItemTests(
					totallyEmptyTemplate,
		...
		suite(
			"Insert one array parameter in totally empty template",
			async () => {
				createInsertItemTests(
					totallyEmptyTemplate,
					oneParameterTemplateArray,
					TemplateSectionType.Parameters,
					["parameter1", "Array", "[]", ""]
				);
			}
		...
		suite(
			"Insert one bool parameter in totally empty template",
			async () => {
				createInsertItemTests(
					totallyEmptyTemplate,
					oneParameterTemplateBool,
					TemplateSectionType.Parameters,
		...
		suite(
			"Insert one object parameter in totally empty template",
			async () => {
				createInsertItemTests(
					totallyEmptyTemplate,
					oneParameterTemplateObject,
					TemplateSectionType.Parameters,
					["parameter1", "Object", "{}", ""]
				);
			}
		...
		suite(
			"Insert one secure object parameter in totally empty template",
			async () => {
				createInsertItemTests(
					totallyEmptyTemplate,
					oneParameterTemplateSecureObject,
					TemplateSectionType.Parameters,
					["parameter1", "Secure object", "{}", ""]
				);
			}
		);
	});
    ...
    "outputs": {
		...
		suite("Insert even one more output", async () => {
			createInsertItemTests(
				twoOutputsTemplate,
				threeOutputsTemplate,
				TemplateSectionType.Outputs,
				["output3", "Secure string"],
				"resourceGroup()"
			);
		});
...
class MockUserInput implements IAzureUserInput {
	...
	public async showQuickPick<T extends vscode.QuickPickItem>(
		items: T[] | Thenable<T[]>,
		options: import("vscode-azureextensionui").IAzureQuickPickOptions
	): Promise<T> {
	...
	public async showWarningMessage<T extends vscode.MessageItem>(
		message: string,
		options: import("vscode-azureextensionui").IAzureMessageOptions,
		...items: T[]
	): Promise<T> {
		return items[0];
	}
