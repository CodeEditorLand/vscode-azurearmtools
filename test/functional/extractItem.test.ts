...
suite("ExtractItem", async (): Promise<void> => {
	...
	async function runExtractParameterTest(
		startTemplate: string | IPartialDeploymentTemplate,
		selectedText: string,
		testInputs: (string | RegExp)[],
		codeActionsCount: number = 2,
		expectedTemplate?: string | IPartialDeploymentTemplate
	): Promise<void> {
		await runExtractItemTest(
			startTemplate,
			selectedText,
			testInputs,
			codeActionsCount,
			expectedTemplate,
			async (extractItem, template, editor) =>
				await extractItem.extractParameter(
					editor,
					template,
					getActionContext()
				)
	...
	async function runExtractVariableTest(
		startTemplate: string | IPartialDeploymentTemplate,
		selectedText: string,
		testInputs: (string | RegExp)[],
		codeActionsCount: number = 2,
		...
		await runExtractItemTest(
			...
			expectedTemplate,
			async (extractItem, template, editor) =>
				await extractItem.extractVariable(
					editor,
					template,
					getActionContext()
				)
	...
	async function runExtractItemTest(
		startTemplate: string | IPartialDeploymentTemplate,
		selectedText: string,
		testInputs: (string | RegExp)[],
		codeActionsCount: number,
		expectedTemplate: string | IPartialDeploymentTemplate | undefined,
		doExtract: (
		...
		await testUserInput.runWithInputs(testInputs, async () => {
			...
			let codeActions = deploymentTemplate.getCodeActions(
				undefined,
			...
			assert.strictEqual(
				codeActions.length,
				codeActionsCount,
				`GetCodeAction should return ${codeActionsCount}`
			);
			if (codeActionsCount > 0) {
				...
				assertEx.strictEqual(
					...
					expectedTemplate,
					{},
					"extractVaraiable should perform expected result"
				);
			}
		});
	}
	...
	suite("Extract within expressions", async () => {
		async function runExtractFromExpressionTest(
			expression: string,
			selectedText: string,
		...
		suite("String literals", () => {
            ...
            */
			...
			test("copyIndex(1)", async () => {
				await runExtractFromExpressionTest(
					`[concat(parameters('vmName'),'pip',copyIndex(1))]`,
					`copyIndex(1)`,
					`[copyIndex(1)]`,
					`[concat(parameters('vmName'),'pip',variables('v1'))]`
				);
			});
			...
			test("user-defined function #1", async () => {
				await runExtractFromExpressionTest(
					...
					`ns.udf3()`,
					`[ns.udf3()]`,
					`[concat(ns.udf1('a'), ns.udf2(variables('v1')))]`
				);
			});
