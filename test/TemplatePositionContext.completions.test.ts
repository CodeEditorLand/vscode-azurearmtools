...
import {
	allTestDataCompletionNames,
	allTestDataExpectedCompletions,
	parameterCompletion,
	propertyCompletion,
	variableCompletion,
} from "./TestData";
	...
	suite("completionItems", async () => {
		function addCursor(documentText: string, markerIndex: number): string {
			return `${documentText.slice(
				0,
				markerIndex
			)}<CURSOR>${documentText.slice(markerIndex)}`;
		}

		function completionItemsTest(
			documentText: string,
			index: number,
			expectedCompletionItems: Completion.Item[]
		): void {
			const testName = `with ${strings.escapeAndQuote(
				addCursor(documentText, index)
			)} at index ${index}`;
			testWithPrep(testName, [UseNoSnippets.instance], async () => {
				let keepInClosureForEasierDebugging = testName;
				keepInClosureForEasierDebugging =
					keepInClosureForEasierDebugging;

				const dt = new DeploymentTemplateDoc(documentText, fakeId, 0);
				const pc: TemplatePositionContext =
					dt.getContextFromDocumentCharacterIndex(index, undefined);

				let completionItems: Completion.Item[] = (
					await pc.getCompletionItems(undefined, 2)
				).items;
				const completionItems2: Completion.Item[] = (
					await pc.getCompletionItems(undefined, 2)
				).items;
				...
				compareTestableCompletionItems(
					completionItems,
		...
		function compareTestableCompletionItems(
			actualItems: Completion.Item[],
			expectedItems: Completion.Item[]
		): void {
			let isFunctionCompletions = expectedItems.some((item) =>
				allTestDataCompletionNames.has(item.label)
			);
			...
			if (isFunctionCompletions) {
				...
				if (expectedItems.length > 0) {
					actualItems = actualItems.filter((item) =>
						allTestDataCompletionNames.has(item.label)
					);
				}
			}
			...
			let expectedNames = expectedItems.map((item) =>
		...
		suite("test 01", () => {
			for (let i = 0; i <= 24; ++i) {
				completionItemsTest(
					...
					i,
					i >= 9 && i <= 15
						? allTestDataExpectedCompletions(9, i - 9)
						: i === 20
						? allTestDataExpectedCompletions(20, 0)
						: []
				);
			}
		});
		...
		for (let repetition = 0; repetition < repetitions; ++repetition) {
			...
			suite("test 03", () => {
				for (let i = 0; i <= 25; ++i) {
					completionItemsTest(
						...
						i,
						i >= 19 && i <= 25
							? allTestDataExpectedCompletions(19, i - 19)
							: []
					);
				}
			});
				...
				for (let i = 0; i <= 33; ++i) {
					completionItemsTest(
						...
						i >= 19 && i <= 25
							? allTestDataExpectedCompletions(19, i - 19)
							: 26 <= i && i <= 29
							? allTestDataExpectedCompletions(i, 0)
							: []
					);
				}
			});
			...
			suite("test 06", () => {
				for (let i = 0; i <= 80; ++i) {
					completionItemsTest(
						`{ 'parameters': { 'pName': { 'type': 'integer' } }, 'a': 'A', 'b': "[concat(')]"`,
						i,
						i >= 69 && i <= 75
							? allTestDataExpectedCompletions(69, i - 69)
							: i === 80
							? allTestDataExpectedCompletions(80, 0)
							: []
					);
				}
			});
			...
			suite("test 08", () => {
				for (let i = 0; i <= 56; ++i) {
					completionItemsTest(
						`{ 'variables': { 'v1': 'value1' }, 'a': "[variables(]" }`,
						i,
						// after the "[": all
						i >= 42 && i <= 51
							? allTestDataExpectedCompletions(42, i - 42)
			...
			suite("test 10", () => {
				for (let i = 0; i <= 52; ++i) {
					completionItemsTest(
						`{ 'variables': { 'vName': 20 }, 'a': "[variables(')]`,
						i,
						i >= 39 && i <= 48
							? allTestDataExpectedCompletions(39, i - 39)
							: i === 49
							? [variableCompletion("vName", 49, 0, false)]
							: i === 50 || i === 51
							? [variableCompletion("vName", 49, 2)]
							: []
					);
				}
			});
			...
			suite("test 11", () => {
				for (let i = 0; i <= 53; ++i) {
					completionItemsTest(
						`{ 'variables': { 'vName': 20 }, 'a': "[variables('v)]`,
						i,
						i >= 39 && i <= 48
							? allTestDataExpectedCompletions(39, i - 39)
							: i === 49
			...
			suite("test 13", () => {
				for (let i = 0; i <= 140; ++i) {
					// 70: ''Microsoft...
					completionItemsTest(
						`{ "parameters": { "adminUsername": {} }, "a": "[resourceId(parameters(''Microsoft.Networks/virtualNetworks', parameters('adminUsername'))]" }`,
						i,
						i === 48 || i === 59 || (73 <= i && i <= 138)
							? allTestDataExpectedCompletions(i, 0)
							: 49 <= i && i <= 58
							...
							: i === 71
							? [parameterCompletion("adminUsername", 70, 2)] // in the string
							: []
					);
				}
			});
				...
				for (let i = 0; i <= 140; ++i) {
					...
					completionItemsTest(
						...
						48 <= i && i <= 58
							...
							: i === 107
							? allTestDataExpectedCompletions(107, 0)
							: 108 <= i && i <= 118
							? allTestDataExpectedCompletions(108, i - 108)
							: i === 119
							? [
									parameterCompletion(
										"adminUsername",
							...
							: i >= 120 && i <= 133
							? [parameterCompletion("adminUsername", 119, 16)]
							: i >= 135 && i <= 136
							? allTestDataExpectedCompletions(i, 0)
							: []
					);
			...
			suite("test 15", () => {
				for (let i = 0; i <= 137; ++i) {
					...
					// 116: 'adminUsername'
					completionItemsTest(
						`{ "variables": { "adminUsername": "" }, "a": "[resourceId(variables('Microsoft.Networks/virtualNetworks', variables('adminUsername'))]" }`,
						i,
						i >= 47 && i <= 57
							? allTestDataExpectedCompletions(47, i - 47)
							: i >= 58 && i <= 67
							? allTestDataExpectedCompletions(58, i - 58)
							: i === 68
							? [
									variableCompletion(
										"adminUsername",
							...
							: i === 105
							? allTestDataExpectedCompletions(105, 0) // space after comma
							: 106 <= i && i <= 115
							? allTestDataExpectedCompletions(106, i - 106)
							: i === 116
							? [
									variableCompletion(
										"adminUsername",
							...
							: 117 <= i && i <= 130
							? [variableCompletion("adminUsername", 116, 16)]
							: i >= 132 && i <= 133
							? allTestDataExpectedCompletions(i, 0)
							: []
					);
				...
				for (let i = 0; i <= 25; ++i) {
					completionItemsTest(
						...
						i >= 9 && i <= 19
							...
							? []
							: i === 21
							? allTestDataExpectedCompletions(21, 0)
							: []
					);
				}
			});
			...
			suite("test 17", () => {
				for (let i = 0; i <= 52; ++i) {
					completionItemsTest(
						`{ 'parameters': { 'p1': {} }, 'a': "[parameters(]" }`,
						i,
						i >= 37 && i <= 47
							? allTestDataExpectedCompletions(37, i - 37)
							: i === 48
							? [parameterCompletion("p1", 48, 0)]
							: []
					);
				}
			});
			...
			suite("test 18", () => {
				for (let i = 0; i <= 81; ++i) {
					completionItemsTest(
						`{ 'parameters': { 'pName': { 'type': 'integer' } }, 'a': 'A', 'b': "[parameters('`,
						i,
						i >= 69 && i <= 79
							? allTestDataExpectedCompletions(69, i - 69)
							: i === 80
							? [parameterCompletion("pName", 80, 0, false)]
							: []
					);
				}
			});
			...
			suite("test 19", () => {
				for (let i = 0; i <= 76; ++i) {
					completionItemsTest(
						`{ 'parameters': { 'pName': { 'type': 'integer' } }, 'a': "[parameters(')]" }`,
						i,
						i >= 59 && i <= 69
							? allTestDataExpectedCompletions(59, i - 59)
							: i === 70
							? [parameterCompletion("pName", 70, 0, false)]
							: i >= 71 && i <= 72
							? [parameterCompletion("pName", 70, 2)] // Don't replace the "]"
							: []
					);
				}
			});
				...
				for (let i = 0; i <= 75; ++i) {
					completionItemsTest(
						...
						i >= 59 && i <= 69
							...
							? [parameterCompletion("pName", 70, 0, false)]
							: i === 71
							? [parameterCompletion("pName", 70, 1)] // Don't replace the "]"
							: []
					);
				}
			});
			...
			suite("test 21", () => {
				for (let i = 0; i <= 53; ++i) {
					completionItemsTest(
						`{ 'variables': { 'vName': 20 }, 'a': "[variables('p)]`,
						i,
						i >= 39 && i <= 48
							? allTestDataExpectedCompletions(39, i - 39)
							: i === 49
							? [variableCompletion("vName", 49, 0, false)]
							: i >= 50 && i <= 52
							? [variableCompletion("vName", 49, 3)] // Don't replace the "]"
							: []
					);
				}
			});
			...
			suite("test 22", () => {
				for (let i = 0; i <= 65; ++i) {
					completionItemsTest(
						`{ 'variables': { 'vName': 20 }, 'a': 'A', 'b': "[concat  spam  ('`,
						i,
						i >= 49 && i <= 55
							? allTestDataExpectedCompletions(49, i - 49)
							: 56 <= i && i <= 63
							? allTestDataExpectedCompletions(i, 0)
							: []
					);
				...
				for (let i = 0; i <= 28; ++i) {
					completionItemsTest(
						...
						i >= 9 && i <= 22
							? allTestDataExpectedCompletions(9, i - 9)
							: i === 23
							? allTestDataExpectedCompletions(23, 0)
							: i === 24
							? allTestDataExpectedCompletions(24, 0)
							: []
					);
				}
			});
			...
			suite("Variable value deep completion for objects", () => {
				...
				suite("test vvdc 02", () => {
					for (let i = 0; i <= 55; ++i) {
						completionItemsTest(
							`{ "variables": { "a": "A" }, "b": "[variables('a').]" }`,
							i,
							36 <= i && i <= 45
								? allTestDataExpectedCompletions(36, i - 36)
								: i === 46
								? [variableCompletion("a", 46, 0, false)]
								: 47 <= i && i <= 48
								? [variableCompletion("a", 46, 4)]
								: []
						);
					}
				});
				...
				suite("test vvdc 04", () => {
					for (let i = 0; i <= 56; ++i) {
						completionItemsTest(
							`{ "variables": { "a": true }, "b": "[variables('a').]" }`,
							i,
							37 <= i && i <= 46
								? allTestDataExpectedCompletions(37, i - 37)
								: i === 47
								? [variableCompletion("a", 47, 0, false)]
								: 48 <= i && i <= 49
								? [variableCompletion("a", 47, 4)]
								: []
						);
					}
				});
				...
				suite("test vvdc 05", () => {
					for (let i = 0; i <= 56; ++i) {
						completionItemsTest(
							`{ "variables": { "a": null }, "b": "[variables('a').]" }`,
							i,
							37 <= i && i <= 46
								? allTestDataExpectedCompletions(37, i - 37)
								: i === 47
				...
				suite("test vvdc 06", () => {
					for (let i = 0; i <= 54; ++i) {
						completionItemsTest(
							`{ "variables": { "a": [] }, "b": "[variables('a').]" }`,
							i,
							35 <= i && i <= 44
								? allTestDataExpectedCompletions(35, i - 35)
								: i === 45
								? [variableCompletion("a", 45, 0, false)]
								: 46 <= i && i <= 47
								? [variableCompletion("a", 45, 4)]
								: []
						);
					}
				});
				...
				suite("test vvdc 08", () => {
					for (let i = 0; i <= 67; ++i) {
						completionItemsTest(
							`{ "variables": { "a": { "name": "A" } }, "b": "[variables('a').]" }`,
							i,
							48 <= i && i <= 57
								? allTestDataExpectedCompletions(48, i - 48)
								: i === 58
								? [variableCompletion("a", 58, 0, false)]
								: 59 <= i && i <= 60
								? [variableCompletion("a", 58, 4)]
								: 62 <= i && i <= 63
								? [propertyCompletion("name", i, 0)]
								: []
						);
				...
				suite("test vvdc 09", () => {
					for (let i = 0; i <= 69; ++i) {
						completionItemsTest(
							`{ "variables": { "a": { "name": "A" } }, "b": "[variables('a').na]" }`,
							i,
							48 <= i && i <= 57
								? allTestDataExpectedCompletions(48, i - 48)
								: i === 58
				...
				suite("test vvdc 10", () => {
					for (let i = 0; i <= 69; ++i) {
						completionItemsTest(
							`{ "variables": { "a": { "name": "A" } }, "b": "[variables('a').ab]" }`,
							i,
							48 <= i && i <= 57
								? allTestDataExpectedCompletions(48, i - 48)
								: i === 58
				...
				suite("test vvdc 11", () => {
					for (let i = 0; i <= 78; ++i) {
						completionItemsTest(
							`{ "variables": { "a": { "bb": { "cc": 200 } } }, "b": "[variables('a').bb.]" }`,
							i,
							56 <= i && i <= 65
								? allTestDataExpectedCompletions(56, i - 56)
								: i === 66
								? [variableCompletion("a", 66, 0, false)]
								: 67 <= i && i <= 68
								? [variableCompletion("a", 66, 4)]
								: 70 <= i && i <= 73
								? [propertyCompletion("bb", 71, 2)]
								: i === 74
								? [propertyCompletion("cc", 74, 0)]
								: []
						);
					}
				});
				...
				suite("test vvdc 13", () => {
					...
					for (let i = 0; i <= 78; ++i) {
						completionItemsTest(
							`{ "variables": { "A": { "Bb": { "cC": 200 } } }, "b": "[variables('a').BB.Cc]" }`,
							i,
							56 <= i && i <= 65
								? allTestDataExpectedCompletions(56, i - 56)
								: i === 66
			...
			function getDocumentAndMarkers(document: object | string): {
				documentText: string;
				tokens: number[];
			} {
				let tokens: number[] = [];
				...
				while (true) {
					...
					document =
						document.slice(0, tokenPos) +
						document.slice(tokenPos + 1);
				}
			...
			suite("Parameter defaultValue deep completion for objects", () => {
				...
				completionItemsTest(documentText, dotAa, [
					propertyCompletion("aa", dotAa, 2),
				]);
				completionItemsTest(documentText, dotBb, [
					propertyCompletion("bb", dotBb, 2),
				]);
				completionItemsTest(documentText, dot, [
		...
		suite("Completion items usability", () => {
			...
			function testCompletionItemsWithRange(
				...
				expectedExample:
					| undefined
					| {
							// Expected representation of one of the completion items, we'll search for it by label (the others are not tested, assumed to be similar)
							label: string;
							insertText: string;
				...
				testWithPrep(testName, [UseNoSnippets.instance], async () => {
					...
					templateWithReplacement = stringify(
						templateWithReplacement
					);
					const template = templateWithReplacement.replace(
						/TESTEXPRESSION/,
						expression
					);
					const {
						dt,
						markers: { cursor, replstart },
					} = parseTemplateWithMarkers(template);
					// tslint:disable-next-line: strict-boolean-expressions
					assert(
						!!replstart!,
						"Didn't find <!replstart!> in test expression"
					);
					// tslint:disable-next-line: strict-boolean-expressions
					assert(
						!!cursor!,
						"Didn't find <!cursor!> in test expression"
					);
					const pc: TemplatePositionContext =
					...
					let foundItem = completionItems.find(
						(ci) => ci.label === expectedExample.label
					);
					assert(
						!!foundItem,
						`Did not find a completion item with the label "${expectedExample.label}"`
					);
			...
			suite("Parameters/variables argument replacements", () => {
				...
				suite(
					"cursor before string - insert completion, don't remove anything",
					() => {
						testCompletionItemsWithRange(
							"",
							template1,
				...
				suite(
					"cursor inside string - replace entire string and closing paren",
					() => {
						testCompletionItemsWithRange(
							"empty string",
							template1,
						...
						suite(
							"Make sure we don't erase the closing ']' if the closing paren or quote are missing (perhaps Auto Closing Brackets settings if off)",
							() => {
								testCompletionItemsWithRange(
									"no closing paren",
									template1,
				...
				suite(
					"#349 tab completion of params/vars shouldn't wipe out remainder of existing string when adding new parameters call before existing string",
					() => {
