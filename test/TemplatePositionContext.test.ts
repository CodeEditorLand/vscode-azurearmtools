...
import {
	...
	FormattedExpressionHoverInfo,
	FunctionSignatureHelp,
	IHoverInfo,
...
suite("TemplatePositionContext", () => {
	suite("allow out of bounds", () => {
		...
		test("documentColumnIndex cannot be greater than the line's maximum index", async () => {
			...
			const pc = dt.getContextFromDocumentLineAndColumnIndexes(
				line,
				col,
				undefined,
				true
	...
	suite(
		"fromDocumentLineAndColumnIndexes(DeploymentTemplate,number,number)",
		() => {
			test("with undefined deploymentTemplate", () => {
				// tslint:disable-next-line:no-any
				assert.throws(() => {
					TemplatePositionContext.fromDocumentLineAndColumnIndexes(
						<any>undefined,
						1,
						2,
						undefined
	...
	suite("fromDocumentCharacterIndex(DeploymentTemplate,number)", () => {
		test("with undefined deploymentTemplate", () => {
			// tslint:disable-next-line:no-any
			assert.throws(() => {
				TemplatePositionContext.fromDocumentCharacterIndex(
					<any>undefined,
					1,
					undefined
				);
		...
		test("with documentCharacterIndex greater than the maximum character index", () => {
			let dt = new DeploymentTemplateDoc("{}", fakeId, 0);
			let pc = TemplatePositionContext.fromDocumentCharacterIndex(
				dt,
				3,
				undefined
			);
	...
	suite("documentPosition", () => {
		test("with PositionContext from line and column indexes", () => {
			...
			let pc = TemplatePositionContext.fromDocumentLineAndColumnIndexes(
				dt,
				1,
				0,
				undefined
	...
	suite("getTokenAtOrAfterCursor", () => {
		function getTextAtReplacementSpan(
			dt: DeploymentTemplateDoc,
			index: number
		): string | undefined {
			const span = dt
				.getContextFromDocumentCharacterIndex(index, undefined)
		...
		test("hyphens and exclamation points", async () => {
			...
			for (let i = 2; i <= 19; ++i) {
				// a-o
				assert.equal(
					getTextAtReplacementSpan(dt, i),
					"arm-keyvault!hello"
				);
			}

			assert.equal(
				getTextAtReplacementSpan(dt, 20),
				"arm-keyvault!hello"
			); // space after hello
	...
	suite("jsonToken", () => {
		test("with characterIndex in whitespace", () => {
			let dt = new DeploymentTemplateDoc(
				"{ 'a': 'A', 'b': \"[concat('B')]\" }",
				fakeId,
				0
			);
		...
		test("with characterIndex at the start of a QuotedString", () => {
			...
			assert.deepStrictEqual(
				pc.jsonToken,
				jsonTest.parseQuotedString(`'a'`, 2)
			);
		});
	...
	suite("tleParseResult", () => {
		test("with characterIndex in whitespace", () => {
			const dt: DeploymentTemplateDoc = new DeploymentTemplateDoc(
				"{ 'a': 'A', 'b': \"[concat('B')]\" }",
				fakeId,
				0
			);
			const pc: TemplatePositionContext =
				dt.getContextFromDocumentCharacterIndex(1, undefined);
		...
		test("with characterIndex at the start of a LeftCurlyBracket", () => {
			...
			const pc: TemplatePositionContext =
				dt.getContextFromDocumentCharacterIndex(0, undefined);
			assert.deepStrictEqual(pc.tleInfo, undefined);
		});
		...
		test("with characterIndex at the start of a closed TLE QuotedString", () => {
			...
			const pc: TemplatePositionContext =
				dt.getContextFromDocumentCharacterIndex(17, undefined);

			const tleParseResult: TLE.TleParseResult =
				pc.tleInfo!.tleParseResult;
			assert.deepStrictEqual(tleParseResult.errors, []);
			assert.deepStrictEqual(
				tleParseResult.leftSquareBracketToken,
				TLE.Token.createLeftSquareBracket(1)
			);
			assert.deepStrictEqual(
				tleParseResult.rightSquareBracketToken,
				TLE.Token.createRightSquareBracket(13)
			);

			const concat: TLE.FunctionCallValue = nonNullValue(
				TLE.asFunctionCallValue(tleParseResult.expression)
			);
	...
	suite("tleCharacterIndex", () => {
		...
		test("with characterIndex inside of a TLE", () => {
			...
			const pc: TemplatePositionContext =
				dt.getContextFromDocumentCharacterIndex(21, undefined);
		...
		test("with characterIndex after the end of an unclosed TLE", () => {
			...
			const pc: TemplatePositionContext =
				dt.getContextFromDocumentCharacterIndex(29, undefined);
	...
	suite("tleValue", () => {
		...
		test("with characterIndex at the start of a non-TLE QuotedString", () => {
			...
			const pc: TemplatePositionContext =
				dt.getContextFromDocumentCharacterIndex(2, undefined);
			assert.deepStrictEqual(
				pc.tleInfo!.tleValue,
		...
		test("with characterIndex inside of a TLE", () => {
			...
			const concat: TLE.FunctionCallValue = nonNullValue(
				TLE.asFunctionCallValue(pc.tleInfo!.tleValue)
			);
			assert.deepStrictEqual(
				concat.nameToken,
				TLE.Token.createLiteral(2, "concat")
		...
		test("with characterIndex after the end of an unclosed TLE", () => {
			...
			const pc: TemplatePositionContext =
				dt.getContextFromDocumentCharacterIndex(
					"{ 'a': 'A', 'b': \"[concat('B'".length,
					undefined
				);

			const b: TLE.StringValue = nonNullValue(
				TLE.asStringValue(pc.tleInfo!.tleValue)
			);
			assert.deepStrictEqual(
				b.token,
				TLE.Token.createQuotedString(9, "'B'")
			...
			const concat: TLE.FunctionCallValue = nonNullValue(
				TLE.asFunctionCallValue(b.parent)
			);
			assert.deepStrictEqual(
				concat.nameToken,
				TLE.Token.createLiteral(2, "concat")
	...
	suite("hoverInfo", () => {
		test("in non-string json token", () => {
			...
			const hoverInfo: IHoverInfo[] = dt
				.getContextFromDocumentCharacterIndex(0, undefined)
				.getHoverInfo();
		...
		test("in property name json token", () => {
			...
			const hoverInfo: IHoverInfo[] = dt
				.getContextFromDocumentCharacterIndex(3, undefined)
				.getHoverInfo();
			assert.deepStrictEqual(hoverInfo, []);
		});
	});
	...
	test("in unrecognized function name", () => {
		...
		const hoverInfo: IHoverInfo[] = dt
			.getContextFromDocumentCharacterIndex(
				"{ 'a': 'A', 'b': \"[to".length,
				undefined
			)
			.getHoverInfo()
			.filter((hi) => !(hi instanceof FormattedExpressionHoverInfo));
		assert.deepStrictEqual(hoverInfo, []);
	});
	...
	test("in recognized function name", () => {
		...
		const infos: IHoverInfo[] = pc
			.getHoverInfo()!
			.filter((hi) => !(hi instanceof FormattedExpressionHoverInfo));
		assert(infos.length === 1);
		assert.deepStrictEqual(
			(<UsageInfoHoverInfo>infos[0]).usage,
			"concat(arg1, arg2, arg3, ...)"
		);
		assert.deepStrictEqual(
			infos[0].span,
			new Span("{ 'a': 'A', 'b': \"[".length, 6)
		);
	});
	...
	test("in recognized parameter reference name", () => {
		const dt = new DeploymentTemplateDoc(
			"{ 'parameters': { 'pName': { 'type': 'integer' } }, 'a': 'A', 'b': \"[parameters('pName')\" }",
			fakeId,
			0
		);
		const pc: TemplatePositionContext =
			dt.getContextFromDocumentCharacterIndex(
				"{ 'parameters': { 'pName': { 'type': 'integer' } }, 'a': 'A', 'b': \"[parameters('pN"
					.length,
				undefined
			);
		const infos: IHoverInfo[] = pc
		...
		assert(infos.length === 1);
		assert.deepStrictEqual(
			`**pName**${os.EOL}*(parameter)*`,
			infos[0].getHoverText().value
		);
		assert.deepStrictEqual(
			new Span(
				"{ 'parameters': { 'pName': { 'type': 'integer' } }, 'a': 'A', 'b': \"[parameters('".length,
				"pName".length
			),
			infos[0].span
		);
	...
	test("in recognized variable reference name", () => {
		const dt = new DeploymentTemplateDoc(
			"{ 'variables': { 'vName': 3 }, 'a': 'A', 'b': \"[variables('vName')\" }",
			fakeId,
			0
		);
		const pc: TemplatePositionContext =
			dt.getContextFromDocumentCharacterIndex(
				"{ 'variables': { 'vName': 3 }, 'a': 'A', 'b': \"[variables('vNam"
					.length,
				undefined
			);
		const infos: IHoverInfo[] = pc
		...
		assert(infos.length === 1);
		assert.deepStrictEqual(
			`**vName**${os.EOL}*(variable)*`,
			infos[0].getHoverText().value
		);
		assert.deepStrictEqual(
			new Span(
				"{ 'variables': { 'vName': 3 }, 'a': 'A', 'b': \"[variables('".length,
				"vName".length
			),
			infos[0].span
		);
	...
	suite("signatureHelp", () => {
		test("not in a TLE", () => {
			const dt = new DeploymentTemplateDoc(`{ "a": "AA" }`, fakeId, 0);
			const pc: TemplatePositionContext =
				dt.getContextFromDocumentCharacterIndex(
					`{ "a": "A`.length,
					undefined
				);
			const functionSignatureHelp: TLE.FunctionSignatureHelp | undefined =
				pc.getSignatureHelp();
		...
		test("in TLE function name", () => {
			const dt = new DeploymentTemplateDoc(`{ "a": "[con]" }`, fakeId, 0);
			const pc: TemplatePositionContext =
				dt.getContextFromDocumentCharacterIndex(
					`{ "a": "[con`.length,
					undefined
		...
		test("after left parenthesis", () => {
			const dt = new DeploymentTemplateDoc(`{ "a": "[concat(`, fakeId, 0);
			const pc: TemplatePositionContext =
				dt.getContextFromDocumentCharacterIndex(
					`{ "a": "[concat(`.length,
					undefined
				);
			const functionSignatureHelp: TLE.FunctionSignatureHelp =
				pc.getSignatureHelp()!;
			...
			assert.deepStrictEqual(
				functionSignatureHelp.functionMetadata.fullName,
				"concat"
			);
		});
		...
		test("in variadic parameter when function signature has 'name...' parameter", () => {
			const dt = new DeploymentTemplateDoc(
				`{ "a": "[resourceId('a', 'b', 'c', 'd', 'e', 'f', 'g'`,
				fakeId,
				0
			);
			const pc: TemplatePositionContext =
				dt.getContextFromDocumentCharacterIndex(
					`{ "a": "[concat('a', 'b', 'c', 'd', 'e', 'f', 'g'`.length,
					undefined
				);
			const functionSignatureHelp: TLE.FunctionSignatureHelp =
				pc.getSignatureHelp()!;
		...
		suite("signatureHelp for UDFs", () => {
			...
			async function testUdfSignatureHelp(
				expressionWithCursorMarker: string,
				expected: TLE.FunctionSignatureHelp | undefined
			): Promise<void> {
				const templateString = stringify(udfTemplate).replace(
					"<o1value>",
				...
				const {
					dt,
					markers: { cursor },
				} = parseTemplateWithMarkers(templateString);
				assert(
					cursor,
					"You must place a '<!cursor!>' cursor in the expression string to indicate position"
				);
				const pc: TemplatePositionContext =
					dt.getContextFromDocumentCharacterIndex(
						cursor.index,
			...
			test("inside second parameter", async () => {
				await testUdfSignatureHelp(
					"udf.concat('hello', 'th<!cursor!>ere')",
			...
			test("param with no type", async () => {
				await testUdfSignatureHelp(
					"udf.mysterious(12<!cursor!>345, 2)",
					new FunctionSignatureHelp(
						0,
						new UserFunctionMetadata(
							"udf.mysterious",
	...
	suite("parameterDefinition", () => {
		...
		): Promise<IParameterDefinition | undefined> {
			const refInfo: IReferenceSite | undefined =
				pc.getReferenceSiteInfo(false);
		...
		test("with no parameters property", async () => {
			...
			const context: TemplatePositionContext =
				dt.getContextFromDocumentCharacterIndex(
					"{ 'a': '[parameters(\"pN".length,
					undefined
				);
			assert.deepStrictEqual(
				await getParameterDefinitionIfAtReference(context),
				undefined
			);
		});
		...
		test("with empty parameters property value", async () => {
			const dt = new DeploymentTemplateDoc(
				"{ 'parameters': {}, 'a': '[parameters(\"pName\")]' }",
				fakeId,
				0
			);
			const context: TemplatePositionContext =
				dt.getContextFromDocumentCharacterIndex(
					"{ 'parameters': {}, 'a': '[parameters(\"pN".length,
					undefined
				);
			assert.deepStrictEqual(
				await getParameterDefinitionIfAtReference(context),
		...
		test("with matching parameter definition", async () => {
			const dt = new DeploymentTemplateDoc(
				"{ 'parameters': { 'pName': {} }, 'a': '[parameters(\"pName\")]' }",
				fakeId,
				0
			);
			const context: TemplatePositionContext =
				dt.getContextFromDocumentCharacterIndex(
					"{ 'parameters': { 'pName': {} }, 'a': '[parameters(\"pNa"
						.length,
					undefined
				);
			const parameterDefinition: IParameterDefinition = nonNullValue(
				await getParameterDefinitionIfAtReference(context)
			);
			assert.deepStrictEqual(
				parameterDefinition.nameValue.toString(),
				"pName"
			);
			assert.deepStrictEqual(parameterDefinition.description, undefined);
			assert.deepStrictEqual(
				parameterDefinition.fullSpan,
				new Span(18, 11)
			);
		});
		...
		test("with cursor inside 'parameters'", async () => {
			...
			const context: TemplatePositionContext =
				dt.getContextFromDocumentCharacterIndex(
					"{ 'parameters': { 'pName': {} }, 'a': '[paramet".length,
					undefined
				);
			const parameterDefinition: IParameterDefinition = nonNullValue(
				await getParameterDefinitionIfAtReference(context)
	...
	suite("variableDefinition", () => {
		function getVariableDefinitionIfAtReference(
			pc: TemplatePositionContext
		...
		test("with no variables property", () => {
			...
			const context: TemplatePositionContext =
				dt.getContextFromDocumentCharacterIndex(
					"{ 'a': '[variables(\"vN".length,
					undefined
				);
			assert.deepStrictEqual(
				getVariableDefinitionIfAtReference(context),
				undefined
			);
		});
		...
		test("with empty variables property value", () => {
			const dt = new DeploymentTemplateDoc(
				"{ 'variables': {}, 'a': '[variables(\"vName\")]' }",
				fakeId,
				0
			);
			const context: TemplatePositionContext =
				dt.getContextFromDocumentCharacterIndex(
					"{ 'variables': {}, 'a': '[variables(\"vN".length,
					undefined
				);
			assert.deepStrictEqual(
				getVariableDefinitionIfAtReference(context),
		...
		test("with matching variable definition", () => {
			const dt = new DeploymentTemplateDoc(
				"{ 'variables': { 'vName': {} }, 'a': '[variables(\"vName\")]' }",
				fakeId,
				0
			);
			const context: TemplatePositionContext =
				dt.getContextFromDocumentCharacterIndex(
					"{ 'variables': { 'vName': {} }, 'a': '[variables(\"vNa"
						.length,
					undefined
				);
			const vDef: IVariableDefinition = nonNullValue(
				getVariableDefinitionIfAtReference(context)
			);
		...
		test("with cursor before variable name start quote with matching variable definition", () => {
			...
			const context: TemplatePositionContext =
				dt.getContextFromDocumentCharacterIndex(
					"{ 'variables': { 'vName': {} }, 'a': '[variables(".length,
					undefined
				);
			const vDef: IVariableDefinition = nonNullValue(
				getVariableDefinitionIfAtReference(context)
		...
		suite("Templates", () => {
			test("Parameter reference", async () => {
				const {
					dt,
					markers: { param1ref },
				} = parseTemplateWithMarkers(template1);
				const pc = TemplatePositionContext.fromDocumentCharacterIndex(
					dt,
					param1ref.index,
					undefined
				);
				...
				assert.equal(
					site?.definition.nameValue?.quotedValue,
					'"parameter1"'
				);
				...
				assert.equal(
					site?.unquotedReferenceSpan.getText(dt.documentText),
					"parameter1"
				);
			});
			...
			test("Parameter definition", async () => {
				...
				assert.equal(
					JSON.stringify(site?.unquotedReferenceSpan),
					JSON.stringify(site?.definition.nameValue?.unquotedSpan)
				);
				assert.equal(
					site?.unquotedReferenceSpan.getText(dt.documentText),
					"parameter1"
		...
		suite("Parameter files", () => {
			test("Deployment parameter file parameter definition", () => {
				...
				const {
					dp,
					markers: { param1def },
				} = parseParametersWithMarkers(params1);
				const ppc =
					ParametersPositionContext.fromDocumentCharacterIndex(
						dp,
						param1def.index,
						dt
					);
				...
				assert.equal(site?.referenceKind, ReferenceSiteKind.reference);
				assert.equal(
					site?.unquotedReferenceSpan.getText(
						site.referenceDocument.documentText
					),
					"parameter1"
				);
			});
