	...
	suite("doesJsonContainArmSchema(string)", () => {
		suite("Just uri", () => {
			...
			test("subscription deployment template: 'http://schema.management.azure.com/schemas/xxxx-yy-zz/subscriptionDeploymentTemplate.json#'", () => {
				assert(
					isArmSchema(
						"http://schema.management.azure.com/schemas/xxxx-yy-zz/subscriptionDeploymentTemplate.json#"
					)
				);
			});
			...
			suite("new root schemas", () => {
				...
				test("xxxxx", () => {
					assert(
						isArmSchema(
							"https://schema.management.azure.com/schemas/xxxxx/deploymentTemplate.json"
						)
					);
				});
				...
				test("managementGroupDeploymentTemplate", () => {
					assert(
						isArmSchema(
							"https://schema.management.azure.com/schemas/2018-05-01/managementGroupDeploymentTemplate.json#"
						)
					);
				});
				...
				test("tenantDeploymentTemplate", () => {
					assert(
						isArmSchema(
							"https://schema.management.azure.com/schemas/2018-05-01/tenantDeploymentTemplate.json#"
						)
					);
				});
			});
			...
			suite("full template", () => {
				test("simple", () => {
                  ...
                  `;
					...
					let dt = new DeploymentTemplateDoc(template, fakeId, 0);
