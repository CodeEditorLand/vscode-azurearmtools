...
suite("areDecoupledChildAndParent", () => {
	suite("areDecoupledChildAndParent", () => {
		function createChildParentTestFromArray(
			...
			test(`${
				name ? name + ":" : ""
			}child=${childNameExpression}, parent=${parentNameExpression}`, async () => {
				...
				writeToLog(
					`Resource Infos found:\n` +
						infos
							.map(
								(i) =>
									`${i.getFullNameExpression()} (${i.getFullTypeExpression()})`
							)
							.join("\n")
				);

				const child = infos.find(
					(i) => i.getFullNameExpression() === childNameExpression
				);
				assert(
					!!child,
					`Could not find resource with full name "${childNameExpression}"`
				);
				const parent = infos.find(
					(i) => i.getFullNameExpression() === parentNameExpression
				);
				assert(
					!!parent,
					`Could not find resource with full name "${parentNameExpression}"`
				);
		...
		function createChildParentTest(
			...
			test(`${name ? name + ": " : ""}child=${child.name}, parent=${
				parent.name
			}`, async () => {
		...
		function testAreDecoupledChildAndParent(
			...
			assert.strictEqual(
				result,
				expected,
				`areChildAndParent: expected ${expected} but received ${result}`
			);
			if (typeof expectedReverse === "boolean") {
				assert.strictEqual(
					reverseResult,
					expectedReverse,
					`reversed areChildAndParent: expected ${expectedReverse} but received ${reverseResult}`
				);
			}
		}
