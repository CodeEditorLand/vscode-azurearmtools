...
suite("mapJsonObjectValue", () => {
	...
	test("single replacement", () => {
		...
		const repl = mapJsonObjectValue(o, (p) =>
			p.nameValue.unquotedValue === "b" ? newProperty("c", "c") : p
		);
	...
	test("replace with two properties", () => {
		...
		const repl = mapJsonObjectValue(o, (p) =>
			p.nameValue.unquotedValue === "b"
				? [newProperty("b1", "b1"), newProperty("b2", "b2")]
				: p
		);
	...
	test("no deep replacement", () => {
		...
		const repl = mapJsonObjectValue(o, (p) =>
			p.nameValue.unquotedValue === "b1"
				? [newProperty("b1a", "b1a"), newProperty("b1b", "b1b")]
				: p
		);
		assert.deepStrictEqual(repl.propertyNames, ["a", "b", "c"]);
		assert.deepStrictEqual(
			Json.asObjectValue(repl.getPropertyValue("b"))!.propertyNames,
			["b1", "b2"]
		);
	});
});
