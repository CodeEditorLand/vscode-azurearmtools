...
import {
	AzureRMAssets,
	looksLikeResourceTypeStringLiteral,
	splitResourceNameIntoSegments,
...
} from "./resourceId.completions.templates";
...
suite("ResourceId completions", () => {
	...
	function createResourceIdCompletionsTest2(
		...
		createExpressionCompletionsTest(
			...
			template,
			{
				...
				ignoreCompletionNames: ignoreTleFunctionCompletions
					? defaultCompletions
					: undefined, // Ignore default completions for built-in functions?
			}
		);
	}

	const defaultCompletions =
		AzureRMAssets.getFunctionsMetadata().functionMetadata.map(
			(c) => c.unqualifiedName
		);
	...
	suite("Multiple resource completions - deduped names", () => {
		...
		createResourceIdCompletionsTest(
			template,
			`resourceId('type',<!cursor!>)`,
			[`'name'`]
		);
	});

	suite(
		"Multiple resource completions - deduped types, case insensitive",
		() => {
			const template: Partial<IPartialDeploymentTemplate> = {
				resources: [
					{
	...
	suite(
		"Multiple resource completions - deduped names, case insensitive",
		() => {
			const template: Partial<IPartialDeploymentTemplate> = {
				resources: [
					{
	...
	suite(
		"Multiple resource completions - names completions, with types matched case-insensitively",
		() => {
			const template: Partial<IPartialDeploymentTemplate> = {
				resources: [
					{
	...
	suite(
		"Only show resource names that match the type in the first arg",
		() => {
			const template: Partial<IPartialDeploymentTemplate> = {
				resources: [
					{
	...
	suite("Type is an expression", () => {
		...
		createResourceIdCompletionsTest(
			template,
			`resourceId(variables('type'),<!cursor!>)`,
			[`'name1'`]
		);
	});
	...
	suite("subscriptionResourceId", () => {
		...
		createResourceIdCompletionsTest(
			template,
			`SUBSCRIPTIONRESOURCEID(<!cursor!>)`,
			[`'type1'`, `'type2'`]
		);
		createResourceIdCompletionsTest(
			template,
			`subscriptionResourceId('type1',<!cursor!>)`,
			[`'name1'`]
		);
		createResourceIdCompletionsTest(
			template,
			`subscriptionResourceId('type2',<!cursor!>)`,
			[`'name2'`]
		);
	});
	...
	suite("other names, no dice", () => {
		...
		createResourceIdCompletionsTest(
			template,
			`extensionResourceId2(<!cursor!>)`,
			[]
		);
		createResourceIdCompletionsTest(
			template,
			`udf1.resourceId('type1',<!cursor!>)`,
			[]
		);
	...
	suite("101_acsengine_swarmmode", () => {
        ...
        */
		...
		createResourceIdCompletionsTest(
			...
			[`variables('agentLbName')`, `variables('masterLbName')`]
		);
		createResourceIdCompletionsTest(
			template_101_acsengine_swarmmode,
		...
		suite(
			"subnets special case - subnet children are located in properties, not resources",
			() => {
				suite("simple", () => {
	...
	suite("child resources", () => {
		...
		function createTestsForChildResources(
			template: IPartialDeploymentTemplate
		): void {
			...
			suite("Referencing the parent", () => {
				...
				createResourceIdCompletionsTest(
					template,
					`resourceId('Microsoft.Network/networkSecurityGroups', <!cursor!>)`,
					[`'networkSecurityGroup1'`, `'networkSecurityGroup2'`]
				);
				createResourceIdCompletionsTest(
					template,
					`resourceId('Microsoft.Network/networkSecurityGroups', 'networkSecurityGroup1', <!cursor!>)`,
					[]
				);
			});
			suite("Referencing the nested children", () => {
				...
				createResourceIdCompletionsTest2(
					...
					template,
					`resourceId('Microsoft.Network/networkSecurityGroups/securityRULES', <!cursor!>)`,
					[`'networkSecurityGroup1'`, `'networkSecurityGroup2'`]
				);
				createResourceIdCompletionsTest2(
					"3rd arg (2nd part of name)",
					template,
					`resourceId('Microsoft.Network/networkSecurityGroups/securityRules', 'NETWORKSecurityGroup1', <!cursor!>)`,
					[
						`'networkSecurityGroupRule1a'`,
						`'networkSecurityGroupRule1b'`,
					]
				);
				createResourceIdCompletionsTest2(
					"4th arg (not valid)",
					template,
					`resourceId('Microsoft.Network/networkSecurityGroups/securityRules', 'NETWORKSecurityGroup1', 'networkSecurityGroupRule2', <!cursor!>)`,
					[]
				);
				...
				createResourceIdCompletionsTest2(
					"3rd arg #2",
					template,
					`resourceId('Microsoft.Network/networkSecurityGroups/securityRules', 'NETWORKSecurityGroup2', <!cursor!>)`,
					[`'networkSecurityGroupRule2'`]
				);

				suite(
					"not matched or invalid but doesn't look like a type - returns match of all resource types",
					() => {
						createResourceIdCompletionsTest2(
							"type doesn't match",
							template,
							`resourceId(variables('a'), <!cursor!>)`,
							allResourceTypeCompletions
						);
					}
				);
		...
		suite("decoupled child resources", () => {
			...
			suite(
				"Name is an expression with concat - resourceId completion should split the expression",
				() => {
					...
					createResourceIdCompletionsTest2(
						"2nd arg",
						template,
						`resourceId('Microsoft.Sql/servers/firewallRules',<!cursor!>)`,
						[`variables('sqlServer')`]
					);
					createResourceIdCompletionsTest2(
						"3rd arg",
						template,
						`resourceId('Microsoft.Sql/servers/firewallRules',variables('sqlServer'), <!cursor!>)`,
						[`variables('firewallRuleName')`]
					);
					createResourceIdCompletionsTest2(
						"4th arg",
						template,
						`resourceId('Microsoft.Sql/servers/firewallRules',variables('sqlServer'), variables('firewallRuleName'),<!cursor!>)`,
						[]
					);

					suite(
						"Whitespace ignored when looking at previous args",
						() => {
							createResourceIdCompletionsTest2(
								"ignore whitespace, 3rd arg",
								template,
								`resourceId('Microsoft.Sql/servers/firewallRules',variables( 'sqlServer' ) , <!cursor!>)`,
								[`variables('firewallRuleName')`]
							);
					...
					suite("splitResourceNameIntoSegments", () => {
						function createSplitResourceNameTest(
							testName: string,
							resourceName: string,
							expected?: string[]
						): void {
							if (!expected) {
								...
								testName +=
									" - do not know how to split, expecting original expression minus brackets";
							}
							test(testName, async () => {
								const actual =
									splitResourceNameIntoSegments(resourceName);
						...
						createSplitResourceNameTest(
							...
							[
								`concat(parameters('chefprefix'), parameters('vmNameDSAgent'), copyIndex())`,
								`variables('vmExtensionName')`,
							]
						);
						createSplitResourceNameTest(
							"realistic #6",
						...
						createSplitResourceNameTest(
							...
							[
								...
								`variables('firewallrules').batch.rules[copyIndex()].Name`,
							]
						);
					});
				}
		...
		suite("optional arguments", () => {
			suite(
				"If first arg is not a recognized type name, complete with all resource types, up to the 3rd arg",
				() => {
					const template: Partial<IPartialDeploymentTemplate> = {
						resources: [
							{
					...
					createResourceIdCompletionsTest2(
						"3rd arg",
						template,
						`resourceId(1, resourceGroup().id, <!cursor!>)`,
						[`'type1'`, `'type2'`]
					);
					createResourceIdCompletionsTest2(
						"4th arg - no type more completions",
						template,
						`resourceId(1, resourceGroup().id, 'nothing', <!cursor!>)`,
						[]
					);
				}
			);
			...
			suite(
				"resourceId using optional subscriptionId as first arg",
				() => {
					createResourceIdCompletionsTest2(
			...
			suite(
				"stop adding resource type completions once an argument looks like a type",
				() => {
					createResourceIdCompletionsTest2(
		...
		suite(
			"template_201_time_series_insights_environment_with_eventhub - grandchild resources",
			() => {
