...
suite("User functions", () => {
	const userFuncsTemplate1: IDeploymentTemplate = {
				...
				"members": {
					"<!udfStringDefinition!>string": {
						...
						"output": {
							"type": "string",
							"value":
								"[concat(<!stringRef1!>string(parameters('<!udfyearReference!>year')), '-', <!stringRef2!>string(<!addReferenceInUdfString!>add(parameters('month'), 1)), '-', <!stringRef3!>string(parameters('day')))]",
						},
					},
				},
			},
		...
		"resources": [
			{
				...
				"apiVersion":
					"[parameters('<!apiVersionReference!>apiVersion')]",
				"location": "westus",
			},
		],
		"parameters": {
		...
		"outputs": {
			...
			"output2": {
				"type": "string",
				"value":
					"[<!udfReferenceAtNamespace!>udf.<!udfReferenceAtName!>string(2019, 10, 5)]", // Using user function udf.string
			},
			"output3": {
				"type": "string",
	...
	suite("UDF Malformed", () => {
						...
						"members": {
							"odd": {
								...
								"output": {
									"type": "bool",
									"value":
										"[equals(mod(parameters('number'), 2), 1)]",
								},
							},
						},
					},
		...
		test("missing function name", () => {
			const template = `"$schema": "https://schema.management.azure.com/schemas/2015-01-01/deploymentTemplate.json#",
            "contentVersion": "1.0.0.0",
            "functions": [
                {
                    "namespace": "udf",
	...
	suite("UDF Function definitions", () => {
		...
		test("Case insensitive keys in definition", () => {
			...
			assert.equal(
				dt.topLevelScope.namespaceDefinitions[0].members.length,
				2
			);
			assert.equal(
				dt.topLevelScope.namespaceDefinitions[0].nameValue.toShortFriendlyString(),
				"udf"
			);
			assert.equal(
				dt.topLevelScope.namespaceDefinitions[0]
					.getMemberDefinition("odd")!
					.nameValue.toString(),
				"odd"
			);
			assert.equal(
			...
			assert.equal(
				dt.topLevelScope.namespaceDefinitions[0].members[0]
	...
	suite("UDF Calling user functions", () => {
		...
		test("Calling function with no parameters, with extra arg", () => {
			const template = {
				...
				"variables": {
					tooManyArgs:
						"[udf.nothing('this arg doesn''t belong here')]",
				},
				"functions": [
					{
						"namespace": "udf",
		...
		test("Missing function argument list 1", () => {
			...
			parseTemplate(template, ["Error: Missing function argument list."]);
		});
						...
						"members": {
							"oddSum": {
								...
								"output": {
									"type": "bool",
									"value":
										"[equals(mod(parameters('number'), 2), 0)]",
								},
							},
						},
					},
						...
						"members": {
							"oddSum": {
								...
								"output": {
									"type": "bool",
									"value":
										"[equals(mod(add(parameters('number'),parameters('sum')), 2), 0)]",
								},
							},
						},
					},
	...
	suite("UDF Find References", () => {
		suite("Find parameter references", () => {
			test("At reference to top-level parameter", () => {
				const {
					dt,
					markers: { apiVersionDef, apiVersionReference },
				} = parseTemplateWithMarkers(userFuncsTemplate1, [], {
					ignoreWarnings: true,
				});

				// Cursor at reference to "apiVersion" inside resources
				testGetReferences(dt, apiVersionReference.index, [
					apiVersionReference.index,
					apiVersionDef.index,
				]);
			});
			...
			test("At definition of top-level parameter", () => {
				...
				testGetReferences(dt, apiVersionDef.index, [
					apiVersionDef.index,
					apiVersionReference.index,
				]);
			});
			...
			test("At reference to user function parameter", () => {
				const {
					dt,
					markers: { udfyearReference, udfyearDefinition },
				} = parseTemplateWithMarkers(userFuncsTemplate1, [], {
					ignoreWarnings: true,
				});

				// Cursor at reference to "year" inside user function output
				testGetReferences(dt, udfyearReference.index, [
					udfyearReference.index,
					udfyearDefinition.index,
				]);
			});
			...
			test("At definition of user function parameter", () => {
				...
				testGetReferences(dt, udfyearDefinition.index, [
					udfyearReference.index,
					udfyearDefinition.index,
				]);
			});
		...
		suite("UDF Find variable references", () => {
			test("At reference to variable", () => {
				const {
					dt,
					markers: { var1Definition, var1Reference1, var1Reference2 },
				} = parseTemplateWithMarkers(userFuncsTemplate1, [], {
					ignoreWarnings: true,
				});

				// Cursor at reference to "var1" inside var2
				testGetReferences(dt, var1Reference1.index, [
					var1Definition.index,
					var1Reference1.index,
					var1Reference2.index,
				]);

				// Cursor at reference to "var1" inside outputs2
				testGetReferences(dt, var1Reference2.index, [
					var1Definition.index,
					var1Reference1.index,
					var1Reference2.index,
				]);
			...
			test("At definition of variable", () => {
				...
				testGetReferences(dt, var1Definition.index, [
					var1Definition.index,
					var1Reference1.index,
					var1Reference2.index,
				]);
		...
		suite("UDF Find user function references", () => {
			...
			test("At reference to user-defined function, cursor inside the namespace portion", () => {
				const {
					dt,
					markers: { udfDef, udfReferenceAtNamespace },
				} = parseTemplateWithMarkers(userFuncsTemplate1, [], {
					ignoreWarnings: true,
				});

				// Cursor at reference to "udf.string" inside the namespace
				testGetReferences(dt, udfReferenceAtNamespace.index, [
					udfDef.index,
			...
			test("At reference to user-defined function, cursor inside the name portion", () => {
				const {
					dt,
					markers: { udfStringDefinition, udfReferenceAtName },
				} = parseTemplateWithMarkers(userFuncsTemplate1, [], {
					ignoreWarnings: true,
				});

				// Cursor at reference to "udf.string" inside the name
				testGetReferences(dt, udfReferenceAtName.index, [
					udfStringDefinition.index,
					udfReferenceAtName.index,
				]);
			});
			...
			test("At definition of user-defined function", () => {
				...
				testGetReferences(dt, udfStringDefinition.index, [
					udfStringDefinition.index,
					udfReferenceAtName.index,
				]);
			});
			...
			test("At definition of user-defined namespace", () => {
				...
				// Cursor at definition of "udf.string"
				testGetReferences(dt, udfDef.index, [
					udfDef.index,
					udfReferenceAtNamespace.index,
				]);
			});
			...
			test("Reference to built-in function with same name as UDF function doesn't find UDF function call", () => {
				const {
					dt,
					markers: { stringRef1, stringRef2, stringRef3, stringRef4 },
				} = parseTemplateWithMarkers(userFuncsTemplate1);
				testGetReferences(dt, stringRef4.index, [
					stringRef1.index,
					stringRef2.index,
					stringRef3.index,
					stringRef4.index,
			...
			test("Reference to built-in function in outer scope finds it in all scopes", () => {
				const {
					dt,
					markers: { addReferenceInUdfString, addReferenceInOutput4 },
				} = parseTemplateWithMarkers(userFuncsTemplate1);

				// Cursor at "add" in output4
				testGetReferences(dt, addReferenceInOutput4.index, [
					addReferenceInOutput4.index,
					addReferenceInUdfString.index,
				]);
			});
			...
			test("Reference to built-in function in function scope finds it in all scopes", () => {
				...
				testGetReferences(dt, addReferenceInUdfString.index, [
					addReferenceInOutput4.index,
					addReferenceInUdfString.index,
				]);
			});
	...
	suite("UDF Hover Info", () => {
		...
		test("Hover over UDF parameter reference", async () => {
			...
			await testHover(
				dt,
				udfyearReference.index,
				`**year**${os.EOL}*(function parameter)*`
			);
		});
		...
		test("Hover over built-in function reference", async () => {
			...
			await testHover(
				dt,
				stringRef4.index,
				`**string(valueToConvert)**${os.EOL}*(function)*${os.EOL}${os.EOL}Converts the specified value to String.`
			);
		});
		...
		test("Hover over user-defined function reference's name", async () => {
			...
			await testHover(
				dt,
				udfReferenceAtName.index,
				`**udf.string(year [int], month, day [int]) [string]**${os.EOL}*(user-defined function)*`
			);
		});
		...
		test("Hover over user-defined function reference's namespace", async () => {
			const {
				dt,
				markers: { udfReferenceAtNamespace },
			} = parseTemplateWithMarkers(userFuncsTemplate1, [], {
				ignoreWarnings: true,
			});
			await testHover(
	...
	suite("UDF Go To Definition", () => {
		async function testGoToDefinition(
			...
			assert.deepStrictEqual(
				refInfo.definition.nameValue!.span.startIndex,
				expectedDefinitionStart
			);
		}
		...
		test("User-defined function, cursor inside the name portion", async () => {
			...
			await testGoToDefinition(
				dt,
				udfReferenceAtName.index,
				DefinitionKind.UserFunction,
				udfStringDefinition.index - 1
			);
		});
	}); // suite UDF Go To Definition
	...
	suite("UDF Completions", () => {
					...
					"members": {
						...
						parameters: {
							...
							"output": {
								"type": "string",
								"value":
									"[concat(string(parameters('year')), '-', string(parameters('month')), '-', string(parameters('day')))]",
							},
						},
						udf: {
							"parameters": [
		...
		const allBuiltinsExpectedCompletions = allTestDataExpectedCompletions(
			0,
			0
		).map((c) => <[string, string]>[c.label, c.insertText]);
		const allNamespaceExpectedCompletions: [string, string][] = [
			["mixedCaseNamespace", "mixedCaseNamespace"],
			["udf", "udf"],
		];
		...
		const allMixedCaseNsFunctionsCompletions: [string, string][] = [
			["mixedCaseNamespace.howdy", "howdy"],
		];

		suite("Completing UDF function names", () => {
			suite("Completing inside xxx in udf.xxx", () => {
				createExpressionCompletionsTestEx(
					userFuncsTemplate2,
					"<output1>",
					"udf.p<!cursor!>",
					allUdfNsFunctionsCompletions
				);
				createExpressionCompletionsTestEx(
					userFuncsTemplate2,
					"<output1>",
			...
			suite(
				"Completing built-in functions with UDF function names",
				() => {
					createExpressionCompletionsTestEx(
						userFuncsTemplate2,
						"<output1>",
						"udf1<!cursor!>",
						allBuiltinsExpectedCompletions.concat(
							allNamespaceExpectedCompletions
						)
					);
				}
			);
			...
			suite(
				"Completing udf.param does not find built-in parameters function",
				() => {
					createExpressionCompletionsTestEx(
						userFuncsTemplate2,
						"<output1>",
			...
			suite("Completing udf. gives udf's functions", () => {
				...
				createExpressionCompletionsTestEx(
					userFuncsTemplate2,
					"<output1>",
					"mixedCaseNamespace.<!cursor!>",
					allMixedCaseNsFunctionsCompletions
				);
			});
		...
		suite(
			"Completing UDF namespaces after a function name already exists",
			() => {
				...
				suite("Matches namespaces and built-in functions", () => {
					createExpressionCompletionsTestEx(
						...
						[
							...allNamespaceExpectedCompletions,
							...allBuiltinsExpectedCompletions,
						]
					);
					createExpressionCompletionsTestEx(
						userFuncsTemplate2,
				...
				suite(
					"User namespaces and functions not available in function scope",
					() => {
						createExpressionCompletionsTestEx(
							userFuncsTemplate2,
							"<stringOutputValue>",
				...
				suite(
					"Property access completions from a function call",
					() => {
						// Built-in function
						createExpressionCompletionsTestEx(
							userFuncsTemplate2,
							...
							["id", "location", "name", "properties", "tags"]
						);
				...
				suite(
					"Property access completions from a properties call",
					() => {
						...
						createExpressionCompletionsTestEx(
							template,
							"<propaccess>",
							"parameters('param1').<!cursor!>",
							["a"]
						);
						createExpressionCompletionsTestEx(
							template,
							"<propaccess>",
							"parameters('param1').a.<!cursor!>",
							["b"]
						);
						...
						createExpressionCompletionsTestEx(
							template,
							"<propaccess>",
							"udf.parameters('param1').<!cursor!>",
							[]
						);
						createExpressionCompletionsTestEx(
							template,
							"<propaccess>",
							"udf.parameters('param1').a.<!cursor!>",
							[]
						);
					}
				);
				...
				suite(
					"Property access completions from a variables call",
					() => {
						const template: Partial<IDeploymentTemplate> = {
							"variables": {
								"var1": {
