...
suite("Nested templates", () => {
	...
	suite("detect scope", () => {
		function createDetectScopeTest(
			...
			test(testName, async () => {
				const template: IPartialDeploymentTemplate = properties
					? {
							"resources": [
								{
		...
		createDetectScopeTest(
			"no properties at all - no scope because it can't be a nested template",
			undefined,
			undefined
		);
		createDetectScopeTest(
			"no expressionEvaluationOptions property",
			{},
			ExpressionScopeKind.outer
		);
		createDetectScopeTest(
	...
	suite("inner scope", () => {
		...
		test("inner scope variables", async () => {
						...
						"properties": {
							...
							"template": {
								...
								"variables": {
									...
									"<!v2innerdef!>v2":
										"[variables('<!v1innerref1!>v1')]", // << References inner v1
								},
								"resources": [
									{
										...
										"location":
											"[variables('<!v2innerref1!>v2')]", // << References inner v2
									},
								],
							},
						},
			...
			// v1 root
			const v1rootref1pc = dt.getContextFromDocumentCharacterIndex(
				v1rootref1.index,
				undefined
			);
			...
			assert.deepEqual(
				...
				[v1rootdef.index, v1rootref1.index],
				"v1 root"
			);

			// v2 root
			const v2rootref1pc = dt.getContextFromDocumentCharacterIndex(
				v2rootdef.index,
				undefined
			);
			...
			assert.deepEqual(
				...
				[v2rootdef.index, v2rootref1.index],
				"v2 root"
			);

			// v1 inner
			const v1innerdef1pc = dt.getContextFromDocumentCharacterIndex(
				v1innerdef.index,
				undefined
			);
			...
			assert.deepEqual(
				...
				[v1innerdef.index, v1innerref1.index, v1innerref2.index],
				"v1 inner"
			);

			// v2 inner
			const v2innerref1pc = dt.getContextFromDocumentCharacterIndex(
				v2innerref1.index,
				undefined
			);
			...
			assert.deepEqual(
				...
				[v2innerdef.index, v2innerref1.index],
				"v2 inner"
			);
		});
		...
		test("inner scope parameters", async () => {
				...
				"resources": [
					{
						...
						"properties": {
							...
							"template": {
								...
								"parameters": {
									"<!p1innerdef!>p1": {
										"type": "string",
										"defaultValue":
											"[parameters('<!p1innerref2!>p1')]", // << References nested p1 (self-referential, but doesn't matter for test)
									},
								},
								"resources": [
									{
							...
							"parameters": {
								"<!p1innerref4!>p1": {
									// << value for inner p1, therefore it's a reference to inner p1
			...
			// root p1
			const p1rootref1pc = dt.getContextFromDocumentCharacterIndex(
				p1rootref1.index,
				undefined
			);
			const p1rootref1References = p1rootref1pc.getReferences();
			assert.deepEqual(getReferencesStartIndices(p1rootref1References!), [
				p1rootdef.index,
				p1rootref1.index,
				p1rootref2.index,
				p1rootref3.index,
			...
			// inner p1
			const p1innerref1pc = dt.getContextFromDocumentCharacterIndex(
				p1innerref1.index,
				undefined
			);
		...
		suite("inner scope functions", () => {
			const template: IPartialDeploymentTemplate = {
							...
							"template": {
								...
								"outputs": {
									"nestedOutput": {
										"type": "string",
										"value":
											"[inner.<!innerfunc1ref1!>func1('from nested')]",
									},
								},
								"functions": [
									{
										...
										"members": {
											"<!innerfunc1def!>func1": {
												...
												"output": {
													"type": "string",
													"value":
														"[parameters('<!innerp1ref!>innerp1')]", // References inner func1's p1
												},
											},
										},
									},
				...
				"outputs": {
					"o1": {
						"type": "string",
						"value":
							"[reference('nested').outputs.nestedOutput.value]",
					},
					"o2": {
						"type": "string",
			...
			test("inner.func1", async () => {
				const {
					dt,
					markers: { innerfunc1def, innerfunc1ref1 },
				} = parseTemplateWithMarkers(template);
				const innerfunc1defpc = dt.getContextFromDocumentCharacterIndex(
					innerfunc1def.index,
					undefined
				);
				...
				assert.deepEqual(
					...
					[innerfunc1def.index, innerfunc1ref1.index]
				);
			});
			...
			test("inner.func1.innerp1", async () => {
				...
				assert.deepEqual(getReferencesStartIndices(refs!), [
					innerp1def.index,
					innerp1ref.index,
				]);
			});
	...
	suite("outer scope", () => {
		...
		test("outer scope always accesses root-level variables", async () => {
								...
								"resources": [
									{
										...
										"location":
											"[variables('<!v2ref1!>v2')]", // << References root v2
									},
								],
							},
						},
			...
			// v1
			const v1ref1pc = dt.getContextFromDocumentCharacterIndex(
				v1ref1.index,
				undefined
			);
			const v1ref1References = v1ref1pc.getReferences();
			assert.deepEqual(getReferencesStartIndices(v1ref1References!), [
				v1def.index,
				v1ref1.index,
				v1ref2.index,
				v1ref3.index,
			...
			// v2
			const v2ref1pc = dt.getContextFromDocumentCharacterIndex(
				v2ref1.index,
				undefined
			);
			const v2ref1References = v2ref1pc.getReferences();
			assert.deepEqual(getReferencesStartIndices(v2ref1References!), [
				v2def.index,
				v2ref1.index,
			]);
		});
		...
		test("outer scope always accesses root-level parameters", async () => {
				...
				"resources": [
					{
						...
						"properties": {
							...
							"template": {
								...
								"parameters": {
									"p1": {
										// No way to reference this
										"type": "string",
										"defaultValue":
											"[parameters('<!p1ref4!>p1')]", // << References root p1
									},
								},
								"resources": [
									{
			...
			// p1
			const p1ref1pc = dt.getContextFromDocumentCharacterIndex(
				p1ref1.index,
				undefined
			);
			const p1ref1References = p1ref1pc.getReferences();
			assert.deepEqual(getReferencesStartIndices(p1ref1References!), [
				p1def.index,
				p1ref1.index,
				p1ref2.index,
				p1ref3.index,
		...
		suite("outer scope always accesses root-level functions", async () => {
							...
							"template": {
								...
								"outputs": {
									"nestedOutput1": {
										"type": "string",
										"value":
											"[root.<!rootfunc1ref1!>func1('from nested')]",
									},
								},
							},
						},
			...
			test("root.func1", async () => {
				const {
					dt,
					markers: { rootfunc1def, rootfunc1ref1, rootfunc1ref2 },
				} = parseTemplateWithMarkers(template);
				const pc = dt.getContextFromDocumentCharacterIndex(
					rootfunc1def.index,
					undefined
	...
	suite("deeply nested", () => {
										...
										"template": {
																...
																"nestedOutput":
																	{
																		"type": "string",
																		"value":
																			"[inner2.<!inner2func1ref1!>func1('inner2.func1 from outer3')]",
																	},
															},
														},
													},
											...
											"outputs": {
												"nestedOutput": {
													"type": "string",
													"value":
														"[inner2.<!inner2func1ref2!>func1('from inner.func1')]",
												},
											},
											"functions": [
												{
														...
														"<!inner2func1def!>func1":
															{
																...
																"output": {
																	"type": "string",
																	"value":
																		"[parameters('<!inner2func1innerp2ref1!>innerp2')]",
																},
															},
													},
												},
						...
						"template": {
							...
							"outputs": {
								"nestedOutput": {
									"type": "string",
									"value":
										"[root.func1('root.func1 from outer')]",
								},
							},
						},
					},
		...
		test("no errors", async () => {
			parseTemplateWithMarkers(deeplyNestedTemplate, [
		...
		test("outer3 references inner2.func1 from parent scope", async () => {
			const {
				dt,
				markers: { inner2func1def, inner2func1ref1, inner2func1ref2 },
			} = parseTemplateWithMarkers(deeplyNestedTemplate);
			const pc = dt.getContextFromDocumentCharacterIndex(
		...
		test("inner2.innerp2", async () => {
			const {
				dt,
				markers: { inner2func1innerp2def, inner2func1innerp2ref1 },
			} = parseTemplateWithMarkers(deeplyNestedTemplate);
			const pc = dt.getContextFromDocumentCharacterIndex(
	...
	suite("real examples", () => {
		testWithRealFunctionMetadata(
			"https://github.com/Azure/azure-resource-manager-schemas/issues/994",
			async () => {
				parseTemplate(
					{
						"$schema":
						...
						"resources": [
							{
								...
								"comments":
									"This adds the UAI to the management recourse group as a storage blob reader.",
								"dependsOn": ["uai-DevOps-AgentInstaller"],
								"tags": {
									"displayName":
										"Role Based Access Control Deployment",
								},
								"resourceGroup":
									"[parameters('AgentManagementRG')]",
								"properties": {
									"expressionEvaluationOptions": {
										"scope": "inner",
									},
										...
										"resources": [
											{
												...
												"properties": {
													"roleDefinitionId":
														"[concat(subscription().id, '/providers/Microsoft.Authorization/roleDefinitions/', 'b7e6dc6d-f1e8-4753-8033-0f276bb0955b')]",
													"principalId":
									...
									"parameters": {
										"principalId": {
											"value":
												"[reference('uai-DevOps-AgentInstaller', '2018-11-30').principalId]",
										},
									},
								},
							},
