...
suite("Parameter file completions", () => {
	...
	function createParamsCompletionsTest(
		...
		const fullName = isNullOrUndefined(options.cursorIndex)
			? testName
			: `${testName}, index=${options.cursorIndex}`;
		test(fullName, async () => {
			let dt: DeploymentTemplateDoc | undefined = template
			...
			const {
				dp,
				markers: { cursor },
			} = parseParametersWithMarkers(params);
			// tslint:disable-next-line: strict-boolean-expressions
			const cursorIndex = !isNullOrUndefined(options.cursorIndex)
				? options.cursorIndex
				: cursor?.index;
			if (isNullOrUndefined(cursorIndex)) {
				assert.fail(
					`Expected either a cursor index in options or a <!cursor!> in the parameters file`
				);
			}
			...
			const completionNames = completions.items
				.map((c) => c.label)
				.sort();
			const completionInserts = completions.items
				.map((c) => c.insertText)
				.sort();

			const expectedNames = (<unknown[]>expectedNamesAndInsertTexts)
				.map((e) => (Array.isArray(e) ? <string>e[0] : <string>e))
				.sort();
			let expectedInsertTexts: string[] | undefined;
			if (
				expectedNamesAndInsertTexts.every(
					(e: [string, string] | string) => Array.isArray(e)
				)
			) {
				expectedNamesAndInsertTexts = (<[string, string][]>(
					expectedNamesAndInsertTexts
				))
					.map((e) => e[1])
					.sort();
			}
			...
			assert.deepStrictEqual(
				completionNames,
				expectedNames,
				"Completion names didn't match"
			);
			if (expectedInsertTexts !== undefined) {
				assert.deepStrictEqual(
					completionInserts,
					expectedInsertTexts,
					"Completion insert texts didn't match"
				);
			}
		});
	}
	...
	suite("Completions for new parameters", async () => {
		suite(
			"Params file with missing parameters section - no completions anywhere",
			() => {
		...
		suite(
			"Offer completions for properties from template that aren't already defined in param file",
			() => {
				createParamsCompletionsTest(
					...
					{},
					[`"p2"`, `"p10"`, newParamValueCompletionLabel]
				);
