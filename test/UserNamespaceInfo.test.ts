...
suite("Hover.UserNamespaceInfo", () => {
	suite("getHoverText", () => {
		test("no members", async () => {
			...
			const info = new UsageInfoHoverInfo(
				"type",
				dt.topLevelScope.getFunctionNamespaceDefinition(
					"udf"
				)!.usageInfo,
				fakeSpan
			);
			assert.equal(
				info.getHoverText().value,
				`**udf**${os.EOL}*(user-defined namespace)*${os.EOL}${os.EOL}No members`
			);
		});
	});
	...
	test("one member, no params", async () => {
		...
		assert.equal(
			info.getHoverText().value,
			`**udf**${os.EOL}*(user-defined namespace)*${os.EOL}${os.EOL}Members:${os.EOL}* date()`
		);
	});
	...
	test("one member, one param, no type", async () => {
		...
		assert.equal(
			info.getHoverText().value,
			`**udf**${os.EOL}*(user-defined namespace)*${os.EOL}${os.EOL}Members:${os.EOL}* date(param)`
		);
	});
	...
	test("one member, two params", async () => {
		...
		assert.equal(
			info.getHoverText().value,
			`**udf**${os.EOL}*(user-defined namespace)*${os.EOL}${os.EOL}Members:${os.EOL}* date(param1 [string], param2 [securestring])`
		);
	});
	...
	test("two members", async () => {
		...
		assert.equal(
			info.getHoverText().value,
			`**udf**${os.EOL}*(user-defined namespace)*${os.EOL}${os.EOL}Members:${os.EOL}* date(param1 [string], param2 [securestring])${os.EOL}* time(param [string])`
		);
	});
});
