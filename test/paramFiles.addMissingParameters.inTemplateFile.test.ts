...
suite("Add missing parameters for nested/linked templates", () => {
	...
	function createAddMissingParamsTest(
		...
		testWithLanguageServer(testName, async () => {
			...
			const { dt: expectedDt } = parseTemplateWithMarkers(
				expectedResult,
				undefined,
				{ fromFile: true }
			);
			...
			const {
				dt,
				markers: { bang },
			} = parseTemplateWithMarkers(template, undefined, {
				fromFile: true,
				documentUri: templateUri,
				tabSize,
			});
			...
			if (linkedTemplate) {
				const { dt: childDt } = parseTemplateWithMarkers(
					linkedTemplate,
					undefined,
					{ fromFile: true, documentUri: childUri, tabSize }
				);
				...
				const allLoadedTemplates = new NormalizedMap<
					Uri,
					DeploymentTemplateDoc
				>(getNormalizedDocumentKey);
			...
			const codeActions: (CodeAction | Command)[] = dt.getCodeActions(
				undefined,
				range,
			...
			assert.equal(
				codeActions.length,
				2,
				"Expecting a 2 add missing params code actions"
			);
			const expectedName =
				whichParams === Params.all
					? "Add all missing parameters"
					: "Add missing required parameters";
			const codeAction: CodeAction | undefined = filterByType(
				codeActions,
				CodeAction
			).find((ca) => ca.title === expectedName);
			const args: IAddMissingParametersArgs = <IAddMissingParametersArgs>(
				codeAction!.command!.arguments![1]
			);

			const doc: TextDocumentFake = new TextDocumentFake(
				dt.documentText,
				templateUri
			);
