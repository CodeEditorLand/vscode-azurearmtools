			...
			test(testName, async () => {
				await testStringAtEachIndex(
					...
					(text: string, index: number) => {
						const dp = new DeploymentParametersDoc(
							text,
							Uri.file("test parameter file"),
							0
						);
						const pc = dp.getContextFromDocumentCharacterIndex(
							index,
							undefined
						);
						const canAddHere = canAddPropertyValueHere(
							pc.document.topLevelParameterValuesSource,
