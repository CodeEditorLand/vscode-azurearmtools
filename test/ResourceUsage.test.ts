...
suite("ResourceUsage (schema.stats telemetry)", () => {
	async function testGetResourceUsage(
		...
		const availableResourceTypesAndVersions = new CaseInsensitiveMap<
			string,
			string[]
		>();
		availableResourceTypesAndVersions.set(
			"Microsoft.Network/virtualNetworks",
			["2016-08-01", "2018-10-01", "2019-08-01"]
		);
		availableResourceTypesAndVersions.set(
			"Microsoft.Resources/deployments",
			["2020-10-01"]
		);
		const [resourceUsage, invalidResourceCounts, invalidVersionCounts] =
			dt.getResourceUsage(availableResourceTypesAndVersions);

		const expectedResourceCountsHistogram = new Histogram();
		for (let propName of Object.getOwnPropertyNames(
			expectedResourceCounts
		)) {
			expectedResourceCountsHistogram.add(
				propName,
				expectedResourceCounts[propName]
			);
		}
		...
		if (expectedInvalidResourceCounts) {
			...
			for (let propName of Object.getOwnPropertyNames(
				expectedInvalidResourceCounts
			)) {
				expectedInvalidResourceCountsHistogram.add(
					propName,
					expectedInvalidResourceCounts[propName]
				);
			}
			assert.deepStrictEqual(
				invalidResourceCounts,
				expectedInvalidResourceCountsHistogram
			);
		}
		if (expectedInvalidVersionCounts) {
			const expectedInvalidVersionCountsHistogram = new Histogram();
			for (let propName of Object.getOwnPropertyNames(
				expectedInvalidVersionCounts
			)) {
				expectedInvalidVersionCountsHistogram.add(
					propName,
					expectedInvalidVersionCounts[propName]
				);
			}
			assert.deepStrictEqual(
				invalidVersionCounts,
				expectedInvalidVersionCountsHistogram
			);
		}
	}
