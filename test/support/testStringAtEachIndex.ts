...
export async function testStringAtEachIndex<T>(
	...
	for (let marker of Object.keys(expectedValues)) {
		for (let i = 1; ; ++i) {
			...
			const newString = textWithMarkers.replace(
				`<!${marker}!>`,
				`<!${marker}$${i}!>`
			);
	...
	for (let i = 0; i < unmarkedText.length; ++i) {
		...
		const currentMarker: string | undefined = Object.getOwnPropertyNames(
			markers
		).find((key) => markers[key].index === i);
		if (currentMarker) {
			...
			if (i === 0) {
				assert.fail(
					"Must have a marker at the very beginning of the string"
				);
			}
		}
