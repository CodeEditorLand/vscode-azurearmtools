...
export function parseTemplate(
	...
	const { dt } = parseTemplateWithMarkers(
		template,
...
export function parseTemplateWithMarkers(
	...
	const withReplacements = options?.replacements
		? replaceInTemplate(template, options.replacements)
		: template;
	const { unmarkedText, markers } = getDocumentMarkers(
		withReplacements,
		options
	);
	const dt: DeploymentTemplateDoc = new DeploymentTemplateDoc(
		unmarkedText,
		options?.documentUri ?? Uri.file("/test-template.json"),
		0
	);
	...
	const errorMessages = errors.map(
		(e) =>
			<DiagIssue>{
				line: e.span.startIndex,
				msg: getMessage(e),
				kind: e.severity,
			}
	);
	const warningMessages = warnings
		.filter((e) => e.severity === IssueSeverity.Warning)
		.map(
			(e) =>
				<DiagIssue>{
					line: e.span.startIndex,
	...
	function getMessage(d: Issue): string {
		const typeString =
			d.severity === IssueSeverity.Error
				? "Error"
				: d.severity === IssueSeverity.Warning
				? "Warning"
				: "Information";
		let msg = `${typeString}: ${d.message}`;
		if (options?.includeDiagnosticLineNumbers) {
			msg = `${
				dt.getDocumentPosition(d.span.startIndex).line + 1
			}: ${msg}`;
		}
	...
	if (expectedDiagnosticMessages) {
		...
		const sortedExpectedDiag = expected.sort(
...
export function parseParametersWithMarkers(
	...
	const dp: DeploymentParametersDoc = new DeploymentParametersDoc(
		unmarkedText,
		Uri.file("/test parameter file.json"),
		0
	);
...
export function getDocumentMarkers(
	...
	while (true) {
		let match: RegExpMatchArray | null = modified.match(
			/<!([a-zA-Z][a-zA-Z0-9$]*)!>/
		);
		if (!match) {
			break;
		...
		modified =
			modified.slice(0, marker.index) +
			modified.slice(index + match[0].length);
	}
...
export function replaceInTemplate(
	...
	// $REPLACE_PROP_LINE$
	function getPropLineReplacementString(
		s: string,
		addComma: boolean
	): string {
		const result = templateString.replace(
			...
			(_substring: string, ...args: any[]) => {
				...
				assert(
					...
					`Replacement not specified for $REPLACE_PROP_LINE$ with key '${key}'`
				);
				if (typeof replacement === "string") {
					return replacement;
				}

				const props = Object.getOwnPropertyNames(replacement)
					.map(
						(propName) =>
	...
	try {
		const newTemplateWithComma = getPropLineReplacementString(
			templateString,
			true
		);
	...
	try {
		const newTemplateWithoutComma = getPropLineReplacementString(
			templateString,
			false
		);
		...
		throw new Error(
			`replaceInTemplate: Could not parse resulting template:\n${
