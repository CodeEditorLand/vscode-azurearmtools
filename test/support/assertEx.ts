	...
	export function arraysEqual<T>(
		...
		try {
			assert.equal(
				actual.length,
				expected.length,
				`${message}: Number of diagnostics does not match expected`
			);
	...
	function deepPartialClone(o: unknown, shape: unknown): unknown {
		if (Array.isArray(o) || Array.isArray(shape)) {
			...
			for (const propName of Object.getOwnPropertyNames(shape)) {
				clonedObject[propName] = deepPartialClone(
					(<keyedObject>o)[propName],
					(<keyedObject>shape)[propName]
				);
			}
