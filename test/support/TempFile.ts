...
export class TempFile {
	...
	public static fromExistingFile(existingFilePath: string): TempFile {
		assert(
			...
			`TempFile: Couldn't find path ${existingFilePath}`
		);
...
export class TempEditor {
	...
	public async open(): Promise<void> {
		if (!this._editor) {
			...
			// tslint:disable-next-line:no-non-null-assertion
			this._editor = await window.showTextDocument(
...
export class TempDocument {
	...
	public async open(): Promise<void> {
		if (!this._document) {
			this._document = await workspace.openTextDocument(
				this.tempFile.fsPath
			);
		}
	}
