...
import {
	backendValidationDiagnosticsSource,
	diagnosticsCompletePrefix,
	expressionsDiagnosticsSource,
	ExpressionType,
	ext,
	LanguageServerState,
	languageServerStateSource,
	readUtf8FileWithBom,
...
export type ITestDiagnosticsOptions = IGetDiagnosticsOptions &
	ICompareDiagnosticsOptions;
...
export interface IGetDiagnosticsOptions {
	...
	waitForDiagnosticsFilter?(
		results: IDiagnosticsResults
	): boolean | Promise<boolean>;
}

export async function testDiagnosticsFromFile(
	filePath: string | Partial<IDeploymentTemplate>,
	options: ITestDiagnosticsOptions,
	expected: ExpectedDiagnostics
): Promise<void> {
	await testDiagnosticsCore(
		async () => await getDiagnosticsForTemplate(filePath, 1, options),
		options,
		expected
	);
}
...
async function testDiagnosticsCore(
	getDiagnostics: () => Promise<IDiagnosticsResults>,
	options: ITestDiagnosticsOptions,
	expected: ExpectedDiagnostics
): Promise<void> {
...
export async function testDiagnosticsFromUri(
	documentUri: Uri,
	options: ITestDiagnosticsOptions,
	expected: ExpectedDiagnostics
): Promise<void> {
	await testDiagnosticsCore(
		async () => await getDiagnosticsForDocument(documentUri, 1, options),
		options,
		expected
	);
}
...
export async function getDiagnosticsForDocument(
	...
	let filterSources: DiagnosticSource[] = Array.from(
		Object.values(diagnosticSources)
	);

	if (options.includeSources) {
		filterSources = filterSources.filter((s) =>
			options.includeSources!.find((s2) => s2.name === s.name)
		);
	}
	if (options.ignoreSources) {
		filterSources = filterSources.filter(
			(s) => !options.ignoreSources!.includes(s)
		);
	}

	const includesLanguageServerSource = filterSources.some(
		isSourceFromLanguageServer
	);
	if (includesLanguageServerSource && DISABLE_LANGUAGE_SERVER) {
		throw new Error(
			"DISABLE_LANGUAGE_SERVER is set, but this test is trying to include a non-expressions diagnostic source"
		);
	}

	const timeoutMs = options.timeoutMs ?? defaultDiagnosticsTimeoutMs;

	let diagnosticsPromise = new Promise<IDiagnosticsResults>(
		async (resolve, reject): Promise<void> => {
			try {
				let currentDiagnostics: Diagnostic[] | undefined;

				function getCurrentDiagnostics(): IDiagnosticsResults {
					const sourceCompletionVersions: {
					...
					currentDiagnostics = currentDiagnostics.filter(
						(d) => d.source !== languageServerStateSource
					);

					// Filter diagnostics according to sources filter
					let filteredDiagnostics = currentDiagnostics.filter((d) =>
						filterSources.find((s) => d.source === s.name)
					);
					...
					filteredDiagnostics = filteredDiagnostics.filter(
						(d) => !d.message.startsWith(diagnosticsCompletePrefix)
					);
					...
					if (includesLanguageServerSource) {
						if (
							...
							throw new Error(
								`Language server failed on start-up: ${ext.languageServerStartupError}`
							);
						} else if (
							ext.languageServerState ===
							...
							throw new Error(
								`Language server is in stopped state`
							);
						}
					}
					...
					return {
						...
						sourceCompletionVersions,
					};
				}

				function areAllSourcesComplete(sourceCompletionVersions: {
					[source: string]: number;
				}): boolean {
					for (let source of filterSources) {
						const completionVersion =
							sourceCompletionVersions[source.name];
						if (completionVersion === undefined) {
							return false;
						}
						if (
							requiredSourceCompletionVersions[source.name] !==
								undefined &&
							completionVersion <
								requiredSourceCompletionVersions[source.name]
						) {
				...
				async function diagnosticsFilterPasses(
					results: IDiagnosticsResults
				): Promise<boolean> {
					if (options.waitForDiagnosticsFilter) {
						const filterValue =
							await options.waitForDiagnosticsFilter(results);
						return filterValue;
					}
				...
				let currentResults: IDiagnosticsResults =
					previousResults ?? getCurrentDiagnostics();
				const requiredSourceCompletionVersions = Object.assign(
					{},
					currentResults.sourceCompletionVersions
				);
				if (options.waitForChange || previousResults) {
					...
					for (let source in requiredSourceCompletionVersions) {
						requiredSourceCompletionVersions[source] =
							requiredSourceCompletionVersions[source] + 1;
					}
				}

				if (
					areAllSourcesComplete(
						currentResults.sourceCompletionVersions
					) &&
					(await diagnosticsFilterPasses(currentResults))
				) {
				...
				timer = setTimeout(() => {
					...
					reject(
						new Error(
							"Timed out waiting for diagnostics. Last retrieved diagnostics: " +
								(currentDiagnostics
									? currentDiagnostics
											.map((d) => d.message)
											.join("\n")
				...
				while (!done) {
					...
					if (
						areAllSourcesComplete(
							results.sourceCompletionVersions
						) &&
						(await diagnosticsFilterPasses(results))
					) {
						resolve(results);
						done = true;
					}
	...
	for (const source of Object.getOwnPropertyNames(
		diagnostics.sourceCompletionVersions
	)) {
		if (
			!(
				diagnostics.sourceCompletionVersions[source] >=
				expectedMinimumVersionForEachSource
			)
		) {
			assert.fail(
				`Expected diagnostics source to be at least 3, but found ${stringify(
					diagnostics
				)}`
			);
		}
	...
	try {
		...
		if (typeof templateContentsOrFileName === "string") {
			if (!!templateContentsOrFileName.match(/\.jsonc?$/)) {
				// It's a filename
				let sourcePath = resolveInTestFolder(
					templateContentsOrFileName
				);
			...
			const templateObject: Partial<IDeploymentTemplate> =
				templateContentsOrFileName;
		...
		if (!templateFile) {
			...
			if (
				!options.doNotAddSchema &&
				!templateContents.includes("$schema")
			) {
			...
			if (options.search) {
				let newContents = templateContents.replace(
					options.search,
					options.replace!
				);
		...
		if (options.parameters || options.parametersFile) {
			if (options.parameters) {
				const { unmarkedText: unmarkedParams } =
					parseParametersWithMarkers(options.parameters);
				paramsFile = TempFile.fromContents(unmarkedParams);
			} else {
				assert(options.parametersFile);

				// First try relative to the template file
				let absPath: string = path.join(
					path.dirname(templateFile.fsPath),
				...
				assert(
					fse.pathExistsSync(absPath),
					`Couldn't find parameter file ${absPath}`
				);
		...
		let diagnostics: IDiagnosticsResults = await getDiagnosticsForDocument(
...
export function expectedDiagnosticToString(
	diagnostic: IExpectedDiagnostic
): string {
	return diagnosticToString(
		{
			...
			severity:
				diagnostic.severity === ExpectedDiagnosticSeverity.Error
					? DiagnosticSeverity.Error
					: diagnostic.severity === ExpectedDiagnosticSeverity.Warning
...
export function diagnosticToString(
	...
	assert(
		diagnostic.code === "",
		`Expecting empty code for all diagnostics, instead found Code="${String(
			diagnostic.code
	...
	let s = `${severity}: ${diagnostic.message} (${
		diagnostic.source
	})${maybeRange(diagnostic.range)}`;
...
function compareDiagnostics(
	...
	// Do the expected messages include ranges?
	let expectedHasRanges =
		expected.length === 0 ||
		(typeof expected[0] === "string" &&
			!!expected[0].match(/\[[0-9]+,[0-9]+-[0-9]+,[0-9]+\]/)) ||
		(typeof expected[0] !== "string" &&
			expected[0].startLineNumber !== undefined);
	let expectedHasPositions =
		expected.length > 0 &&
		typeof expected[0] === "string" &&
		!!expected[0].match(/\[[0-9]+,[0-9]+\]/);
	let includeRange: IncludeRange =
		!!options.includeRange || expectedHasRanges
			? IncludeRange.range
			: !!options.includePositions || expectedHasPositions
			? IncludeRange.position
			: IncludeRange.no;

	let expectedAsStrings =
		expected.length === 0
			? []
			: typeof expected[0] === "string"
			? expected
			: (<IExpectedDiagnostic[]>expected).map((d) =>
					typeof d === "string" ? d : expectedDiagnosticToString(d)
			  );
	let actualAsStrings = actual.map((d) =>
		diagnosticToString(d, options, includeRange)
	);
...
function transformResults(
	results: Diagnostic[],
	options: ICompareDiagnosticsOptions
): void {
...
export function simplifyBadTypeResourceMessage(msg: string): string {
	return msg.replace(
		/Value must be one of the following values: [a-zA-Z0-9-, "./]+/,
