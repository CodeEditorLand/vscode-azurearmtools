...
export function createExpressionCompletionsTestEx(
	...
	testWithPrep(
		...
		async () => {
			...
			template = stringify(template).replace(
				contextFind,
				expressionWithCursorMarker
			);
			...
			const completionItems = completions.items.filter(
				(c) => !(options?.ignoreCompletionNames ?? []).includes(c.label)
			);

			const completionNames = completionItems.map((c) => c.label).sort();

			const completionInserts = completionItems
				.map((c) => c.insertText)
				.sort();

			let expectedNames = (<unknown[]>expectedCompletions).map((e) =>
			...
			if (expectedCompletions.every((e) => Array.isArray(e))) {
				expectedInsertTexts = (<[string, string][]>expectedCompletions)
					.map((e) => e[1])
					.sort();
			}
