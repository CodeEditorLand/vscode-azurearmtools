...
export async function actThenWait<T, U>(
	action: () => Promise<T> | T,
	promise: Promise<U>
): Promise<U> {
	await action();
...
export function getDocumentChangedPromise(
...
export function getCompletionItemsPromise(
	document: TextDocument,
	timeout: number = defaultTimeout
): Promise<ICompletionsSpyResult> {
...
export function getCompletionItemResolutionPromise(
	item?: CompletionItem,
	timeout: number = defaultTimeout
