...
export async function simulateCompletion(
	...
	let deploymentTemplate: DeploymentTemplateDoc = new DeploymentTemplateDoc(
		editor.document.getText(),
		editor.document.uri,
		editor.document.version
	);
	let pc = deploymentTemplate.getContextFromDocumentLineAndColumnIndexes(
		pos.line,
		pos.character,
		undefined,
		true
	...
	if (triggerCharacter) {
		...
		const newContents = await typeInDocumentAndWait(
			editor,
			triggerCharacter
		);
		deploymentTemplate = new DeploymentTemplateDoc(
			newContents,
			editor.document.uri,
			editor.document.version
		);
		pos = editor.selection.anchor;
		pc = deploymentTemplate.getContextFromDocumentLineAndColumnIndexes(
			pos.line,
			pos.character,
			undefined,
			true
	...
	let result = await pc.getCompletionItems(
		triggerCharacter,
	...
	if (result.triggerSuggest) {
		...
		result = await pc.getCompletionItems(
			undefined,
			<number>editor.options.tabSize
		);
	...
	const snippet = result.items.find(
		(s) => s.label === completion || s.label === `"${completion}"`
	);
	writeToLog(`Available completions: ${stringify(result)}`);
	if (!snippet) {
		throw new Error(
			`Couldn't find completion with label '${completion}' that is available at this location`
		);
	}
	...
	if (snippet.additionalEdits?.length) {
		assert(
			snippet.additionalEdits.length === 1,
			"Not implemented: More than one edit"
		);
