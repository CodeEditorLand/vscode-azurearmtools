...
export async function ensureExtensionHasInitialized(
	totalTimeout: number
): Promise<void> {
	async function ensureDotnetExtensionActivated(): Promise<void> {
		...
		await delayWhileSync(
			5 * 1000,
			() => {
				extensionDotnet = extensions.getExtension(
					"ms-dotnettools.vscode-dotnet-runtime"
				);
				writeToLog(
					extensionDotnet !== undefined
						? "Dotnet extension found"
						: "Dotnet extension not found",
					true
				);
	...
	async function waitForExtensionInitialization(
		timeout: number
	): Promise<boolean> {
		let start = Date.now();
		writeToLog(
			`Extension initialization state: ${
				ext.extensionStartupComplete
					? "Completed"
					: ext.extensionStartupComplete === undefined
					? "Not started"
					: "In progress"
			}`,
			true
		...
		while (true) {
			...
			if (extensionStartupComplete) {
				...
				writeToLog(
					`>>> Extension initialization failed: ${ext.extensionStartupError}`,
					true
				);
	...
	writeToLog(
		">>> First timeout hit, will try to activate extension manually",
		true
	);
	...
	if (!result2) {
		throw new Error(
			"Timed out waiting for extension initialization to complete"
		);
	}
}
