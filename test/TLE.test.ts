...
suite("TLE", () => {
	const emptyScope = new TopLevelTemplateScope(
		new DeploymentTemplateDoc("", Uri.file("/doc"), 0),
		undefined,
		"empty scope"
	);
	...
	function getReferenceErrors(
		scope: TemplateScope,
		expression: TLE.Value | undefined
	): Issue[] {
		...
		FindReferencesAndErrorsVisitor.visit(
			...
			0,
			expression,
			AzureRMAssets.getFunctionsMetadata(),
			referenceListsMap,
			issues
		);
		return issues;
	...
	suite("isExpression", () => {
		function createIsExpressionTest(
	...
	suite("StringValue", () => {
		suite("constructor(tle.Token)", () => {
			test("with undefined token", () => {
				...
				assert.throws(() => {
					new TLE.StringValue(<any>undefined);
				});
			});
		...
		suite("contains(number)", () => {
			test("with character index less than start index", () => {
				const value: TLE.StringValue = new TLE.StringValue(
					TLE.Token.createQuotedString(5, "'hello'")
				);
	...
	suite("NumberValue", () => {
		suite("contains(number)", () => {
			test("with character index less than start index", () => {
				const value: TLE.NumberValue = new TLE.NumberValue(
					TLE.Token.createNumber(3, "1")
				);
			...
			test("with character index inside value", () => {
				const value: TLE.NumberValue = new TLE.NumberValue(
					TLE.Token.createNumber(3, "1235235")
				);
				assert(value.contains(7));
			});
			...
			test("ends with comments and whitespace", () => {
				const value: TLE.NumberValue = new TLE.NumberValue(
					TLE.Token.createNumber(3, "1237 /* there */")
				);
				assert(value.contains(7));
			});
		});
	...
	suite("ArrayAccess", () => {
		suite("constructor(tle.Value,tle.Token,tle.Value,tle.Token)", () => {
			...
			test("with undefined _leftSquareBracket", () => {
				let source = new TLE.NumberValue(
					TLE.Token.createNumber(5, "2")
				);
				let index = new TLE.NumberValue(TLE.Token.createNumber(7, "3"));
				let rightSquareBracket = TLE.Token.createRightSquareBracket(8);
				// tslint:disable-next-line:no-any
			...
			test("with undefined _index", () => {
				...
				let leftSquareBracket = TLE.Token.createLeftSquareBracket(6);
				let rightSquareBracket = TLE.Token.createRightSquareBracket(8);
				let arrayAccess = new TLE.ArrayAccessValue(
					source,
					leftSquareBracket,
					undefined,
					rightSquareBracket
				...
				assert.deepStrictEqual(
					leftSquareBracket,
					arrayAccess.leftSquareBracketToken
				);
				assert.deepStrictEqual(undefined, arrayAccess.indexValue);
				assert.deepStrictEqual(
					rightSquareBracket,
					arrayAccess.rightSquareBracketToken
				);
			});
		...
		suite("getSpan()", () => {
			...
			test("with no right square bracket", () => {
				...
				let leftSquareBracket = TLE.Token.createLeftSquareBracket(8);
				let index = new TLE.NumberValue(
					TLE.Token.createNumber(10, "10")
				);
				let arrayAccess = new TLE.ArrayAccessValue(
					source,
					leftSquareBracket,
		...
		suite("toString()", () => {
			...
			test("with no right square bracket", () => {
				...
				let leftSquareBracket = TLE.Token.createLeftSquareBracket(10);
				let index = new TLE.StringValue(
					TLE.Token.createQuotedString(20, "'hello'")
				);
				let arrayAccess = new TLE.ArrayAccessValue(
					source,
					leftSquareBracket,
	...
	suite("FunctionCallValue", () => {
		suite("constructor(tle.Token,tle.Token,tle.Value[],tle.Token)", () => {
			test("with undefined namespaceToken", () => {
				...
				let f = new TLE.FunctionCallValue(
					undefined,
					undefined,
					name,
					leftParenthesis,
	...
	suite("BraceHighlighter", () => {
		function getHighlights(
			...
			const context = template.getContextFromDocumentCharacterIndex(
		...
		suite("getHighlightCharacterIndexes(number,TLEParseResult)", () => {
			test("with quoted string that isn't a TLE", () => {
				let template = new DeploymentTemplateDoc(
					'"Hello world"',
					fakeId,
					0
				);
			...
			test("with empty TLE", () => {
				...
				assert.deepStrictEqual(
					[1, 2],
					getHighlights(template, 1),
					"When the caret is before a TLE's left square bracket, then the left and right square brackets should be highlighted."
				);
				assert.deepStrictEqual(
					[],
					getHighlights(template, 2),
					"When the caret is to the right of a TLE's left square bracket and to the left of the right square bracket, nothing should be highlighted."
				);
				assert.deepStrictEqual(
					[1, 2],
					getHighlights(template, 3),
					"When the caret is after a TLE's right square bracket, then the left and right square brackets should be highlighted."
				);
			});
			...
			test("with function with left and right parenthesis", () => {
				...
				assert.deepStrictEqual(
					[8, 9],
					getHighlights(template, '"[concat'.length),
					"Both left and right parentheses should be highlighted when the caret is before the left parenthesis."
				);
				assert.deepStrictEqual([], getHighlights(template, 9));
				assert.deepStrictEqual(
					[8, 9],
					getHighlights(template, '"[concat()'.length),
					"Both left and right parentheses should be highlighted when the caret is after the right parenthesis."
				);
			});
		});
	});
	...
	suite("Undefined Parameters And Variables", () => {
		suite("visitString(StringValue)", () => {
			test("with empty StringValue in parameters() function", () => {
				...
				const referenceListsMap = new Map<
					INamedDefinition,
					ReferenceList
				>();
				const issues: Issue[] = [];
				...
				const stringValue = new TLE.StringValue(
					TLE.Token.createQuotedString(17, "''")
				);
	...
	suite("Parser", () => {
		suite("parse(string)", () => {
			...
			test("with non-empty quoted string", () => {
				...
				assert.deepStrictEqual(
					new TLE.StringValue(
						TLE.Token.createQuotedString(0, '"hello"')
					),
					pr.expression
				);
				assert.equal(undefined, pr.rightSquareBracketToken);
			...
			test("with left square bracket (but no right square bracket)", () => {
				...
				assert(pr);
				assert.deepStrictEqual(
					TLE.Token.createLeftSquareBracket(1),
					pr.leftSquareBracketToken
				);
			...
			test("with two left square brackets", () => {
				...
				assert.deepStrictEqual(
					new TLE.StringValue(
						TLE.Token.createQuotedString(0, '"[["')
					),
					pr.expression
				);
				assert.equal(undefined, pr.rightSquareBracketToken);
			...
			test("with left and right square brackets", () => {
				...
				assert.equal(undefined, pr.expression);
				assert.deepStrictEqual(
					TLE.Token.createRightSquareBracket(2),
					pr.rightSquareBracketToken
				);
				assert.deepStrictEqual(
					[
						new Issue(
			...
			test("with left and right square brackets after whitespace", () => {
				...
				assert(pr);
				assert.deepStrictEqual(
					TLE.Token.createLeftSquareBracket(3),
					pr.leftSquareBracketToken
				);
				assert.equal(undefined, pr.expression);
				assert.deepStrictEqual(
					TLE.Token.createRightSquareBracket(4),
					pr.rightSquareBracketToken
				);
				assert.deepStrictEqual(
					[
			...
			test("with left and right square brackets with whitespace between them", () => {
				...
				assert.deepStrictEqual(
					TLE.Token.createRightSquareBracket(6),
					pr.rightSquareBracketToken
				);
				assert.deepStrictEqual(
					[
			...
			test("with function namespace and period but no function name", () => {
				...
				assert.deepStrictEqual(
					new TLE.FunctionCallValue(
						TLE.Token.createLiteral(2, "concat"),
						TLE.Token.createPeriod(2 + "concat".length),
						undefined,
						undefined,
						[],
						[],
			...
			test("with function name without parentheses or arguments", () => {
				...
				assert.deepStrictEqual(
					TLE.Token.createRightSquareBracket(8),
					pr.rightSquareBracketToken
				);
				assert.deepStrictEqual(
					[
			...
			test("with function name and left parenthesis", () => {
				...
				assert.deepStrictEqual(
					TLE.Token.createRightSquareBracket(10),
					pr.rightSquareBracketToken
				);
				assert.deepStrictEqual(
					[
			...
			test("with function name and right parenthesis", () => {
				...
				assert.deepStrictEqual(
					TLE.Token.createRightSquareBracket(9),
					pr.rightSquareBracketToken
				);
				assert.deepStrictEqual(
					[
			...
			test("with function with no arguments", () => {
				...
				assert(pr);
				assert.deepStrictEqual(
					TLE.Token.createLeftSquareBracket(2),
					pr.leftSquareBracketToken
				);
				assert.deepStrictEqual(
					new TLE.FunctionCallValue(
				...
				assert.deepStrictEqual(
					TLE.Token.createRightSquareBracket(19),
					pr.rightSquareBracketToken
				);
				assert.deepStrictEqual([], pr.errors);
			});
			...
			test("with function with one number argument", () => {
				...
				assert.deepStrictEqual(
					TLE.Token.createRightSquareBracket(12),
					pr.rightSquareBracketToken
				);
				assert.deepStrictEqual([], pr.errors);

				const concat: TLE.FunctionCallValue = nonNullValue(
					TLE.asFunctionCallValue(pr.expression)
				);
				...
				assert.deepStrictEqual(
					concat.leftParenthesisToken,
					TLE.Token.createLeftParenthesis(8)
				);
				assert.deepStrictEqual(
					concat.rightParenthesisToken,
				...
				const arg1: TLE.NumberValue = nonNullValue(
					TLE.asNumberValue(concat.argumentExpressions[0])
				);
				assert.deepStrictEqual(arg1.parent, concat);
				assert.deepStrictEqual(
					arg1.token,
					TLE.Token.createNumber(9, "12")
				);
			});
			...
			test("with user namespace and function with one number argument", () => {
				...
				assert.deepStrictEqual(
					concat.namespaceToken,
					TLE.Token.createLiteral(2, "con")
				);
				assert.deepStrictEqual(
					concat.nameToken,
					TLE.Token.createLiteral(6, "at")
				);
				assert.deepStrictEqual(
					concat.leftParenthesisToken,
					TLE.Token.createLeftParenthesis(8)
			...
			test("with function with one string argument", () => {
				const pr: TLE.TleParseResult = parseExpression(
					"\"[concat('test')]\""
				);
				assert(pr);
				assert.deepStrictEqual(
				...
				assert.deepStrictEqual(
					pr.rightSquareBracketToken,
					TLE.Token.createRightSquareBracket(16)
				);
				assert.deepStrictEqual(pr.errors, []);

				const concat: TLE.FunctionCallValue | undefined =
					TLE.asFunctionCallValue(pr.expression);
				if (!concat) {
				...
				assert.deepStrictEqual(concat.argumentExpressions.length, 1);
				const arg1: TLE.StringValue | undefined = TLE.asStringValue(
					concat.argumentExpressions[0]
				);
				assert.deepStrictEqual(arg1!.parent, concat);
				assert.deepStrictEqual(
					arg1!.token,
					TLE.Token.createQuotedString(9, "'test'")
				);
			});
			...
			test("with function with one string argument with square brackets", () => {
				...
				assert.deepStrictEqual(
					pr.rightSquareBracketToken,
					TLE.Token.createRightSquareBracket(18)
				);
				assert.deepStrictEqual(pr.errors, []);
				...
				assert.deepStrictEqual(
					concat.rightParenthesisToken,
					TLE.Token.createRightParenthesis(17)
				);
				assert.deepStrictEqual(concat.argumentExpressions.length, 1);
				const arg1: TLE.StringValue | undefined = TLE.asStringValue(
					concat.argumentExpressions[0]
			...
			test("with function with one string argument and a comma", () => {
				...
				assert.deepStrictEqual(
					pr.rightSquareBracketToken,
					TLE.Token.createRightSquareBracket(17)
				);
				assert.deepStrictEqual(pr.errors, [
					new Issue(
						new Span(16, 1),
				...
				const arg2: TLE.Value | undefined =
					concat.argumentExpressions[1];
			...
			test("with function with missing first argument and string second argument", () => {
				...
				assert.deepStrictEqual(concat.argumentExpressions.length, 2);
				const arg1: TLE.Value | undefined =
					concat.argumentExpressions[0];
				assert.deepStrictEqual(arg1, undefined);
				const arg2: TLE.StringValue | undefined = TLE.asStringValue(
					concat.argumentExpressions[1]
				);
				if (!arg2) {
					throw new Error("failed");
				...
				assert.deepStrictEqual(
					arg2.token,
					TLE.Token.createQuotedString(10, "'test'")
				);
			});
			...
			test("with function with one missing argument and no right parenthesis", () => {
				...
				assert.deepStrictEqual(
					arg1.token,
					TLE.Token.createQuotedString(9, "'a1'")
				);
				const arg2: TLE.Value | undefined =
					concat.argumentExpressions[1];
				assert.deepStrictEqual(arg2, undefined);
			...
			test("with function with two arguments", () => {
				...
				assert.deepStrictEqual(
					arg1.token,
					TLE.Token.createQuotedString(9, "'a'")
				);
				const arg2: TLE.StringValue | undefined = TLE.asStringValue(
					concat.argumentExpressions[1]
				);
				...
				assert.deepStrictEqual(
					arg2.token,
					TLE.Token.createQuotedString(14, "'b'")
				);
			});
			...
			test("with function with three arguments", () => {
				...
				assert.deepStrictEqual(
					pr.rightSquareBracketToken,
					TLE.Token.createRightSquareBracket(21)
				);
				assert.deepStrictEqual(pr.errors, []);
				...
				assert.deepStrictEqual(
					concat.rightParenthesisToken,
					TLE.Token.createRightParenthesis(20)
				);
				assert.deepStrictEqual(concat.commaTokens, [
					TLE.Token.createComma(12),
				...
				const arg3: TLE.NumberValue | undefined = TLE.asNumberValue(
					concat.argumentExpressions[2]
				);
				if (!arg3) {
					throw new Error("failed");
				}
			...
			test("with function with function argument", () => {
				...
				assert.deepStrictEqual(
					pr.rightSquareBracketToken,
					TLE.Token.createRightSquareBracket(27)
				);
				assert.deepStrictEqual(pr.errors, []);
				...
				assert.deepStrictEqual(
					concat.rightParenthesisToken,
					TLE.Token.createRightParenthesis(26)
				);
				assert.deepStrictEqual(concat.commaTokens, [
					TLE.Token.createComma(12),
				...
				assert.deepStrictEqual(concat.argumentExpressions.length, 3);
				const concatArg1: TLE.StringValue | undefined =
					TLE.asStringValue(concat.argumentExpressions[0]);
				if (!concatArg1) {
					throw new Error("failed");
				}
				...
				const concatArg2: TLE.FunctionCallValue | undefined =
					TLE.asFunctionCallValue(concat.argumentExpressions[1]);
				if (!concatArg2) {
					throw new Error("failed");
				}
				assert.deepStrictEqual(concatArg2.parent, concat);
				assert.deepStrictEqual(
					concatArg2.nameToken,
					TLE.Token.createLiteral(14, "add")
				);
				assert.deepStrictEqual(
					concatArg2.leftParenthesisToken,
					TLE.Token.createLeftParenthesis(17)
				);
				assert.deepStrictEqual(
					concatArg2.rightParenthesisToken,
					TLE.Token.createRightParenthesis(22)
				);
				assert.deepStrictEqual(concatArg2.commaTokens, [
					TLE.Token.createComma(19),
				]);
				assert.deepStrictEqual(
					concatArg2.argumentExpressions.length,
					2
				);
				const addArg1: TLE.NumberValue | undefined = TLE.asNumberValue(
					concatArg2.argumentExpressions[0]
				);
				if (!addArg1) {
					throw new Error("failed");
				}
				...
				assert.deepStrictEqual(
					addArg1!.token,
					TLE.Token.createNumber(18, "5")
				);
				const addArg2: TLE.NumberValue | undefined = TLE.asNumberValue(
					concatArg2.argumentExpressions[1]
				);
				assert(addArg2);
				assert.deepStrictEqual(addArg2!.parent, concatArg2);
				assert.deepStrictEqual(
					addArg2!.token,
					TLE.Token.createNumber(21, "7")
				);
				const concatArg3: TLE.NumberValue | undefined =
					TLE.asNumberValue(concat.argumentExpressions[2]);
				assert(concatArg3);
				assert.deepStrictEqual(concatArg3!.parent, concat);
				assert.deepStrictEqual(
					concatArg3!.token,
					TLE.Token.createNumber(25, "3")
				);
			});

			test("with function with single single-quote argument", () => {
				const pr: TLE.TleParseResult =
					parseExpression('"[concat(\')]"');
				assert(pr);
				assert.deepStrictEqual(
					pr.leftSquareBracketToken,
					TLE.Token.createLeftSquareBracket(1)
				...
				assert.deepStrictEqual(
					arg1!.token,
					TLE.Token.createQuotedString(9, "')]")
				);
			});
			...
			test("with function with missing comma between two arguments", () => {
				...
				assert.deepStrictEqual(
					concat!.rightParenthesisToken,
					TLE.Token.createRightParenthesis(18)
				);
				assert.deepStrictEqual(concat!.commaTokens, []);
				assert.deepStrictEqual(concat!.argumentExpressions.length, 1);
				const arg1: TLE.StringValue | undefined = TLE.asStringValue(
			...
			test("with function with missing comma between three arguments", () => {
				...
				assert.deepStrictEqual(
					pr.rightSquareBracketToken,
					TLE.Token.createRightSquareBracket(26)
				);
				assert.deepStrictEqual(pr.errors, [
					new Issue(
						new Span(16, 2),
				...
				assert.deepStrictEqual(
					concat.rightParenthesisToken,
					TLE.Token.createRightParenthesis(25)
				);
				assert.deepStrictEqual(concat.commaTokens, []);
				assert.deepStrictEqual(concat.argumentExpressions.length, 1);
				const arg1: TLE.StringValue | undefined = TLE.asStringValue(
			...
			test("with property access", () => {
				...
				assert.deepStrictEqual(
					pr.rightSquareBracketToken,
					TLE.Token.createRightSquareBracket(22)
				);
				assert.deepStrictEqual(pr.errors, []);

				const propertyAccess: TLE.PropertyAccess | undefined =
					TLE.asPropertyAccessValue(pr.expression);
				if (!propertyAccess) {
					throw new Error("failed");
				}
				...
				assert.deepStrictEqual(
					...
					TLE.Token.createLiteral(18, "name")
				);
				assert.deepStrictEqual(
					propertyAccess!.periodToken,
					TLE.Token.createPeriod(17)
				);
				const resourceGroup: TLE.FunctionCallValue | undefined =
					TLE.asFunctionCallValue(propertyAccess.source);
				if (!resourceGroup) {
					throw new Error("failed");
				}
				assert.deepStrictEqual(resourceGroup.parent, propertyAccess);
				assert.deepStrictEqual(
					resourceGroup.nameToken,
					TLE.Token.createLiteral(2, "resourceGroup")
				);
				assert.deepStrictEqual(
					resourceGroup.leftParenthesisToken,
					TLE.Token.createLeftParenthesis(15)
				);
				assert.deepStrictEqual(
					resourceGroup.rightParenthesisToken,
					TLE.Token.createRightParenthesis(16)
				);
			...
			test("with quoted string instead of literal for property access", () => {
				...
				assert.deepStrictEqual(
					pr.rightSquareBracketToken,
					TLE.Token.createRightSquareBracket(24)
				);
				assert.deepStrictEqual(pr.errors, [
					new Issue(
						new Span(18, 6),
			...
			test("with a two-deep property access", () => {
				...
				assert.deepStrictEqual(
					pr.rightSquareBracketToken,
					TLE.Token.createRightSquareBracket(29)
				);
				assert.deepStrictEqual([], pr.errors);

				const length: TLE.PropertyAccess | undefined =
					TLE.asPropertyAccessValue(pr.expression);
				if (!length) {
					throw new Error("failed");
				}
				...
				assert.deepStrictEqual(
					length.nameToken,
					TLE.Token.createLiteral(23, "length")
				);
				assert.deepStrictEqual(
					length.periodToken,
					TLE.Token.createPeriod(22)
				);
				const name: TLE.PropertyAccess | undefined =
					TLE.asPropertyAccessValue(length.source);
				if (!name) {
					throw new Error("failed");
				}
				...
				const resourceGroup: TLE.FunctionCallValue | undefined =
					TLE.asFunctionCallValue(name.source);
				if (!resourceGroup) {
					throw new Error("failed");
				}
			...
			test("with array access", () => {
				...
				assert.deepStrictEqual(
					pr.rightSquareBracketToken,
					TLE.Token.createRightSquareBracket(20)
				);
				assert.deepStrictEqual(pr.errors, []);

				const arrayAccess: TLE.ArrayAccessValue | undefined =
					TLE.asArrayAccessValue(pr.expression);
				if (!arrayAccess) {
					throw new Error("failed");
				}
				assert.deepStrictEqual(
				...
				const index: TLE.NumberValue | undefined = TLE.asNumberValue(
					arrayAccess.indexValue
				);
				assert(index);
				assert.deepStrictEqual(index!.parent, arrayAccess);
				assert.deepStrictEqual(
					index!.token,
					TLE.Token.createNumber(17, "15")
				);
				assert.deepStrictEqual(
					arrayAccess.leftSquareBracketToken,
					TLE.Token.createLeftSquareBracket(16)
				);
				const variables: TLE.FunctionCallValue | undefined =
					TLE.asFunctionCallValue(arrayAccess.source);
				if (!variables) {
					throw new Error("failed");
				}
				...
				assert.deepStrictEqual(
					variables.nameToken,
					TLE.Token.createLiteral(2, "variables")
				);
				assert.deepStrictEqual(
					variables.leftParenthesisToken,
					TLE.Token.createLeftParenthesis(11)
				);
				assert.deepStrictEqual(
					variables.rightParenthesisToken,
					TLE.Token.createRightParenthesis(15)
				);
				assert.deepStrictEqual(variables.commaTokens, []);
				assert.deepStrictEqual(variables.argumentExpressions.length, 1);
				const arg1: TLE.StringValue | undefined = TLE.asStringValue(
					variables.argumentExpressions[0]
				);
				assert.deepStrictEqual(arg1!.parent, variables);
				assert.deepStrictEqual(
					arg1!.token,
					TLE.Token.createQuotedString(12, "'a'")
				);
			});
			...
			test("with two array accesses", () => {
				...
				assert.deepStrictEqual(
					pr.rightSquareBracketToken,
					TLE.Token.createRightSquareBracket(28)
				);
				assert.deepStrictEqual(pr.errors, []);

				const arrayAccess1: TLE.ArrayAccessValue | undefined =
					TLE.asArrayAccessValue(pr.expression);
				if (!arrayAccess1) {
					throw new Error("failed");
				}
				assert.deepStrictEqual(
					arrayAccess1.rightSquareBracketToken,
					TLE.Token.createRightSquareBracket(27)
				);
				assert.deepStrictEqual(
					arrayAccess1.leftSquareBracketToken,
					TLE.Token.createLeftSquareBracket(20)
				);
				const fido: TLE.StringValue | undefined = TLE.asStringValue(
					arrayAccess1.indexValue
				);
				if (!fido) {
					throw new Error("failed");
				}
				...
				assert.deepStrictEqual(
					fido.token,
					TLE.Token.createQuotedString(21, "'fido'")
				);
				const arrayAccess2: TLE.ArrayAccessValue | undefined =
					TLE.asArrayAccessValue(arrayAccess1.source);
				if (!arrayAccess2) {
					throw new Error("failed");
				}
				assert.deepStrictEqual(arrayAccess2.parent, arrayAccess1);
				assert.deepStrictEqual(
					arrayAccess2.rightSquareBracketToken,
					TLE.Token.createRightSquareBracket(19)
				);
				assert.deepStrictEqual(
					arrayAccess2.leftSquareBracketToken,
					TLE.Token.createLeftSquareBracket(16)
				);
				const fifteen: TLE.NumberValue | undefined = TLE.asNumberValue(
					arrayAccess2.indexValue
				);
				if (!fifteen) {
					throw new Error("failed");
				}
				...
				const variables: TLE.FunctionCallValue | undefined =
					TLE.asFunctionCallValue(arrayAccess2.source);
				if (!variables) {
					throw new Error("failed");
				}
				...
				assert.deepStrictEqual(variables.commaTokens, []);
				assert.deepStrictEqual(variables.argumentExpressions.length, 1);
				const a: TLE.StringValue | undefined = TLE.asStringValue(
					variables.argumentExpressions[0]
				);
				if (!a) {
					throw new Error("failed");
			...
			test("with array access with function index", () => {
				...
				const add: TLE.FunctionCallValue | undefined =
					TLE.asFunctionCallValue(arrayAccess.indexValue);
				if (!add) {
					throw new Error("failed");
				}
				...
				assert.deepStrictEqual(
					add.nameToken,
					TLE.Token.createLiteral(17, "add")
				);
				assert.deepStrictEqual(
					add.leftParenthesisToken,
					TLE.Token.createLeftParenthesis(20)
				);
				assert.deepStrictEqual(
					add.rightParenthesisToken,
					TLE.Token.createRightParenthesis(25)
				);
				assert.deepStrictEqual(add.commaTokens, [
					TLE.Token.createComma(23),
				]);
				assert.deepStrictEqual(add.argumentExpressions.length, 2);
				const addArg1: TLE.NumberValue | undefined = TLE.asNumberValue(
					add.argumentExpressions[0]
				);
				assert.deepStrictEqual(addArg1!.parent, add);
				assert.deepStrictEqual(
					addArg1!.token,
					TLE.Token.createNumber(21, "12")
				);
				const addArg2: TLE.NumberValue | undefined = TLE.asNumberValue(
					add.argumentExpressions[1]
				);
				assert.deepStrictEqual(addArg2!.parent, add);
				assert.deepStrictEqual(
					addArg2!.token,
					TLE.Token.createNumber(24, "3")
				);
				const variables: TLE.FunctionCallValue | undefined =
					TLE.asFunctionCallValue(arrayAccess.source);
				if (!variables) {
			...
			test("with string followed by literal", () => {
				...
				assert.deepStrictEqual(
					TLE.Token.createRightSquareBracket(14),
					pr.rightSquareBracketToken
				);
				assert.deepStrictEqual(
					[
			...
			test(`with "[concat(parameters('_artifactsLocation'), '/', '/scripts/azuremysql.sh', parameters('_artifactsLocationSasToken'))], )]"`, () => {
				const pr = parseExpression(
					`"[concat(parameters('_artifactsLocation'), '/', '/scripts/azuremysql.sh', parameters('_artifactsLocationSasToken'))], )]"`
				);
				assert.deepStrictEqual(pr.errors, [
					new Issue(
						new Span(116, 1),
	...
	suite("Tokenizer", () => {
		suite("readToken()", () => {
			test("with undefined stringValue", () => {
				// tslint:disable-next-line:no-any
				assert.throws(() => {
					TLE.Tokenizer.fromString(<any>undefined);
				});
			});

			test("with '' stringValue", () => {
				assert.throws(() => {
					TLE.Tokenizer.fromString("");
				});
			});
			...
			test("with empty TLE object with whitespace inside", () => {
				...
				assert.deepStrictEqual(
					TLE.Token.createWhitespace(2, " "),
					tt.readToken()
				);
				assert.deepStrictEqual(
					TLE.Token.createRightSquareBracket(3),
					tt.readToken()
				);
				assert.equal(undefined, tt.readToken());
			});
			...
			test("with escaped single-quoted empty string", () => {
				let tt = TLE.Tokenizer.fromString("\"''\"");
				assert.deepStrictEqual(
					TLE.Token.createQuotedString(1, "''"),
					tt.readToken()
				);
				assert.equal(undefined, tt.readToken());
			});
			...
			test("with unterminated double-quoted string", () => {
				...
				assert.deepStrictEqual(
					TLE.Token.createQuotedString(1, '"hello'),
					tt.readToken()
				);
				assert.equal(undefined, tt.readToken());
			});
			...
			test("with double-quoted string with escaped back-slash inside", () => {
				...
				assert.deepStrictEqual(
					TLE.Token.createQuotedString(1, "'C:\\\\Users\\\\'"),
					tt.readToken()
				);
				assert.equal(undefined, tt.readToken());
			});
			...
			test("with double-quoted string with escaped double-quote inside", () => {
				...
				assert.deepStrictEqual(
					TLE.Token.createQuotedString(1, '"hello\\"there"'),
					tt.readToken()
				);
				assert.equal(undefined, tt.readToken());
			});
			...
			test("with expression with single constant string", () => {
				...
				assert.deepStrictEqual(
					TLE.Token.createQuotedString(3, "'apples'"),
					tt.readToken()
				);
				assert.deepStrictEqual(
					TLE.Token.createRightSquareBracket(11),
					tt.readToken()
				);
				assert.equal(undefined, tt.readToken());
			});
			...
			test("with true", () => {
				...
				assert.deepStrictEqual(
					TLE.Token.createLiteral(1, "true"),
					tt.readToken()
				);
				assert.equal(undefined, tt.readToken());
			});
			...
			test("with value that starts with '.'", () => {
				...
				assert.deepStrictEqual(
					TLE.Token.createLiteral(3, "hello"),
					tt.readToken()
				);
				assert.deepStrictEqual(
					TLE.Token.createWhitespace(8, " "),
					tt.readToken()
				);
				assert.deepStrictEqual(
					TLE.Token.createLiteral(9, "there"),
					tt.readToken()
				);
				assert.equal(undefined, tt.readToken());
			});
			...
			test("with literal that ends with a number", () => {
				...
				assert.deepStrictEqual(
					TLE.Token.createLiteral(1, "base64"),
					tt.readToken()
				);
				assert.equal(undefined, tt.readToken());
			});
			...
			test("with several invalid literals", () => {
				...
				assert.deepStrictEqual(
					TLE.Token.createNumber(4, "82348923"),
					tt.readToken()
				);
				assert.deepStrictEqual(
					TLE.Token.createLiteral(12, "asdglih"),
					tt.readToken()
				);
				assert.deepStrictEqual(
					TLE.Token.createWhitespace(19, "   "),
					tt.readToken()
				);
				assert.deepStrictEqual(
					TLE.Token.createLiteral(22, "asl"),
					tt.readToken()
				);
				assert.deepStrictEqual(
					TLE.Token.createWhitespace(25, " "),
					tt.readToken()
				);
				assert.deepStrictEqual(
					TLE.Token.createPeriod(26),
					tt.readToken()
				);
				assert.deepStrictEqual(
					TLE.Token.createComma(27),
					tt.readToken()
				);
				assert.deepStrictEqual(
					TLE.Token.createQuotedString(28, "'"),
					tt.readToken()
				);
				assert.equal(undefined, tt.readToken());
			});
			...
			test("with user-defined function TLE with no arguments", () => {
				...
				assert.deepStrictEqual(
					TLE.Token.createLiteral(2, "co"),
					tt.readToken()
				);
				assert.deepStrictEqual(
					TLE.Token.createPeriod(4),
					tt.readToken()
				);
				assert.deepStrictEqual(
					TLE.Token.createLiteral(5, "cat"),
					tt.readToken()
				);
				assert.deepStrictEqual(
					TLE.Token.createLeftParenthesis(8),
			...
			test("with function TLE with one argument", () => {
				...
				assert.deepStrictEqual(
					TLE.Token.createQuotedString(9, "'Seattle'"),
					tt.readToken()
				);
				assert.deepStrictEqual(
					TLE.Token.createRightParenthesis(18),
			...
			test("with function TLE with two arguments", () => {
				...
				assert.deepStrictEqual(
					TLE.Token.createQuotedString(20, "'WA'"),
					tt.readToken()
				);
				assert.deepStrictEqual(
					TLE.Token.createRightParenthesis(24),
					tt.readToken()
				);
				assert.deepStrictEqual(
					TLE.Token.createRightSquareBracket(25),
					tt.readToken()
				);
				assert.equal(undefined, tt.readToken());
			});
			...
			suite("Quoted TLE strings", () => {
				test("simple string", () => {
					...
					assert.deepStrictEqual(
						tt.readToken(),
						TLE.Token.createQuotedString(2, "'Seattle'")
					);
					assert.deepStrictEqual(
						tt.readToken(),
						TLE.Token.createRightSquareBracket(11)
				...
				test("empty string", () => {
					...
					assert.deepStrictEqual(
						tt.readToken(),
						TLE.Token.createQuotedString(2, "''")
					);
					assert.deepStrictEqual(
						tt.readToken(),
						TLE.Token.createRightSquareBracket(4)
				...
				test("with single escaped apostrophe", () => {
					...
					assert.deepStrictEqual(
						tt.readToken(),
						TLE.Token.createQuotedString(2, "'That''s all, folks!'")
					);
					assert.deepStrictEqual(
						tt.readToken(),
						TLE.Token.createRightSquareBracket(23)
					);
					assert.equal(tt.readToken(), undefined);
				});
			...
			test("with function TLE with multiple arguments", () => {
				...
				assert.deepStrictEqual(
					TLE.Token.createQuotedString(26, "'USA'"),
					tt.readToken()
				);
				assert.deepStrictEqual(
					TLE.Token.createRightParenthesis(31),
					tt.readToken()
				);
				assert.deepStrictEqual(
					TLE.Token.createRightSquareBracket(32),
					tt.readToken()
				);
				assert.equal(undefined, tt.readToken());
			});
	...
	suite("UnrecognizedFunctionVisitor", () => {
		suite("visit(tle.Value)", () => {
			const functionMetadata: FunctionsMetadata = new FunctionsMetadata([
				new BuiltinFunctionMetadata(
					"CONCAT",
					"",
					"",
					0,
			...
			test("with unrecognized function", () => {
				...
				assert.strictEqual(
					errors[0].message,
					"Unrecognized function name 'concatenate'."
				);
			});
		});
	});
	...
	suite("IncorrectFunctionArgumentCountVisitor", () => {
		suite("visit(tle.Value, FunctionsMetadata)", () => {
			...
			test("with number value", () => {
				const errors = getReferenceErrors(
					emptyScope,
					new TLE.NumberValue(TLE.Token.createNumber(17, "3"))
				);
				assert.deepStrictEqual(errors, []);
			});

			test("with concat() with zero arguments", () => {
				const concat: TLE.Value = nonNullValue(
					parseExpression(`"[concat()]"`).expression
				);
			...
			test("with concat() with one argument", () => {
				const concat: TLE.Value = nonNullValue(
					parseExpression(`"[concat(12)]"`).expression
				);
				const errors = getReferenceErrors(emptyScope, concat);
				assert.deepStrictEqual(errors, []);
			});
			...
			test("with concat() with two arguments", () => {
				const concat: TLE.Value = nonNullValue(
					parseExpression(`"[concat(12, 'test')]"`).expression
				);
				const errors = getReferenceErrors(emptyScope, concat);
				assert.deepStrictEqual(errors, []);
			});
			...
			test("with add() with zero arguments", () => {
				const concat: TLE.Value = nonNullValue(
					parseExpression(`"[add()]"`).expression
				);
				const errors = getReferenceErrors(emptyScope, concat);
				assert.deepStrictEqual(errors, [
					new IncorrectArgumentsCountIssue(
						new Span(2, 5),
			...
			test("with add() with one argument", () => {
				const concat: TLE.Value = nonNullValue(
					parseExpression(`"[add(5)]"`).expression
				);
				const errors = getReferenceErrors(emptyScope, concat);
				assert.deepStrictEqual(errors, [
					new IncorrectArgumentsCountIssue(
			...
			test("with add() with two arguments", () => {
				const concat: TLE.Value = nonNullValue(
					parseExpression(`"[add(5, 6)]"`).expression
				);
				const errors = getReferenceErrors(emptyScope, concat);
				assert.deepStrictEqual(errors, []);
			});
			...
			test("with add() with three arguments", () => {
				const concat: TLE.Value = nonNullValue(
					parseExpression(`"[add(5, 6, 7)]"`).expression
				);
				const errors = getReferenceErrors(emptyScope, concat);
				assert.deepStrictEqual(errors, [
					new IncorrectArgumentsCountIssue(
			...
			test("with resourceId() with zero arguments", () => {
				const concat: TLE.Value = nonNullValue(
					parseExpression(`"[resourceId()]"`).expression
				);
				const errors = getReferenceErrors(emptyScope, concat);
				assert.deepStrictEqual(
					errors,
					...
					[
						new IncorrectArgumentsCountIssue(
							new Span(2, 12),
							"The function 'resourceId' takes at least 2 arguments.",
							"resourceId",
							0,
							2,
							<any>undefined
			...
			test("with resourceId() with one argument", () => {
				const concat: TLE.Value = nonNullValue(
					parseExpression(`"[resourceId(5)]"`).expression
				);
				const errors = getReferenceErrors(emptyScope, concat);
				assert.deepStrictEqual(
					errors,
			...
			test("with resourceId() with two arguments", () => {
				const concat: TLE.Value = nonNullValue(
					parseExpression(`"[resourceId(5, 6)]"`).expression
				);
				const errors = getReferenceErrors(emptyScope, concat);
				assert.deepStrictEqual(errors, []);
			});
			...
			test("with resourceId() with three arguments", () => {
				const concat: TLE.Value = nonNullValue(
					parseExpression(`"[resourceId(5, 6, 7)]"`).expression
				);
				const errors = getReferenceErrors(emptyScope, concat);
				assert.deepStrictEqual(errors, []);
			});
			...
			test("with substring() with zero arguments", () => {
				const concat: TLE.Value = nonNullValue(
					parseExpression(`"[substring()]"`).expression
				);
				const errors = getReferenceErrors(emptyScope, concat);
				assert.deepStrictEqual(errors, [
					new IncorrectArgumentsCountIssue(
						new Span(2, 11),
						"The function 'substring' takes between 1 and 3 arguments.",
						"substring",
						0,
						1,
						3
			...
			test("with substring() with one argument", () => {
				const concat: TLE.Value = nonNullValue(
					parseExpression(`"[substring('abc')]"`).expression
				);
				const errors = getReferenceErrors(emptyScope, concat);
				assert.deepStrictEqual(errors, []);
			});
			...
			test("with substring() with two arguments", () => {
				const concat: TLE.Value = nonNullValue(
					parseExpression(`"[substring('abc', 1)]"`).expression
				);
				const errors = getReferenceErrors(emptyScope, concat);
				assert.deepStrictEqual(errors, []);
			});
			...
			test("with substring() with three arguments", () => {
				const concat: TLE.Value = nonNullValue(
					parseExpression(`"[substring('abc', 1, 1)]"`).expression
				);
				const errors = getReferenceErrors(emptyScope, concat);
				assert.deepStrictEqual(errors, []);
			});
			...
			test("with substring() with four arguments", () => {
				const concat: TLE.Value = nonNullValue(
					parseExpression(`"[substring('abc', 1, 1, 'blah')]"`)
						.expression
				);
				const errors = getReferenceErrors(emptyScope, concat);
				assert.deepStrictEqual(errors, [
			...
			test("user function with name matching a built-in, with zero arguments", () => {
				const concat: TLE.Value = nonNullValue(
					parseExpression(`"[Contoso.resourceId()]"`).expression
				);
				const errors = getReferenceErrors(emptyScope, concat).map(
					(e) => e.message
				);
				assert.deepStrictEqual(errors, [
		...
		suite("visitPropertyAccess(TLE.PropertyAccess)", () => {
			test("with child property access from undefined variable reference", () => {
				const dt = new DeploymentTemplateDoc(
					`{ "a": "[variables('v1').apples]" }`,
					fakeId,
					0
				);
				const context: TemplatePositionContext =
					dt.getContextFromDocumentCharacterIndex(
						`{ "a": "[variables('v1').app`.length,
						undefined
					);
				const visitor = UndefinedVariablePropertyVisitor.visit(
					context.tleInfo!.tleValue,
					dt.topLevelScope
				);
				assert.deepStrictEqual(
					visitor.errors,
			...
			test("with grandchild property access from undefined variable reference", () => {
				const dt = new DeploymentTemplateDoc(
					`{ "a": "[variables('v1').apples.bananas]" }`,
					fakeId,
					0
				);
				const context: TemplatePositionContext =
					dt.getContextFromDocumentCharacterIndex(
						`{ "a": "[variables('v1').apples.ban`.length,
						undefined
					);
				const visitor = UndefinedVariablePropertyVisitor.visit(
					context.tleInfo!.tleValue,
			...
			test("with child property access from variable reference to non-object variable", () => {
				const dt = new DeploymentTemplateDoc(
					`{ "variables": { "v1": "blah" }, "a": "[variables('v1').apples]" }`,
					fakeId,
					0
				);
				const context: TemplatePositionContext =
					dt.getContextFromDocumentCharacterIndex(
						`{ "variables": { "v1": "blah" }, "a": "[variables('v1').app`
							.length,
						undefined
					);
				const visitor = UndefinedVariablePropertyVisitor.visit(
				...
				assert.deepStrictEqual(visitor.errors, [
					new Issue(
						new Span(18, 6),
						`Property "apples" is not a defined property of "variables('v1')".`,
			...
			test("with grandchild property access from variable reference to non-object variable", () => {
				const dt = new DeploymentTemplateDoc(
					`{ "variables": { "v1": "blah" }, "a": "[variables('v1').apples.bananas]" }`,
					fakeId,
					0
				);
				const context: TemplatePositionContext =
					dt.getContextFromDocumentCharacterIndex(
						`{ "variables": { "v1": "blah" }, "a": "[variables('v1').apples.ban`
							.length,
						undefined
					);
				const visitor = UndefinedVariablePropertyVisitor.visit(
	...
	suite("FindReferencesVisitor", () => {
		...
		suite("visit(tle.Value,string,string)", () => {
			test("with undefined TLE", async () => {
				...
				assert(param);
				const visitor = FindReferencesAndErrorsVisitor.visit(
					dt.topLevelScope,
					0,
					undefined,
					metadata,
			...
			test("with TLE", async () => {
				...
				assert.deepStrictEqual(
					...
					new ReferenceList(DefinitionKind.Parameter, [
						{ document: dt, span: new Span(14, 5) },
					])
				);
				assert.deepStrictEqual(issues, []);
			});
