...
suite("Variable iteration (copy blocks)", () => {
	suite("top-level variable copy blocks", () => {
		...
		test("copy block names are added as variables", async () => {
			...
			assert(
				dt.topLevelScope.variableDefinitions[0].nameValue
					.unquotedValue === "diskNames"
			);
		...
		test("copy block value is an array of the input property", async () => {
			...
			const value =
				dt.topLevelScope.getVariableDefinition("diskNames")!.value!;
			assert(value);
			assert(value instanceof Json.ArrayValue);
			assert(
				(<Json.ArrayValue>value).elements[0] instanceof Json.StringValue
			);
			// Right now the value consists of just a single array element of the 'input' property value
			assert.equal(
				(<Json.StringValue>(<Json.ArrayValue>value).elements[0])
					.unquotedValue,
				"[concat('myDataDisk', copyIndex('diskNames', 1))]"
			);
		});
		...
		test("copy block usage info", async () => {
			...
			const diskNames =
				dt.topLevelScope.getVariableDefinition("diskNames")!;
		...
		test("Find references at variable reference points to the copy block element's name property", async () => {
			const {
				dt,
				markers: { diskNamesDefName, diskNamesRef },
			} = parseTemplateWithMarkers(
				{
			...
			testGetReferences(dt, diskNamesRef.index, [
				diskNamesRef.index,
				diskNamesDefName.index,
			]);
		});
		...
		test("Regular and iteration vars together", async () => {
			...
			assert.deepStrictEqual(
				dt.topLevelScope.variableDefinitions.map(
					(v) => v.nameValue.unquotedValue
				),
		...
		suite("Top-level variable iteration completion", () => {
			const disksTopLevelArrayTemplate: IDeploymentTemplate = {
					...
					"copy": [
						{
							...
							"input": {
								...
								"diskIndex":
									"[copyIndex('disks-top-level-array-of-object')]",
								"data": {
									"diskSizeGB": "1",
								},
							},
				...
				"outputs": {
					"o1ArrayOfObject": {
						"value":
							"[variables('disks-top-level-array-of-object')]",
						"type": "array",
					},
					"o2Elem": {
						"value":
							"[variables('disks-top-level-array-of-object')[1]]",
						"type": "object",
					},
					"o2ElemDotData": {
						"value":
							"[variables('disks-top-level-array-of-object')[1].data]",
						"type": "object",
					},
					"o2ElemDotDataDotDiskSizeGB": {
						"value":
							"[variables('disks-top-level-array-of-object')[1].data.diskSizeGB]",
						"type": "int",
					},
					"output1": {
						"value": "<output1>",
			...
			createExpressionCompletionsTestEx(
				disksTopLevelArrayTemplate,
				"<output1>",
				"[variables('disks-top-level-array-of-object').<!cursor!>]",
				[]
			);
			createExpressionCompletionsTestEx(
				disksTopLevelArrayTemplate,
				"<output1>",
				"[variables('disks-top-level-array-of-object').data<!cursor!>]",
				[]
			);
		});
	...
	suite("embedded variable copy blocks", async () => {
		...
		test("'copy' not found as member of the variable's value", async () => {
			assert.equal(
				Json.asObjectValue(variable.value)!.getPropertyValue("copy"),
				undefined
			);
		});
		...
		test("copy block names are added as members of the variable", async () => {
			...
			const v = dt.topLevelScope.getVariableDefinition(
				"Disk-array-in-object"
			)!;
		...
		test("copy block value is an object with an array 'disks' of the input property's value", async () => {
			const valueObject = Json.asObjectValue(
				dt.topLevelScope.getVariableDefinition("disk-array-in-object")!
					.value
			)!;
			...
			const element1Object = Json.asObjectValue(
				disksArrayValue.elements[0]
			)!;
			assert(element1Object);
			assert.deepStrictEqual(element1Object.propertyNames, [
				"name",
				"diskSizeGB",
				"diskIndex",
			]);
		...
		test("embedded copy block usage info", async () => {
			const diskNames = dt.topLevelScope.getVariableDefinition(
				"disk-array-in-object"
			)!;
			assert.deepStrictEqual(diskNames.usageInfo, {
				description: undefined,
		...
		test("multiple copy members", async () => {
			...
			assert(valueObject);
			assert.deepStrictEqual(valueObject.propertyNames.sort(), [
				"array1",
				"array2",
				"member2",
			]);
		...
		test("case insensitive keys", () => {
			...
			assert.deepStrictEqual(
				Json.asObjectValue(
					dt2.topLevelScope.getVariableDefinition("object")!.value
				)!.propertyNames,
				["array1"]
			);
		});
		...
		test("no input property", () => {
			...
			const objectVar =
				dt2.topLevelScope.getVariableDefinition("object")!;
			assert(objectVar);
		});
		...
		test("deep COPY blocks", async () => {
			...
			const vTop: IVariableDefinition =
				dt2.topLevelScope.getVariableDefinition("top")!;
			const vTopValue = Json.asObjectValue(vTop.value)!;
			assert.deepStrictEqual(vTopValue.propertyNames.sort(), [
				"array1",
				"mid",
			]);

			const vMidValue = Json.asObjectValue(
				vTopValue.getPropertyValue("mid")
			)!;
			assert.deepStrictEqual(vMidValue.propertyNames.sort(), [
				"array2",
				"bottom",
			]);

			const vBottomValue = Json.asObjectValue(
				vMidValue.getPropertyValue("bottom")
			)!;
			assert.deepStrictEqual(vBottomValue.propertyNames.sort(), [
				"array3",
			]);
		...
		suite("Embedded variable iteration completion", () => {
						...
						"copy": [
							{
								...
								"count": 5,
								"input":
									"[concat('myDataDisk', copyIndex('disks', 1))]",
							},
							{
								"name": "array2",
								"count": 5,
			...
			createExpressionCompletionsTestEx(
				template,
				"<output1>",
				"[variables('disk-array-in-object').<!cursor!>]",
				["member1", "member2", "array1", "array2"]
			);
		});
					...
					"copy": [
						{
							...
							"input": {
								...
								"diskSizeGB": "1",
								"diskIndex":
									"[copyIndex('top-level-object-array')]",
							},
						},
						{
							...
							"count": 5,
							"input":
								"[concat('myDataDisk', copyIndex('top-level-string-array', 1))]",
						},
						{
