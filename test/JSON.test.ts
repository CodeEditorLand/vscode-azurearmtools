 ...
 */
export function parseUnrecognized(
	text: string,
	startIndex: number = 0
): Json.Token {
	const basicTokens: basic.Token[] = parseBasicTokens(text);
...
suite("JSON", () => {
	suite("ParseResult", () => {
		...
		suite("getCharacterIndex(number,number)", () => {
			...
			test("with a single line", () => {
				...
				assert.deepStrictEqual(
					"hello".length,
					pr.getCharacterIndex(0, "hello".length)
				);
				assert.deepStrictEqual(
					"hello there".length,
					pr.getCharacterIndex(0, "hello there".length)
				);
			});
			...
			test("with multiple lines", () => {
				...
				assert.throws(() => {
					pr.getCharacterIndex(4, 5);
				});

				assert.throws(() => {
					pr.getCharacterIndex(5, 0);
				});
			});
		});
		...
		suite("getTokenAtCharacterIndex(number)", () => {
			...
			test("with character index inside character index range", () => {
				...
				assert.deepStrictEqual(
					Json.QuotedString(2, parseBasicTokens("'hello'")),
					pr.getTokenAtCharacterIndex(2)
				);
				assert.deepStrictEqual(
					Json.Colon(9),
					pr.getTokenAtCharacterIndex(9)
				);
				assert.deepStrictEqual(
					undefined,
					pr.getTokenAtCharacterIndex(10)
				);
				assert.deepStrictEqual(
					parseNumber("42", 11),
					pr.getTokenAtCharacterIndex(11)
				);
				assert.deepStrictEqual(
					undefined,
					pr.getTokenAtCharacterIndex(13)
				);
				assert.deepStrictEqual(
					Json.RightCurlyBracket(14),
					pr.getTokenAtCharacterIndex(14)
				);
				assert.deepStrictEqual(
					Json.RightCurlyBracket(14),
					pr.getTokenAtCharacterIndex(15)
				);
				assert.deepStrictEqual(
					undefined,
					pr.getTokenAtCharacterIndex(16)
				);
			});
		});
	});
	...
	suite("parse(string)", () => {
		...
		test("with quoted string", () => {
			...
			assert.deepStrictEqual(
				result.value,
				new Json.StringValue(new Span(0, 13), "'hello there'")
			);
		});
		...
		test("with number", () => {
			...
			assert.deepStrictEqual(
				result.value,
				new Json.NumberValue(new Span(0, 2), "14")
			);
		});
		...
		test("with boolean (false)", () => {
			...
			assert.deepStrictEqual(
				result.value,
				new Json.BooleanValue(new Span(0, 5), false)
			);
		});
		...
		test("with boolean (true)", () => {
			...
			assert.deepStrictEqual(
				result.value,
				new Json.BooleanValue(new Span(0, 4), true)
			);
		});
		...
		test("with left curly bracket", () => {
			...
			assert.deepStrictEqual(
				result.value,
				new Json.ObjectValue(new Span(0, 1), [])
			);
		});
		...
		test("with empty object", () => {
			...
			assert.deepStrictEqual(
				result.value,
				new Json.ObjectValue(new Span(0, 2), [])
			);
		});
		...
		test("with object with one string property", () => {
			...
			const v1: Json.ObjectValue | undefined = Json.asObjectValue(
				result.value
			);
			if (!v1) {
			...
			const v2: Json.StringValue | undefined = Json.asStringValue(
				v1.getPropertyValue("name")
			);
			if (!v2) {
				throw new Error("failed");
			}
		...
		test("with object with one string property and one number property", () => {
			...
			const top: Json.ObjectValue | undefined = Json.asObjectValue(
				result.value
			);
			if (!top) {
				throw new Error("failed");
		...
		test("with object with one boolean property and one number property", () => {
			const result: Json.ParseResult = Json.parse(
				"{ 'a': true, 'b': 30 }"
			);
			assert.deepStrictEqual(9, result.tokenCount);
			...
			const a: Json.BooleanValue | undefined = Json.asBooleanValue(
				top.getPropertyValue("a")
			);
			if (!a) {
				throw new Error("failed");
		...
		test("with object with object property", () => {
			...
			const a: Json.ObjectValue | undefined = Json.asObjectValue(
				top.getPropertyValue("a")
			);
			if (!a) {
				throw new Error("failed");
			...
			const b: Json.StringValue | undefined = Json.asStringValue(
		...
		test("with object with empty array property", () => {
			...
			const a: Json.ArrayValue | undefined = Json.asArrayValue(
				top.getPropertyValue("a")
			);
			if (!a) {
				throw new Error("failed");
		...
		test("with object with 1 element array property", () => {
			...
			const a0: Json.StringValue | undefined = Json.asStringValue(
				a.elements[0]
			);
			if (!a0) {
				throw new Error("failed");
		...
		test("with object with 2 element array property", () => {
			...
			const a1: Json.NumberValue | undefined = Json.asNumberValue(
				a.elements[1]
			);
			if (!a1) {
				throw new Error("failed");
	...
	suite("Tokenizer", () => {
		suite("next()", () => {
			function nextTest(
				text: string,
				expectedTokens?: Json.Token | Json.Token[]
			): void {
				if (!expectedTokens) {
					expectedTokens = [];
				...
				test(`with ${strings.escapeAndQuote(text)}`, () => {
					...
					for (const expectedToken of expectedTokens as Json.Token[]) {
						...
						assert.deepStrictEqual(
							tokenizer.hasStarted(),
							true,
							"Expected hasStarted() to be true"
						);
						assert.deepStrictEqual(
							tokenizer.current,
							expectedToken
					...
					for (let i = 0; i < 2; ++i) {
						...
						assert.deepStrictEqual(
							tokenizer.hasStarted(),
							true,
							"Expected hasStarted() to be true (after all expected tokens)"
						);
						assert.deepStrictEqual(
							tokenizer.current,
							undefined,
							"Expected current to be undefined"
						);
					}
				});
			}
			...
			nextTest("/a", [parseLiteral("/", 0), parseLiteral("a", 1)]);
	...
	suite("LeftSquareBracket()", () => {
		function leftSquareBracketTest(startIndex: number): void {
			test(`with ${startIndex} startIndex`, () => {
				...
				assert.deepStrictEqual(
					t.type,
					Json.TokenType.LeftSquareBracket
				);
				assert.deepStrictEqual(t.span, new Span(startIndex, 1));
				assert.deepStrictEqual(t.toString(), "[");
			});
	...
	suite("RightSquareBracket()", () => {
		function rightSquareBracketTest(startIndex: number): void {
			test(`with ${startIndex} startIndex`, () => {
				...
				assert.deepStrictEqual(
					t.type,
					Json.TokenType.RightSquareBracket
				);
				assert.deepStrictEqual(t.span, new Span(startIndex, 1));
				assert.deepStrictEqual(t.toString(), "]");
			});
	...
	suite("getLastTokenOnLine", () => {
		function createGetLastTokenOnLineTest(
			...
			if (!expectedTokenTypesIgnoringComments) {
				expectedTokenTypesIgnoringComments =
					expectedTokenTypesIncludingComments;
			}

			createTest(
				`${testName}, including comments`,
				Json.Comments.includeCommentTokens,
				expectedTokenTypesIncludingComments
			);
			createTest(
				`${testName}, ignoring comments`,
			...
			function createTest(
				...
				expected: (Json.TokenType | undefined)[]
			): void {
				test(name, () => {
		...
		suite("simple and line comments", () => {
			...
			createGetLastTokenOnLineTest("single line", `hi`, [
            ...
            `,
				[Json.TokenType.Comment, Json.TokenType.Comment, undefined],
				[
					Json.TokenType.LeftCurlyBracket,
					Json.TokenType.RightCurlyBracket,
					undefined,
			...
			test(testName, () =>
				testStringAtEachIndex(
					...
					(text, index) => {
						...
						const token =
							parseResults.getCommentTokenAtDocumentIndex(
								index,
								containsBehavior
							);
						return !!token;
