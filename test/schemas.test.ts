...
suite("Schemas", () => {
	suite("getPreferredSchema", () => {
		function createTest(
			schema: string,
			expectedPreferred: string | undefined
		): void {
			// tslint:disable-next-line: strict-boolean-expressions
			test(schema || "(empty)", () => {
				const preferred: string | undefined =
					getPreferredSchema(schema);
		...
		createTest(
			"://schema.management.azure.com/schemas/2014-04-01-preview/deploymentTemplate.json#",
			undefined
		);
		createTest(
			"2://schema.management.azure.com/schemas/2014-04-01-preview/deploymentTemplate.json#",
			undefined
		);
		createTest(
			"https://schema.management.azure.com/schema/2014-04-01-preview/deploymentTemplate.json#",
			undefined
		); // misspelled /schemas/
		createTest(
			"https://schemas.management.azure.com/schemas/2014-04-01-preview/deploymentTemplate.json#",
			undefined
		); // misspelled /schema./management

		// Newer schemas shouldn't trip and therefore recommend an older schema
		createTest(
			"https://schema.management.azure.com/schemas/2020-01-01/deploymentTemplate.json#",
			undefined
		);
	...
	suite("findSchemaInfo", () => {
		function createFindSchemaInfoTest(
			schemaUri: string,
			expected: ISchemaInfo | undefined
		): void {
			test(schemaUri, () => {
				...
				if (expected) {
					...
					assert.strictEqual(
						info.normalizedSchema,
						expected.normalizedSchema,
						"normalizedSchema value incorrect"
					);
					assert.strictEqual(
						info.deploymentScopeKind,
						expected.deploymentScopeKind,
						"deploymentScopeKind value incorrect"
					);
					assert.strictEqual(
						info.isDeprecated,
						expected.isDeprecated,
						"isDeprecated value incorrect"
					);
				} else {
