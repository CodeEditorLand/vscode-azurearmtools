...
suite("ParameterFiles", () => {
	suite("mayBeMatchingParamFile", () => {
		function testIsLikelyMatch(
			expected: boolean,
			templateFileName: string,
			possibleParamFileName: string
		): void {
			test(`(${templateFileName}, ${possibleParamFileName})`, () => {
				const result = mayBeMatchingParameterFile(
					templateFileName,
					possibleParamFileName
				);
		...
		testIsLikelyMatch(
			true,
			"template.json",
			"template.parameters.dev.json"
		);

		testIsLikelyMatch(
			true,
			"template.json",
			"template.params.dev.whatever.json"
		);
		testIsLikelyMatch(
			true,
			"template.json",
		...
		testIsLikelyMatch(
			true,
			"TEMPlate.json",
			"template.params.dev.whatever.JSONc"
		);
		testIsLikelyMatch(
			true,
			"template.JSON",
			"Template.PARAMETERS.deV.JsonC"
		);

		testIsLikelyMatch(true, "new-vm.template.json", "new-vm.params.json");
		testIsLikelyMatch(
			true,
			"new-vm.template.json",
			"new-vm.parameters.json"
		);
