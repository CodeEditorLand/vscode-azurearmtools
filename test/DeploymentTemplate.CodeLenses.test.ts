...
suite("DeploymentTemplate code lenses", () => {
	class FakeParameterValuesSourceProvider
	...
	suite("parameters section code lens", () => {
		suite("if no parameter file then", () => {
			test("expect only a single parameters section code lens", async () => {
				...
				assert.equal(
					lenses.length,
					1,
					"Expecting only a code lens for the parameters section itself"
				);
			});
			...
			test("code lens should show command to select/create one", async () => {
				...
				assert.equal(
					stringify(lenses[0].range),
					stringify(
						new Range(new Position(3, 2), new Position(63, 3))
					)
				);
				assert.equal(
				...
				assert.equal(
					lenses[0].command?.arguments![0].toString(),
					dt.documentUri.toString()
				);
			});
		});
		...
		suite("if there is a parameter file then", () => {
			test("parameter section code lens should show command to open current parameter file and one to change the selection", async () => {
				...
				const lenses = dt.getCodeLenses(
					new FakeParameterValuesSourceProvider(
						dp.documentUri,
						dp.topLevelParameterValuesSource
					)
				);
				assert.equal(
					lenses.length,
					2 + dt.topLevelScope.parameterDefinitions.length
				);
				for (const lens of lenses) {
					const result = await lens.resolve();
					assert(result);
				...
				const openLens = lenses.filter(
					(l) => l instanceof ShowCurrentParameterFileCodeLens
				)[0];
				assert.equal(
					stringify(openLens.range),
					stringify(
						new Range(new Position(3, 2), new Position(63, 3))
					)
				);
				assert.equal(
					openLens.command?.title,
					`Parameter file: "test parameter file.json" $(error) Not found`
				);
				assert.equal(
					openLens.command?.command,
				...
				assert.equal(
					openLens.command?.arguments![0].toString(),
					dt.documentUri.toString()
				);

				const selectLens = lenses[1];
				assert.equal(
					stringify(selectLens.range),
					stringify(
						new Range(new Position(3, 2), new Position(63, 3))
					)
				);
	...
	suite("parameter definition code lenses", () => {
		suite(
			"with top-level parameter definitions and values in a parameter file",
			async () => {
				function createParamLensTest(
					topLevelParamName: string,
					valueInParamFile:
						| { value?: string; reference?: string }
						| undefined,
					...
					const testName =
						valueInParamFile === undefined
							...
							: `${topLevelParamName} with value ${JSON.stringify(
									valueInParamFile
							  ).replace(/\r\n|\n/g, " ")}`;
					test(testName, async () => {
						let a = testName;
						...
						const lenses = dt
							...
							.filter(
								(l) => l instanceof ParameterDefinitionCodeLens
							)
							.map((l) => <ParameterDefinitionCodeLens>l);
						assert.equal(
							lenses.length,
							dt.topLevelScope.parameterDefinitions.length
						);

						// Find the code lens for the parameter
						const lens = lenses.find(
							(l) => l.parameterDefinition === param
						);
						assert(
							!!lens,
							`Couldn't find a code lens for parameter ${param.nameValue.unquotedValue}`
						);
				...
				createParamLensTest(
					"requiredInt",
					undefined,
					"$(warning) No value found - click here to enter a value"
				);
		...
		suite("parameters for nested inner-scoped template", () => {
			function createCodeLensTest(
				...
				testWithLanguageServer(testName, async () => {
					...
					let lenses = dt
						.getCodeLenses(undefined)
						.filter((cl) => !(cl instanceof ParentOrChildCodeLens));
					for (const lens of lenses) {
						const result = await lens.resolve();
						assert(result);

						if (lens.command) {
							lens.command.title = lens.command.title.replace(
								" - loading schemas...",
								""
							);
						}
					...
					const actual: string[] = lenses.map((l) => {
						return `${l.scope.scopeKind}: "${l.command?.title}" (${l
							.command?.command}) at ${rangeToString(l.range)}`;
					});
			...
			createCodeLensTest(
				...
				[
					...
					'NestedDeploymentWithInnerScope: "$(warning) Full template validation off. Add parameter file or top-level parameter defaults to enable." (azurerm-vscode-tools.selectParameterFile) at [14,21-39,10]',
