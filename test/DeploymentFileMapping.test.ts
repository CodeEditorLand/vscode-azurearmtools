...
suite("DeploymentFileMapping", () => {
	...
	const param1 = Uri.file(
		isWin32
			? "c:\\temp\\template1.params.json"
			: "/temp/template1.params.json"
	);
	const param1variation = Uri.file(
		isWin32
			? "c:\\temp\\abc\\..\\template1.params.json"
			: "/temp/abc/../template1.params.json"
	);

	const param1subfolder = Uri.file(
		isWin32
			? "c:\\temp\\sub\\template1.params.json"
			: "/temp/sub/template1.params.json"
	);
	const param1parentfolder = Uri.file(
		isWin32 ? "c:\\template1.params.json" : "/template1.params.json"
	);

	const template1 = Uri.file(
		isWin32 ? "c:\\temp\\template1.json" : "/temp/template1.json"
	);
	const template1variation = Uri.file(
		isWin32
			? "c:\\temp\\abc\\..\\template1.json"
			: "/temp/abc/../template1.json"
	);
	...
	test("Update/get - paths normalized", async () => {
		...
		assert.equal(
			mapping.getParameterFile(t)?.fsPath,
			path.resolve(p.fsPath)
		);
		assert.equal(
			mapping.getTemplateFile(p)?.fsPath,
			path.resolve(t.fsPath)
		);
	});
	...
	test("Param paths are stored in settings relative to template folder", async () => {
		...
		const parameterFiles = <{ [key: string]: string }>(
			testConfig.get("parameterFiles")
		);
	...
	testOnWin32("Case-insensitive on Windows", async () => {
		...
		assert.equal(
			mapping.getParameterFile(Uri.file("c:\\temp\\tEMPLATE1.jSON"))
				?.fsPath,
			p.fsPath.toLocaleLowerCase()
		);
		assert.equal(
			mapping.getTemplateFile(Uri.file("c:\\temP\\TemPLATE1.pARAMS.jSOn"))
				?.fsPath,
			t.fsPath.toLowerCase()
		);
	});
	...
	testOnWin32("Case-insensitive on Windows - last entry wins", async () => {
		...
		assert(
			normalizedTemplate === normalizeFilePath(t2.fsPath) &&
				normalizedTemplate === normalizeFilePath(t3.fsPath)
		);
	...
	test("Bad settings 2", async () => {
		...
		assert.equal(
			mapping.getParameterFile(Uri.file("temp/relative/foo.json")),
			undefined
		);
		assert.equal(
			mapping.getParameterFile(Uri.file(`${root}t1.json`)),
			undefined
		);
		assert.equal(
			mapping.getParameterFile(Uri.file(`${root}t3.json`)),
			undefined
		);
		assert.equal(
			mapping.getParameterFile(Uri.file(`${root}t4.json`)),
			undefined
		);
		assert.equal(
			mapping.getParameterFile(Uri.file(`${root}good1.json`))?.fsPath,
			`${root}a.`
		);
		...
		assert.equal(
			mapping.getTemplateFile(Uri.file("foo.params.json")),
			undefined
		);
	});
	...
	test("Remove mapping", async () => {
		...
		assert.equal(
			mapping.getParameterFile(template1)?.fsPath,
			param1.fsPath
		);
