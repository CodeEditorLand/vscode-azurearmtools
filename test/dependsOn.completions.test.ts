...
suite("dependsOn completions", () => {
	type PartialCompletionItem = Partial<
		Completion.Item & { replaceSpanStart: number; replaceSpanText: string }
	>;
	...
	function createDependsOnCompletionsTest(
		...
		test(testName, async () => {
			const {
				dt,
				markers: { cursor, replaceStart },
			} = parseTemplateWithMarkers(options.template);
			assert(cursor, "Missing <!cursor!> in testcase template");
			const pc = dt.getContextFromDocumentCharacterIndex(
				cursor.index,
				undefined
			);
			const { items: completions } = await pc.getCompletionItems(
			...
			const vscodeItems = completions.map((item) =>
				toVsCodeCompletionItem(
					dt,
					item,
					getVSCodePositionFromPosition(pc.documentPosition)
				)
			);
			assert.strictEqual(vscodeItems.length, completions.length);

			const actual: PartialCompletionItem[] =
				completions.map(filterActual);

			assertEx.deepEqual(actual, options.expected, {
	...
	suite("completionKinds", () => {
		createDependsOnCompletionsTest("completionKinds", {
			template: {
				resources: [
					{
						dependsOn: ["<!replaceStart!><!cursor!>"],
					},
					{
	...
	suite("detail", () => {
		createDependsOnCompletionsTest("detail matches friendy type name", {
			template: {
				resources: [
					{
						dependsOn: ["<!replaceStart!><!cursor!>"],
	...
	suite("documentation", () => {
		createDependsOnCompletionsTest("documentation", {
			template: {
				resources: [
					{
						dependsOn: ["<!replaceStart!><!cursor!>"],
			...
			expected: [
				...
				},
				{
					// tslint:disable-next-line: no-any
					"documention": <any>{
						value: `Inserts this COPY element reference:
\`\`\`arm-template
"copyname"
\`\`\`
from resource \`name1a\` of type \`def\``,
	...
	suite("label", () => {
		...
		createDependsOnCompletionsTest("label for copy loop", {
			template: {
				resources: [
					{
						dependsOn: ["<!replaceStart!><!cursor!>"],
	...
	suite("expressions", () => {
		createDependsOnCompletionsTest("type is expression", {
			template: {
				resources: [
					{
						dependsOn: ["<!replaceStart!><!cursor!>"],
	...
	createDependsOnCompletionsTest("insertionText for copy loop", {
		template: {
			resources: [
				{
					dependsOn: ["<!replaceStart!><!cursor!>"],
	...
	createDependsOnCompletionsTest("name is expression", {
		template: {
			resources: [
				{
					dependsOn: ["<!replaceStart!><!cursor!>"],
	...
	suite("completion triggering", () => {
		createDependsOnCompletionsTest("ctrl+space without any quotes", {
			template: `{
                    "resources": [
                        {
                            "dependsOn": [
		...
		createDependsOnCompletionsTest("replacement includes entire string", {
			template: {
				resources: [
					{
						dependsOn: ["<!replaceStart!><!cursor!>name2a"],
					},
					{
						type: "microsoft.abc/def",
						name: "name1a",
		...
		createDependsOnCompletionsTest(
			"no dependsOn completions inside an expression (#1008/1033)",
			{
				template: {
					resources: [
						{
							dependsOn: ["[<!replaceStart!>name<!cursor!>2a"],
						},
						{
							type: "microsoft.abc/def",
							name: "name1a",
	...
	createDependsOnCompletionsTest("case insensitive dependsOn", {
		template: {
			resources: [
				{
					DEPENDSON: ["<!replaceStart!><!cursor!>name2a"],
	...
	createDependsOnCompletionsTest("multiple resources", {
		template: {
			resources: [
				{
					dependsOn: ["<!replaceStart!><!cursor!>"],
	...
	suite("Child resources", () => {
		suite("Decoupled parent/child", () => {
			...
			createDependsOnCompletionsTest("Reference parent from child", {
				template: {
					"resources": [
						{
							// Parent
		...
		suite("nested parent/child", () => {
			createDependsOnCompletionsTest("Two ways to name nested children", {
				template: {
					"resources": [
						{
							// sibling
	...
	suite("Regression", () => {
		createDependsOnCompletionsTest("#974 simple", {
			template: {
				"resources": [
					{
		...
		createDependsOnCompletionsTest("#974", {
			template: {
				"$schema":
				...
				"resources": [
					{
						...
						"properties": {
							...
							"settings": {
								"ModulesUrl":
									"[concat(parameters('dscLocation'),'DSC/DeployRDSLab.zip')]",
								"ConfigurationFunction":
									"[parameters('dscFunction')]",
								"Properties": {
									"AdminCreds": {
										"UserName": "[parameters('adminUser')]",
										"Password":
											"PrivateSettingsRef:AdminPassword",
									},
									"RDSParameters": [
										{
											"TimeZoneID":
											...
											"DomainName":
												"[parameters('adDomainName')]",
											"DNSServer":
												"[parameters('firstDcIP')]",
											"MainConnectionBroker":
												"[parameters('MainConnectionBroker')]",
											"WebAccessServer":
												"[concat(parameters('WebAccessServerName'),'1')]",
											"SessionHost":
												"[concat(parameters('SessionHostName'),'1')]",
											"LicenseServer":
												"[concat(parameters('LicenseServerName'),'1')]",
											"ExternalFqdn":
												"[parameters('ExternalFqdn')]",
											"ExternalDnsDomain":
												"[parameters('ExternalDnsDomain')]",
											"IntBrokerLBIP":
												"[parameters('intLbBrokerIP')]",
											"IntWebGWLBIP":
												"[parameters('intLbWebGWIP')]",
											"WebGWDNS":
												"[parameters('webGwName')]",
										},
									],
								},
							},
