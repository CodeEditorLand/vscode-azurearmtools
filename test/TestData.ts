...
import {
	ITestPreparation,
	ITestPreparationResult,
	testWithPrep,
} from "./support/testWithPrep";

// By default we use the test metadata for tests
export function useTestFunctionMetadata(): void {
	let testMetadata = fs.readFileSync(
		path.join(
			__dirname,
			"..",
			"..",
			"test",
			"TestData.ExpressionMetadata.json"
		)
	);
...
export class UseRealFunctionMetadata implements ITestPreparation {
	public static readonly instance: UseRealFunctionMetadata =
		new UseRealFunctionMetadata();
...
export async function runWithRealFunctionMetadata(
...
export function testWithRealFunctionMetadata(
	expectation: string,
...
export const allTestDataCompletionNames = new Set<string>(
	allTestDataExpectedCompletions(0, 0).map((item) => item.label)
);

export function allTestDataExpectedCompletions(
	startIndex: number,
	length: number
): Completion.Item[] {
	return [
...
export function expectedAddCompletion(
	startIndex: number,
	length: number
): Completion.Item {
	return newCompletionItem(
		"add",
		"add",
		new Span(startIndex, length),
		Completion.CompletionKind.tleFunction,
		"(function) add(operand1, operand2)",
		"Returns the sum of the two provided integers."
	);
}

export function expectedBase64Completion(
	startIndex: number,
	length: number
): Completion.Item {
	return newCompletionItem(
...
export function expectedConcatCompletion(
	startIndex: number,
	length: number
): Completion.Item {
	return newCompletionItem(
		...
		new Span(startIndex, length),
		Completion.CompletionKind.tleFunction,
		"(function) concat(arg1, arg2, arg3, ...)",
...
export function expectedCopyIndexCompletion(
	startIndex: number,
	length: number
): Completion.Item {
	return newCompletionItem(
		...
		new Span(startIndex, length),
		Completion.CompletionKind.tleFunction,
		"(function) copyIndex([offset]) or copyIndex(loopName, [offset])",
		"Returns the current index of an iteration loop.\nThis function is always used with a copy object."
	);
}

export function expectedDeploymentCompletion(
	startIndex: number,
	length: number
): Completion.Item {
	return newCompletionItem(
		...
		"Returns information about the current deployment operation. This function returns the object that is passed during deployment. The properties in the returned object will differ based on whether the deployment object is passed as a link or as an in-line object."
	);
}

export function expectedDivCompletion(
	startIndex: number,
	length: number
): Completion.Item {
	return newCompletionItem(
...
export function expectedEqualsCompletion(
	startIndex: number,
	length: number
): Completion.Item {
	return newCompletionItem(
...
export function expectedIntCompletion(
	startIndex: number,
	length: number
): Completion.Item {
	return newCompletionItem(
...
export function expectedLengthCompletion(
	startIndex: number,
	length: number
): Completion.Item {
	return newCompletionItem(
...
export function expectedListKeysCompletion(
	startIndex: number,
	length: number
): Completion.Item {
	return newCompletionItem(
		...
		new Span(startIndex, length),
		Completion.CompletionKind.tleFunction,
		"(function) listKeys(resourceName/resourceIdentifier, apiVersion) [object]",
		"Returns the keys of a storage account. The resourceId can be specified by using the resourceId function or by using the format providerNamespace/resourceType/resourceName. You can use the function to get the primary (key[0]) and secondary key (key[1])."
	);
}

export function expectedListPackageCompletion(
	startIndex: number,
	length: number
): Completion.Item {
	return newCompletionItem(
		...
		new Span(startIndex, length),
		Completion.CompletionKind.tleFunction,
		"(function) listPackage(resourceName/resourceIdentifier, apiVersion)",
		"Lists the virtual network gateway package. The resourceId can be specified by using the resourceId function or by using the format providerNamespace/resourceType/resourceName."
	);
}

export function expectedModCompletion(
	startIndex: number,
	length: number
): Completion.Item {
	return newCompletionItem(
...
export function expectedMulCompletion(
	startIndex: number,
	length: number
): Completion.Item {
	return newCompletionItem(
...
export function expectedPadLeftCompletion(
	startIndex: number,
	length: number
): Completion.Item {
	return newCompletionItem(
...
export function expectedParametersCompletion(
	startIndex: number,
	length: number
): Completion.Item {
	return newCompletionItem(
...
export function expectedProvidersCompletion(
	startIndex: number,
	length: number
): Completion.Item {
	return newCompletionItem(
		...
		new Span(startIndex, length),
		Completion.CompletionKind.tleFunction,
		"(function) providers(providerNamespace, [resourceType])",
...
export function expectedReferenceCompletion(
	startIndex: number,
	length: number
): Completion.Item {
	return newCompletionItem(
		...
		new Span(startIndex, length),
		Completion.CompletionKind.tleFunction,
		"(function) reference(resourceName/resourceIdentifier, [apiVersion], ['Full'])",
		"Enables an expression to derive its value from another resource's runtime state."
	);
}

export function expectedReplaceCompletion(
	startIndex: number,
	length: number
): Completion.Item {
	return newCompletionItem(
...
export function expectedResourceGroupCompletion(
	startIndex: number,
	length: number
): Completion.Item {
	return newCompletionItem(
		...
		new Span(startIndex, length),
		Completion.CompletionKind.tleFunction,
		"(function) resourceGroup() [object]",
		"Returns a structured object that represents the current resource group."
	);
}

export function expectedResourceIdCompletion(
	startIndex: number,
	length: number
): Completion.Item {
	return newCompletionItem(
		...
		new Span(startIndex, length),
		Completion.CompletionKind.tleFunction,
		"(function) resourceId([subscriptionId], [resourceGroupName], resourceType, resourceName1, [resourceName2]...)",
...
export function expectedSkipCompletion(
	startIndex: number,
	length: number
): Completion.Item {
	return newCompletionItem(
...
export function expectedStringCompletion(
	startIndex: number,
	length: number
): Completion.Item {
	return newCompletionItem(
...
export function expectedSubCompletion(
	startIndex: number,
	length: number
): Completion.Item {
	return newCompletionItem(
...
export function expectedSubscriptionCompletion(
	startIndex: number,
	length: number
): Completion.Item {
	return newCompletionItem(
...
export function expectedSubscriptionResourceIdCompletion(
	startIndex: number,
	length: number
): Completion.Item {
	return newCompletionItem(
		...
		new Span(startIndex, length),
		Completion.CompletionKind.tleFunction,
		"(function) subscriptionResourceId([subscriptionId], resourceType, resourceName1, [resourceName2]...)",
		"Returns the unique resource identifier of a subscription scoped resource. You use this function to create a resourceId for a given resource as required by a property value."
	);
}

export function expectedSubstringCompletion(
	startIndex: number,
	length: number
): Completion.Item {
	return newCompletionItem(
...
export function expectedTakeCompletion(
	startIndex: number,
	length: number
): Completion.Item {
	return newCompletionItem(
...
export function expectedToUpperCompletion(
	startIndex: number,
	length: number
): Completion.Item {
	return newCompletionItem(
...
export function expectedTrimCompletion(
	startIndex: number,
	length: number
): Completion.Item {
	return newCompletionItem(
...
export function expectedUniqueStringCompletion(
	startIndex: number,
	length: number
): Completion.Item {
	return newCompletionItem(
		...
		new Span(startIndex, length),
		Completion.CompletionKind.tleFunction,
		"(function) uniqueString(stringForCreatingUniqueString, ...)",
...
export function expectedUriCompletion(
	startIndex: number,
	length: number
): Completion.Item {
	return newCompletionItem(
...
export function expectedVariablesCompletion(
	startIndex: number,
	length: number
): Completion.Item {
	return newCompletionItem(
...
export function parameterCompletion(
	...
	startIndex: number,
	length: number,
	includeRightParenthesis: boolean = true
): Completion.Item {
	return newCompletionItem(
		`'${parameterName}'`,
		`'${parameterName}'${includeRightParenthesis ? ")" : ""}`,
		new Span(startIndex, length),
		Completion.CompletionKind.tleParameter,
		"(parameter)",
		undefined
	);
}
...
export function propertyCompletion(
	...
	return newCompletionItem(
		...
		`${propertyName}`,
		new Span(startIndex, length),
		Completion.CompletionKind.tleProperty,
		"(property)"
	);
}
...
export function variableCompletion(
	...
	return newCompletionItem(
		`'${variableName}'`,
		`'${variableName}'${includeRightParenthesis ? ")" : ""}`,
		new Span(startIndex, length),
		Completion.CompletionKind.tleVariable,
		"(variable)"
	);
}
