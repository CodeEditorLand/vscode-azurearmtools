...
import {
	...
	deleteTestLog,
	getTestLogContents,
	setTestLogOutputFile,
	writeToError,
	writeToLog,
} from "./support/testLog";
...
suiteSetup(async function (this: mocha.Context): Promise<void> {
	...
	previousSettings.autoDetectJsonTemplates = vscode.workspace
		.getConfiguration(configPrefix)
		.get<boolean>(configKeys.autoDetectJsonTemplates);
	await vscode.workspace
		.getConfiguration(configPrefix)
		.update(
			configKeys.autoDetectJsonTemplates,
			true,
	...
	let fileAssociations = (previousSettings.fileAssociations = Object.assign(
		{},
		vscode.workspace.getConfiguration("files").get<{}>("associations")
	));
	console.warn("Old file associations:", fileAssociations);
	let newAssociations = Object.assign({}, fileAssociations, {
		"*.azrm": armTemplateLanguageId,
	});
	vscode.workspace
	...
	const confirmedNewAssociations = Object.assign(
		{},
		vscode.workspace.getConfiguration("files").get<{}>("associations")
	);
...
suiteTeardown(async function (this: mocha.Context): Promise<void> {
	...
	if (ext.extensionStartupComplete) {
        ...
        */
		...
		console.warn(
			"Cannot publish logs because extension context is not available (startup didn't complete)"
		);
	}
});
...
teardown(function (this: Mocha.Context): void {
	let message: string;
	const failed =
		!this.currentTest!.state || this.currentTest!.state === "failed";
	...
	if (failed) {
		if (getTestLogContents()) {
			...
			message = `Test Failed: (test log is empty): ${
	...
	if (
		...
		parseError(this.currentTest?.err).message.match(/timed/i)
	) {
		throw new Error(
			`A test timed out ('${
				this.currentTest!.title
			}'), so exiting out of the rest of the tests because tests tend to be unstable after a timeout`
		);
	}
