...
export const DISABLE_SLOW_TESTS = !!/^(true|1)$/i.test(
	process.env.DISABLE_SLOW_TESTS || ""
);
writeToLog(`DISABLE_SLOW_TESTS = ${DISABLE_SLOW_TESTS}`);

// tslint:disable-next-line: strict-boolean-expressions
export const DISABLE_LANGUAGE_SERVER: boolean =
	!!/^(true|1)$/i.test(process.env.DISABLE_LANGUAGE_SERVER || "") ||
	DISABLE_SLOW_TESTS;
...
export namespace testMessages {
	export function nestedTemplateNoValidation(
		templateName: string,
		range?: string
	): string {
		return (
			`Information: Nested template "${templateName}" will not have validation or parameter completion. To enable, either add default values to all top-level parameters or add a parameter file ("Select/Create Parameter File" command).` +
			(range ? ` (arm-template (expressions)) ${range}` : "")
		);
	}

	export function linkedTemplateNoValidation(
		templateName: string,
		range?: string
	): string {
		return (
			`Information: Linked template "${templateName}" will not have validation or parameter completion. To enable, either add default values to all top-level parameters or add a parameter file ("Select/Create Parameter File" command).` +
			(range ? ` (arm-template (expressions)) ${range}` : "")
		);
	}
}
