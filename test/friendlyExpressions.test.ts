...
import {
	getFriendlyExpressionFromJsonString,
	getFriendlyExpressionFromTleExpression,
	...
	suite("getFriendlyExpressionFromTleExpression", () => {
		function createFriendlyExpressionTest(
			...
			test(tleExpression, () => {
				...
				const actual =
					getFriendlyExpressionFromTleExpression(tleExpression);
		...
		suite(
			"Expressions with errors - just return original, plus var/par interpolation",
			() => {
				createFriendlyExpressionTest("", "[]");
				createFriendlyExpressionTest("a", "[a]");
				createFriendlyExpressionTest(
		...
		suite("concat strings", () => {
			...
			createFriendlyExpressionTest(
				"concat(variables('sqlServer'), 'a')",
				"${sqlServer}a"
			);
			createFriendlyExpressionTest(
				"concat(variables('a'),'b',variables('abc'))",
				"${a}b${abc}"
			);
			createFriendlyExpressionTest(
				"concat(variables('sqlServer'), '/', variables('firewallRuleName2'))",
				"${sqlServer}/${firewallRuleName2}"
			);
		});
		...
		suite("concat other things", () => {
			...
			createFriendlyExpressionTest(
				"concat(1, parameters('a'), 'b', 2, 3)",
				"[concat(1, '${a}b', 2, 3)]"
			);
			createFriendlyExpressionTest("concat(1, 2)", "[concat(1, 2)]");
			createFriendlyExpressionTest(
				"concat(concat(1, 2), concat(3, 4), 5)",
				"[concat(concat(1, 2), concat(3, 4), 5)]"
			);
			createFriendlyExpressionTest(
				"concat('a', variables('b'))",
				"a${b}"
			);
			createFriendlyExpressionTest(
				"concat(variables('a'), 'b')",
				"${a}b"
			);
			createFriendlyExpressionTest(
				"concat(variables('a'), 'b', 1)",
				"[concat('${a}b', 1)]"
			);
			createFriendlyExpressionTest(
				"concat(variables('a'), 1, 'b')",
				"[concat(${a}, 1, 'b')]"
			);
			createFriendlyExpressionTest(
		...
		createFriendlyExpressionTest(
			"format('{0}/default/logs', variables('storageAccountName'))",
			"[format('{0}/default/logs', ${storageAccountName})]"
		);
		createFriendlyExpressionTest(
			"concat(parameters('virtualMachineName'),'/', variables('diagnosticsExtensionName'))",
			"${virtualMachineName}/${diagnosticsExtensionName}"
		);

		suite(
			"Shouldn't concat when an expression is an interpolated string with property index",
			() => {
				createFriendlyExpressionTest(
		...
		suite("from 101-sql-logical-server.json", () => {
			createFriendlyExpressionTest(
				"concat(variables('storageName'), '/Microsoft.Authorization/', variables('uniqueRoleGuid') )",
				"${storageName}/Microsoft.Authorization/${uniqueRoleGuid}"
			);
			createFriendlyExpressionTest(
				"resourceGroup().name",
				"[resourceGroup().name]"
			...
			createFriendlyExpressionTest(
				"tolower(concat('sqlva', variables('uniqueStorage')))",
				"[tolower('sqlva${uniqueStorage}')]"
			);
		});
		...
		suite("from copy-in-outputs3.json", () => {
			createFriendlyExpressionTest(
				...
				"${rgNamePrefix}-${rgEnvList}[copyIndex()]"
			);
			createFriendlyExpressionTest(
				"concat('nic-', copyIndex())",
				"[concat('nic-', copyIndex())]"
			);
			createFriendlyExpressionTest(
				'array(json(\'["one","two","three"]\'))[copyIndex()]',
				'[array(json(\'["one","two","three"]\'))[copyIndex()]]'
			);
		});

		createFriendlyExpressionTest(
			"if(parameters('globalRedundancy'), 'Standard_GRS', 'Standard_LRS')",
			"[if(${globalRedundancy}, 'Standard_GRS', 'Standard_LRS')]"
		);
		createFriendlyExpressionTest(
			"format('{0}/default/logs', variables('storageAccountName'))",
			"[format('{0}/default/logs', ${storageAccountName})]"
		...
		suite("from icons.json", () => {
			createFriendlyExpressionTest(
				"myFunctions.vmName(parameters('virtualMachineName'),copyindex(1))",
				"[myFunctions.vmName(${virtualMachineName}, copyindex(1))]"
			);
			createFriendlyExpressionTest(
				"myFunctions.diskName(parameters('virtualMachineName'),copyindex(1))",
				"[myFunctions.diskName(${virtualMachineName}, copyindex(1))]"
			);
			createFriendlyExpressionTest(
				"myFunctions.nicName(parameters('virtualMachineName'),copyindex(1))",
				"[myFunctions.nicName(${virtualMachineName}, copyindex(1))]"
			);
		});

		suite("from keyvalue-with-param.json", () => {
			createFriendlyExpressionTest(
				"concat('sql-', uniqueString(resourceGroup().id, 'sql'))",
				"[concat('sql-', uniqueString(resourceGroup().id, 'sql'))]"
			);
		});

		createFriendlyExpressionTest(
			"concat(parameters('storagePrefix'), uniqueString(resourceGroup().id))",
			"[concat(${storagePrefix}, uniqueString(resourceGroup().id))]"
		);
		createFriendlyExpressionTest(
			"concat(parameters('storagePrefix'), uniqueString(parameters('secondSubscriptionID'), parameters('secondResourceGroup')))",
			"[concat(${storagePrefix}, uniqueString(${secondSubscriptionID}, ${secondResourceGroup}))]"
		);

		suite("from outofbounds1.json", () => {
			createFriendlyExpressionTest(
				"parameters('vmNames')[0]",
				"[${vmNames}[0]]"
			);
			createFriendlyExpressionTest(
				"concat(parameters('projectName'),'sqlsrv/',variables('rdsDBName'))",
				"${projectName}sqlsrv/${rdsDBName}"
			);
		});
	});
