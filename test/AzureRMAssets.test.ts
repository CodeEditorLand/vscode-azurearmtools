...
suite("AzureRMAssets", () => {
	networkTest("getFunctionMetadata()", async () => {
		...
		assert(
			functionMetadataArray.length > 0,
			`Expected to get at least 1 function metadata, but got ${functionMetadataArray.length} instead.`
		);
	});

	suite("FunctionMetadata", () => {
		test("constructor(string,string,string)", () => {
			const metadata = new BuiltinFunctionMetadata(
				"a",
				"b",
				"c",
				1,
		...
		test("findByName", () => {
			const metadata = new FunctionsMetadata([
				new BuiltinFunctionMetadata("hi", "", "", 0, 0, [], undefined),
			...
			assert.equal(
				metadata.findbyName("MyFunction")!.fullName,
				"MyFunction"
			);
			assert.equal(
				metadata.findbyName("myfunction")!.fullName,
		...
		test("findByPrefix", () => {
			...
			assert.deepStrictEqual(
				metadata.filterByPrefix("On").map((meta) => meta.fullName),
				["One", "Onerous"]
			);
			assert.deepStrictEqual(
				metadata.filterByPrefix("on").map((meta) => meta.fullName),
			...
			assert.deepStrictEqual(
				metadata.filterByPrefix("ONE").map((meta) => meta.fullName),
				["One", "Onerous"]
			);
			assert.deepStrictEqual(
				metadata.filterByPrefix("Oner").map((meta) => meta.fullName),
				["Onerous"]
			);
			assert.deepStrictEqual(
				metadata.filterByPrefix("Onerous").map((meta) => meta.fullName),
				["Onerous"]
			);
		...
		suite("fromString(string)", () => {
			test("with empty string", () => {
				assert.deepStrictEqual(
					BuiltinFunctionMetadata.fromString(""),
					[]
				);
			});
			...
			test("with non-JSON string", () => {
				assert.deepStrictEqual(
					BuiltinFunctionMetadata.fromString("hello there"),
					[]
				);
			});
			...
			test("with one function signature with only name property", () => {
				assert.deepStrictEqual(
					BuiltinFunctionMetadata.fromString(
						`{ "functionSignatures": [ { "name": "a", "expectedUsage": "z", "description": "1" } ] }`
					),
					[
			...
			test("with two function signatures with only name property", () => {
				assert.deepStrictEqual(
					...
					BuiltinFunctionMetadata.fromString(
						`{ "functionSignatures": [ { "name": "a", "expectedUsage": "z" }, { "name": "b", "expectedUsage": "y", "description": "7" } ] }`
					),
					[
						// tslint:disable-next-line:no-any
						new BuiltinFunctionMetadata(
			...
			test("with actual ExpressionMetadata.json file contents", () => {
				...
				const functionMetadata: BuiltinFunctionMetadata[] =
					BuiltinFunctionMetadata.fromString(fileContents);
