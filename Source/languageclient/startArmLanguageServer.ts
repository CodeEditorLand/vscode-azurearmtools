...
const _notifyTemplateGraphAvailableEmitter: EventEmitter<
 ...
 */
export const notifyTemplateGraphAvailable: Event<
	INotifyTemplateGraphArgs & ITelemetryContext
> = _notifyTemplateGraphAvailableEmitter.event;
...
export async function stopArmLanguageServer(): Promise<void> {
	...
	if (
		ext.languageServerState === LanguageServerState.NotStarted ||
				...
				await callWithTelemetryAndErrorHandling(
					...
					async (actionContext: IActionContext) => {
						try {
							...
							ext.languageServerState =
								haveFirstSchemasFinishedLoading
	...
	await callWithTelemetryAndErrorHandling(
		...
		async (actionContext: IActionContext) => {
			...
			const waitForDebugger =
				workspace
					.getConfiguration(configPrefix)
					.get<boolean>(configKeys.waitForDebugger, false) === true;
				...
				middleware: {
					handleDiagnostics: (
						...
						for (const d of diagnostics) {
							if (
								d.source === backendValidationDiagnosticsSource
							) {
...
async function getDotNetPath(): Promise<string | undefined> {
	return await callWithTelemetryAndErrorHandling(
		...
		async (actionContext: IActionContext) => {
			...
			const overriddenDotNetExePath = workspace
				.getConfiguration(configPrefix)
				.get<string>(configKeys.dotnetExePath);

			if (
				typeof overriddenDotNetExePath === "string" &&
...
function findLanguageServer(): string {
	const serverDllPath: string | undefined =
		callWithTelemetryAndErrorHandlingSync(
			"findLanguageServer",
			(actionContext: IActionContext) => {
				...
				if (
					typeof serverDllPathSetting !== "string" ||
					serverDllPathSetting === ""
				) {
...
async function isFile(pathPath: string): Promise<boolean> {
	return (
		(await fse.pathExists(pathPath)) && (await fse.stat(pathPath)).isFile()
	);
}
...
function onSchemaValidationNotication(
	...
	console.warn(
		args.completed
			? `Finished loading schemas for ${args.uri}`
			: `Loading schemas for ${args.uri}`,
	);
	...
	const isLoadingInitialSchemas =
		haveFirstSchemasStartedLoading && !haveFirstSchemasFinishedLoading;

	const newState =
		isLoadingInitialSchemas &&
