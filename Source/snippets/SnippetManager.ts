...
export class SnippetManager implements ISnippetManager {
	private _snippetMap: CachedPromise<Map<string, ISnippetInternal>> =
		new CachedPromise<Map<string, ISnippetInternal>>();
	...
	private static async readSnippetFile(
		...
		const snippets = <{ [key: string]: ISnippetDefinitionFromFile }>(
				...
				async (actionContext: IActionContext) => {
					...
					for (const entry of map.entries()) {
						...
						if (
							...
							if (
								insertionContext.insideJsonString &&
								!label.startsWith('"')
							) {
								label = `"${label}"`;
							}
