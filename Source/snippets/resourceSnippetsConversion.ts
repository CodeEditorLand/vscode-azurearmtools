...
function getJsonFromResourceSnippetFile(
	...
	try {
		...
		throw new Error(
			`Error processing resource snippet '${snippetName}': ${parseError(ex).message}`,
		);
	}
}

function getBodyFromResourceSnippetJson(json: {
	[key: string]: unknown;
}): string[] {
	...
	for (const line of lines) {
		const tabs = (<string[]>line.match(/^ */))[0].length - 1; // Remove the indentation of the '[]' that we removed
