...
export class StringValue extends Value {
	...
	public hasCloseQuote(): boolean {
		return (
			this.length > 1 &&
			this.toString()[this.length - 1] === this.quoteCharacter
		);
	}
	...
	private isBuiltinFunctionArgument(functionName: string): boolean {
		const funcCall: FunctionCallValue | undefined =
			this.getFunctionCallParentOfArgument();
		return (
			!!funcCall &&
			funcCall.isCallToBuiltinWithName(functionName) &&
...
export class FunctionCallValue extends ParentValue {
	...
	public get namespace(): string | undefined {
		...
		return this._namespaceToken?.stringValue || undefined;
	}
	...
	public get name(): string | undefined {
		// tslint:disable-next-line: strict-boolean-expressions
		return this._nameToken?.stringValue || undefined;
	}
	...
	public get fullNameSpan(): Span {
		const result: Span | undefined = Span.union(
			// tslint:disable-next-line: strict-boolean-expressions
			Span.union(this._namespaceToken?.span, this.periodToken?.span),
			// tslint:disable-next-line: strict-boolean-expressions
			this._nameToken?.span,
		);
	...
	public doesNameMatch(
		...
		const thisNamespace = this._namespaceToken
			? this._namespaceToken.stringValue
			: "";

		return (
			thisNamespace.toLowerCase() === namespaceName.toLowerCase() &&
	...
	public get argumentListSpan(): Span | undefined {
		...
		if (this._leftParenthesisToken) {
			...
			if (this._rightParenthesisToken) {
				...
				this._argumentExpressions.length > 0 ||
				this._commaTokens.length > 0
			) {
				for (
	...
	public getSpan(): Span {
		return this.fullNameSpan.union(this.argumentListSpan);
	}
	...
	public formatCore(
		...
		if (
			this.argumentExpressions.length === 0 ||
	...
	private static coalesceConcatArguments(
		...
		for (let i = 0; i < expressionsLength; ++i) {
			const currentExpression =
				expressions[i]?.formatCore(options, currentIndent) ?? "";

			const prevCoalescedExpression =
			...
			if (prevCoalescedExpression) {
				...
				if (
					isString(prevCoalescedExpression) &&
					isString(currentExpression)
				) {
					...
					const previousContainsPropertyIndex =
...
export class PropertyAccess extends ParentValue {
	...
	public get sourcesNameStack(): string[] {
		...
		let propertyAccessSource: PropertyAccess | undefined =
			asPropertyAccessValue(this._source);
		while (propertyAccessSource?.nameToken) {
	...
	public static getHighlightCharacterIndexes(
		...
		if (context.tleInfo) {
			...
			if (
				...
				const tleValue: Value | undefined =
					tleParseResult.getValueAtCharacterIndex(tleCharacterIndex);
				if (tleValue instanceof FunctionCallValue) {
					if (
						!!tleValue.leftParenthesisToken &&
						tleValue.leftParenthesisToken.span.startIndex ===
							tleCharacterIndex
					) {
					...
					if (
						tleValue.leftSquareBracketToken.span.startIndex ===
						tleCharacterIndex
					) {
			...
			if (
				...
				if (tleValue instanceof FunctionCallValue) {
					if (
						!!tleValue.rightParenthesisToken &&
					...
					if (
						!!tleValue.rightSquareBracketToken &&
						tleValue.rightSquareBracketToken.span.startIndex ===
							leftOfTLECharacterIndex
					) {
						BraceHighlighter.addTLEArrayHighlights(
							highlightCharacterIndexes,
...
export class Parser {
	...
	private static parseExpression(
		...
		while (<Token | undefined>tokenizer.current) {
			if (tokenizer.current.getType() === TokenType.Period) {
				...
				const indexValue: Value | undefined = Parser.parseExpression(
					tokenizer,
					errors,
				);
				...
				expression = new ArrayAccessValue(
					expression,
					leftSquareBracketToken,
					indexValue,
					rightSquareBracketToken,
	...
	private static parseFunctionCall(
		...
		if (
			tokenizer.current &&
			tokenizer.current.getType() === TokenType.Period
		) {
			...
			while (tokenizer.current) {
				if (
					...
					if (
						!expression &&
						tokenizer.hasCurrent() &&
						tokenizer.current.getType() === TokenType.Comma
					) {
						errors.push(
							new Issue(
								tokenizer.current.span,
		...
		function getFullNameSpan(): Span {
			if (nameToken) {
				// tslint:disable-next-line: strict-boolean-expressions
				return nameToken.span.union(namespaceToken?.span);
			} else {
...
export class TleParseResult {
	...
	public getValueAtCharacterIndex(characterIndex: number): Value | undefined {
		...
		if (current?.contains(characterIndex)) {
			while (!result) {
				...
				if (currentValue instanceof FunctionCallValue) {
					...
					for (const argumentExpression of currentValue.argumentExpressions) {
						if (argumentExpression?.contains(characterIndex)) {
					...
					if (currentValue.source.contains(characterIndex)) {
						...
						currentValue.indexValue?.contains(characterIndex)
					) {
...
export class Tokenizer {
	...
	private _currentTokenStartIndex = 1;
	...
	public static fromString(stringValue: string): Tokenizer {
		...
		const trimmedLength: number =
			stringValue.length -
			(stringValue.endsWith(initialQuoteCharacter) ? 2 : 1);
			...
			switch (currentBasicToken.getType()) {
				...
				default: {
					...
					while (
						...
						(this.currentBasicToken.getType() ===
	...
	private skipWhitespace(): void {
		while (
			this._current &&
			this._current.getType() === TokenType.Whitespace
		) {
			this.next();
		}
	}
...
export function readQuotedTLEString(
	...
	while (iterator.current()) {
		...
		if (escaped) {
			escaped = false;
		} else if (current.getType() === basic.TokenType.Backslash) {
			escaped = true;
