...
import { LineColPos } from "../LineColPos";
...
export abstract class Segment {
	...
	public get span(): Span {
		return new Span(this._startIndex, this.length());
	}
...
export function readNumber(iterator: Iterator<basic.Token>): basic.Token[] {
	...
	if (decimal && decimal.getType() === basic.TokenType.Period) {
		...
		if (
			fractionalDigits &&
			fractionalDigits.getType() === basic.TokenType.Digits
		) {
	...
	if (exponentLetter) {
		if (
			exponentLetter.getType() === basic.TokenType.Letters &&
			exponentLetter.toString().toLowerCase() === "e"
		) {
			// e
			...
			if (
				exponentSign &&
				(exponentSign.getType() === basic.TokenType.Dash ||
					exponentSign.getType() === basic.TokenType.Plus)
			) {
			...
			if (
				exponentDigits &&
				exponentDigits.getType() === basic.TokenType.Digits
			) {
...
export class Tokenizer {
	...
	private currentBasicTokenType(): basic.TokenType | undefined {
		const currentBasicToken: basic.Token | undefined =
	...
	private moveNextBasicToken(): boolean {
		...
		if (currentBasicToken) {
			...
			if (
				currentBasicTokenType === basic.TokenType.NewLine ||
				currentBasicTokenType === basic.TokenType.CarriageReturnNewLine
			) {
	...
	private asBasicTokenIterator(): Iterator<basic.Token> {
		...
		return {
			...
			current: (): basic.Token | undefined => {
				return this.currentBasicToken();
			},
			moveNext: (): boolean => {
				return this.moveNextBasicToken();
			},
		};
	}
...
export abstract class Value {
	...
	public isPropertyWithName(propertyNameCaseInsensitive: string): boolean {
		return (
			...
			propertyNameCaseInsensitive.toLowerCase()
		);
	}
...
export class ObjectValue extends Value {
	...
	private _caseInsensitivePropertyMap: CachedValue<
	 ...
	 * created lazily.
	 */
	private get caseInsensitivePropertyMap(): CaseInsensitiveMap<
		string,
		Property | undefined
	> {
		return this._caseInsensitivePropertyMap.getOrCacheValue(() => {
			const caseInsensitivePropertyMap = new CaseInsensitiveMap<
				string,
				Property | undefined
			>();
	...
	public getPropertyValue(propertyName: string): Value | undefined {
		const result: Property | undefined =
			this.caseInsensitivePropertyMap.get(propertyName);
	...
	public toFullFriendlyString(): string {
		...
		return (
			...
			this.properties
				.map(
					(pv) =>
						`"${pv.nameValue.toString()}": ${Value.toFullFriendlyString(pv.value)}`,
				)
				.join(", ") +
			"}"
		);
...
export class StringValue extends Value {
	...
	public get unquotedSpan(): Span {
		return new Span(this.startIndex + 1, this._unquotedValue.length);
	}
	...
	public toString(): string {
		// CONSIDER: remove in favor of unquotedValue
		return this._unquotedValue;
	}
...
export class ParseResult {
	...
	// Might or might not make a copy
	public getTokensInSpan(span: Span, commentsBehavior: Comments): Token[] {
		const results: Token[] = [];
	...
	public getCharacterIndex(
		...
		if (columnIndex > maxColumnIndex) {
			if (options?.allowOutOfBounds) {
				...
				assert.fail(
					`Cannot get a character index for a columnIndex (${columnIndex}) that is greater than the lineIndex's (${lineIndex}) line max column index (${this.getMaxColumnIndex(lineIndex)}).`,
				);
			}
		}
	...
	public getPositionFromCharacterIndex(characterIndex: number): LineColPos {
		assert(0 <= characterIndex);
	...
	private static getToken(tokens: Token[], tokenIndex: number): Token {
		...
		assert(
			...
			`The tokenIndex (${tokenIndex}) must always be between 0 and the token count - 1 (${tokens.length - 1}).`,
		);
...
function parseObject(
	...
	let objectSpan: Span = tokenizer.current.span;
	const properties: Property[] = [];

	next(tokenizer, tokens, commentTokens);

	let propertySpan: Span | undefined;
	...
	while (tokenizer.current) {
		...
		if (tokenizer.current.type === TokenType.RightCurlyBracket) {
			...
			propertySpan = propertySpan
				? propertySpan.union(tokenizer.current.span)
				: tokenizer.current.span;
...
function parseArray(
	...
	let span: Span = tokenizer.current.span;
