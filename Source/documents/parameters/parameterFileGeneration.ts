...
export async function queryCreateParameterFile(
	...
	const required = <QuickPickItem>{
		label: "Only required parameters",
		description:
			"Uses only parameters that have no default value in the template file",
	};
	...
	const fileNameWithoutJsonC: string = path
		.basename(templateUri.fsPath)
...
export function createParameterFromTemplateParameter(
    ...
    */
	...
	if (matchingParentParameter) {
		...
		const isExpression =
			parameter.defaultValue instanceof Json.StringValue &&
