...
export function getParameterValuesCodeActions(
	...
	if (parametersProperty) {
		...
		if (
			...
			lineIndexOfParametersProperty <= range.end.line
		) {
			const missingParameters: IParameterDefinition[] =
...
export async function addMissingParameters(
	...
	if (
		parameterValuesSource.parameterValuesProperty &&
		...
		for (
			let i = parametersProperty.span.endIndex - 1; // Start before the closing "}"
		...
		const insertPosition =
			parameterValuesDocument.getDocumentPosition(insertIndex);
		...
		const lastParameter =
			parameterValuesSource.parameterValueDefinitions.length > 0
				? parameterValuesSource.parameterValueDefinitions[
						parameterValuesSource.parameterValueDefinitions.length -
							1
				  ]
				: undefined;
		const lastParameterIndent = lastParameter
			...
			: undefined;
		const newTextIndent =
			lastParameterIndent === undefined
				? parametersObjectIndent + defaultTabSize
				: lastParameterIndent;
...
export function getCompletionsForMissingParameters(
	...
	for (let param of parameterDefinitionsSource.parameterDefinitions) {
		...
		const detail =
			(isRequired ? "(required parameter)" : "(optional parameter)") +
			EOL +
			EOL +
			paramText;
...
export function getPropertyValueCompletionItems(
	...
	if (
		...
		canAddPropertyValueHere(parameterValuesSource, documentIndex)
	) {
		if (parameterDefinitionsSource) {
			completions.push(
				...getCompletionsForMissingParameters(
					parameterDefinitionsSource,
					parameterValuesSource,
					parentParameterDefinitionsSource,
					tabSize,
...
export function getMissingParameterErrors(
	...
	const span =
		...
		(parameterValues.deploymentRootObject
			? new Span(parameterValues.deploymentRootObject?.startIndex, 0)
			: undefined) ??
		new Span(0, 0);
	...
	if (definition.nameValue) {
		...
		if (paramValue) {
			results.add({
				...
				span: paramValue.nameValue.unquotedSpan,
			});
		}
	}
