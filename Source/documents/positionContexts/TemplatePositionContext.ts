...
export class TemplatePositionContext extends PositionContext {
	private _tleInfo: CachedValue<TleInfo | undefined> = new CachedValue<
		TleInfo | undefined
	>();

	public static fromDocumentLineAndColumnIndexes(
		deploymentTemplate: DeploymentTemplateDoc,
		documentLineIndex: number,
		documentColumnIndex: number,
		associatedParameters: DeploymentParametersDoc | undefined,
		allowOutOfBounds: boolean = true,
	): TemplatePositionContext {
		let context = new TemplatePositionContext(
			deploymentTemplate,
	...
	public static fromDocumentCharacterIndex(
		...
		context.initFromDocumentCharacterIndex(
	...
	public get tleInfo(): TleInfo | undefined {
		return this._tleInfo.getOrCacheValue(() => {
			...
			if (
				...
				const tleParseResult =
					this.document.getTLEParseResultFromJsonStringValue(
						this.jsonValue,
					);

				const tleCharacterIndex =
					this.documentCharacterIndex - this.jsonTokenStartIndex;

				const tleValue =
					tleParseResult.parseResult.getValueAtCharacterIndex(
						tleCharacterIndex,
					);

				return new TleInfo(
					tleParseResult.parseResult,
					tleCharacterIndex,
					tleValue,
	...
	public getReferenceSiteInfo(
		considerDefinitionItself: boolean,
		...
		if (tleInfo) {
			...
			const tleFuncCall: TLE.FunctionCallValue | undefined =
			...
			if (tleFuncCall) {
				if (
					...
					const nsDefinition =
						scope.getFunctionNamespaceDefinition(ns);

					if (nsDefinition) {
						const unquotedReferenceSpan: Span =
							tleFuncCall.namespaceToken.span.translate(
					...
					const unquotedReferenceSpan: Span =
						tleFuncCall.nameToken.span.translate(
							this.jsonTokenStartIndex,
						);
					...
					if (
						tleFuncCall.nameToken.span.contains(
							tleCharacterIndex,
							ContainsBehavior.strict,
						)
					) {
						if (tleFuncCall.namespaceToken) {
							...
							const userFunctiondefinition =
								scope.getUserFunctionDefinition(ns, name);
							...
							const builtInFuncName: string =
								tleFuncCall.nameToken.stringValue.toLowerCase();
							...
							if (tleFuncCall.argumentExpressions.length === 1) {
								...
								if (arg instanceof TLE.StringValue) {
									const argStringValue: string | undefined =
										tleFuncCall.argumentExpressions[0]
							...
							const functionMetadata:
								| BuiltinFunctionMetadata
								| undefined =
								AzureRMAssets.getFunctionMetadataFromName(
									tleFuncCall.nameToken.stringValue,
								);
			...
			const tleStringValue: TLE.StringValue | undefined =
				TLE.asStringValue(tleInfo.tleValue);
			...
			if (tleStringValue) {
				...
				if (tleStringValue.isParametersArgument()) {
					...
					const ref = this.getParameterReference(
						referenceDocument,
						definitionDocument,
						scope,
						referenceKind,
					...
					const ref = this.getVariableReference(
						referenceDocument,
						definitionDocument,
						scope,
						referenceKind,
			...
			if (considerDefinitionItself) {
				...
				if (definition && definition.nameValue) {
					return {
						...
						unquotedReferenceSpan:
							definition.nameValue?.unquotedSpan,
					};
				}
			}
	...
	public getHoverInfo(): IHoverInfo[] {
		...
		if (
			this.jsonValue instanceof Json.StringValue &&
			isTleExpression(this.jsonValue.unquotedValue)
		) {
			...
			this.jsonValue instanceof Json.StringValue &&
			this.jsonValue.unquotedValue.match(/\\r|\\n|\r|\n/)
		) {
	...
	private getParameterReference(
		...
		const parameterDefinition: IParameterDefinition | undefined =
			scope.getParameterDefinition(arg.unquotedValue);

		if (parameterDefinition) {
			const unquotedReferenceSpan: Span = arg.unquotedSpan.translate(
				this.jsonTokenStartIndex,
			);
	...
	private getVariableReference(
		...
		const variableDefinition: IVariableDefinition | undefined =
			scope.getVariableDefinition(arg.unquotedValue);
	...
	private isInsideParameterDefinitions(
		parents: (Json.ObjectValue | Json.ArrayValue | Json.Property)[],
	): boolean {
	...
	private isInsideParameterValues(
		parents: (Json.ObjectValue | Json.ArrayValue | Json.Property)[],
	): boolean {
	...
	private isInsideUserFunctionParameterDefinitions(
		parents: (Json.ObjectValue | Json.ArrayValue | Json.Property)[],
	): boolean {
		// "functions": [
		// {
			...
			if (context === "parameters") {
				if (this.isInsideParameterDefinitions(parents)) {
					insertionContext.context =
						KnownContexts.parameterDefinitions;
				} else if (this.isInsideParameterValues(parents)) {
					insertionContext.context = KnownContexts.parameterValues;
				} else if (
					this.isInsideUserFunctionParameterDefinitions(parents)
				) {
					insertionContext.context =
						KnownContexts.userFuncParameterDefinitions;
				}
			} else if (
	...
	public async getCompletionItems(
			...
			if (uniqueScope.parameterValuesSource) {
				completions.push(
					...getPropertyValueCompletionItems(
						...
						uniqueScope.parentWithUniqueParamsVarsAndFunctions
		...
		if (!tleInfo) {
			...
			const snippets =
				await this.getSnippetCompletionItems(triggerCharacter);
			...
			if (!tleValue || !tleValue.contains(tleInfo.tleCharacterIndex)) {
				...
				if (
					TemplatePositionContext.isInsideSquareBrackets(
						tleInfo.tleParseResult,
						tleInfo.tleCharacterIndex,
					)
				) {
					...
					const functionCompletions =
						TemplatePositionContext.getFunctionCompletions(
							scope,
							undefined,
							replaceSpan,
						);

					const namespaceCompletions =
						TemplatePositionContext.getNamespaceCompletions(
							scope,
							replaceSpan,
						);
				...
				completions.push(
					...this.getFunctionCallCompletions(
						tleValue,
						this.jsonToken!,
						tleInfo.tleCharacterIndex,
						scope,
				...
				completions.push(
					...this.getStringLiteralCompletions(
						tleValue,
						tleInfo.tleCharacterIndex,
						scope,
					),
				...
				completions.push(
					...this.getPropertyAccessCompletions(
						tleValue,
						tleInfo.tleCharacterIndex,
						scope,
					),
	...
	public getScope(): TemplateScope {
		if (this.jsonValue && this.document.topLevelValue) {
			const objectLineage = <(Json.ObjectValue | Json.ArrayValue)[]>(
				this.document.topLevelValue
			...
			for (const parent of objectLineage.reverse()) {
				const innermostMachingScope = scopes.find(
					(s) => s.rootObject === parent,
				);
	...
	private getDependsOnCompletionItems(
		...
		const insertionContext = this.getInsertionContext({
			triggerCharacter,
	...
	private async getSnippetCompletionItems(
		triggerCharacter: string | undefined,
	): Promise<ICompletionItemsResult> {
	 ...
	 */
	private static isInsideSquareBrackets(
		parseResult: TLE.TleParseResult,
		characterIndex: number,
	): boolean {
		const leftSquareBracketToken: TLE.Token | undefined =
			parseResult.leftSquareBracketToken;

		const rightSquareBracketToken: TLE.Token | undefined =
			parseResult.rightSquareBracketToken;

		if (
			leftSquareBracketToken &&
			leftSquareBracketToken.span.afterEndIndex <= characterIndex &&
			(!rightSquareBracketToken ||
				characterIndex <= rightSquareBracketToken.span.startIndex)
		) {
			return true;
		}
	...
	private getStringLiteralCompletions(
		tleStringValue: TLE.StringValue,
	...
	private getPropertyAccessCompletions(
		tleValue: TLE.PropertyAccess,
		tleCharacterIndex: number,
		scope: TemplateScope,
	): Completion.Item[] {
		const functionSource: TLE.FunctionCallValue | undefined =
			tleValue.functionSource;
		...
		if (functionSource) {
			...
			if (propertyNameToken) {
				replaceSpan = propertyNameToken.span.translate(
					this.jsonTokenStartIndex,
				);
				propertyPrefix = propertyNameToken.stringValue
					.substring(
						0,
						tleCharacterIndex - propertyNameToken.span.startIndex,
					)
					.toLowerCase();
			}

			const variableProperty: IVariableDefinition | undefined =
				scope.getVariableDefinitionFromFunctionCall(functionSource);

			const parameterProperty: IParameterDefinition | undefined =
				scope.getParameterDefinitionFromFunctionCall(functionSource);
			...
			if (variableProperty) {
				...
				const sourceVariableDefinition: Json.ObjectValue | undefined =
				...
				const parameterDefValue: Json.ObjectValue | undefined =
					parameterProperty.defaultValue
						? Json.asObjectValue(parameterProperty.defaultValue)
						: undefined;
				...
				if (parameterDefValue) {
					const sourcePropertyDefinition:
						...
						parameterDefValue.getPropertyValueFromStack(
							sourcesNameStack,
						),
					);
				...
				if (functionName && !functionSource.namespaceToken) {
					let functionMetadata: BuiltinFunctionMetadata | undefined =
						AzureRMAssets.getFunctionMetadataFromName(functionName);
					...
					if (functionMetadata) {
						...
						for (const returnValueMember of functionMetadata.returnValueMembers) {
							if (
								...
								result.push(
									TemplatePositionContext.createPropertyCompletionItem(
										returnValueMember,
										replaceSpan,
									),
								);
	...
	private getFunctionCallCompletions(
		tleValue: TLE.FunctionCallValue,
		...
		assert(
			tleValue
				.getSpan()
				.contains(tleCharacterIndex, ContainsBehavior.extended),
			"Position should be inside the function call, or right after it",
		);

		const completions: Completion.Item[] = [];

		const namespaceName: string | undefined = tleValue.namespaceToken
			? tleValue.namespaceToken.stringValue
			: undefined;
		// tslint:disable-next-line: strict-boolean-expressions
		const namespace: UserFunctionNamespaceDefinition | undefined =
			(namespaceName &&
				scope.getFunctionNamespaceDefinition(namespaceName)) ||
			undefined;
		...
		if (
			...
			namespaceName &&
			tleValue.periodToken &&
			tleValue.periodToken.span.afterEndIndex === tleCharacterIndex
		) {
			...
			tleValue.namespaceToken &&
			tleValue.periodToken &&
			tleValue.namespaceToken.span.contains(
				tleCharacterIndex,
				ContainsBehavior.extended,
			)
		) {
			...
			tleValue.isCallToBuiltinWithName(templateKeys.parameters) &&
			tleValue.argumentExpressions.length === 0
		) {
			...
			tleValue.isCallToBuiltinWithName(templateKeys.variables) &&
			tleValue.argumentExpressions.length === 0
		) {
		...
		if (tleTokenToComplete) {
			const tokenToCompleteStartIndex: number =
				tleTokenToComplete.span.startIndex;

			const completionLength =
				tleCharacterIndex - tokenToCompleteStartIndex;
			assert(completionLength >= 0);
			replaceSpan = new Span(
				this.jsonTokenStartIndex,
				completionLength,
			).translate(tleTokenToComplete.span.startIndex);
		} else {
		...
		assert(
			completeBuiltinFunctions ||
				completeUserFunctions ||
				completeNamespaces,
			"Should be completing something",
		);

		if (completeBuiltinFunctions || completeUserFunctions) {
			if (completeUserFunctions && namespace) {
				const userFunctionCompletions =
					TemplatePositionContext.getFunctionCompletions(
						scope,
						namespace,
						replaceSpan,
		...
		completions.push(
			...getResourceIdCompletions(this, tleValue, parentStringToken),
		);

		return completions;
	}

	private getDeepPropertyAccessCompletions(
		propertyPrefix: string,
		variableOrParameterDefinition: Json.ObjectValue,
		sourcesNameStack: string[],
		replaceSpan: Span,
	): Completion.Item[] {
		const result: Completion.Item[] = [];

		const sourcePropertyDefinitionObject: Json.ObjectValue | undefined =
			Json.asObjectValue(
				variableOrParameterDefinition.getPropertyValueFromStack(
					sourcesNameStack,
				),
			);
		...
		if (sourcePropertyDefinitionObject) {
			...
			if (!propertyPrefix) {
				matchingPropertyNames =
					sourcePropertyDefinitionObject.propertyNames;
			} else {
				...
				for (const propertyName of sourcePropertyDefinitionObject.propertyNames) {
					if (
						propertyName.toLowerCase().startsWith(propertyPrefixLC)
					) {
	...
	private static createPropertyCompletionItem(
		propertyName: string,
		replaceSpan: Span,
	): Completion.Item {
	...
	public getReferences(): ReferenceList | undefined {
		...
		if (tleInfo) {
			// If we're inside a string (whether an expression or not)
			const refInfo = this.getReferenceSiteInfo(true);

			if (refInfo) {
				// References in this document
				const references: ReferenceList =
					this.document.findReferencesToDefinition(
						refInfo.definition,
					);
				...
				let parameterValuesSource: IParameterValuesSource | undefined =
					refInfo.definitionScope &&
					this.getParameterValuesSource(refInfo.definitionScope);
	...
	protected getParameterValuesSource(
		scope: TemplateScope,
	): IParameterValuesSource | undefined {
		// tslint:disable-next-line: no-suspicious-comment
	...
	private getDefinitionAtSite(): INamedDefinition | undefined {
		...
		if (tleInfo) {
			const jsonStringValue: Json.StringValue | undefined =
				Json.asStringValue(this.jsonValue);
			...
			if (jsonStringValue) {
				...
				const parameterDefinition: IParameterDefinition | undefined =
					scope.getParameterDefinition(unquotedString);

				if (
					parameterDefinition &&
					parameterDefinition.nameValue === jsonStringValue
				) {
				...
				const variableDefinition: IVariableDefinition | undefined =
					scope.getVariableDefinition(unquotedString);

				if (
					variableDefinition &&
					variableDefinition.nameValue === jsonStringValue
				) {
				...
				const namespaceDefinition:
					...
					| undefined =
					scope.getFunctionNamespaceDefinition(unquotedString);

				if (
					namespaceDefinition &&
					namespaceDefinition.nameValue === jsonStringValue
				) {
				...
				for (let ns of scope.namespaceDefinitions) {
					const userFunctionDefinition:
						...
						| undefined = scope.getUserFunctionDefinition(
						ns.nameValue.unquotedValue,
						unquotedString,
					);

					if (
						userFunctionDefinition &&
						userFunctionDefinition.nameValue === jsonStringValue
					) {
			...
			if (functionToHelpWith && functionToHelpWith.name) {
				...
				if (functionToHelpWith.namespaceToken) {
					...
					const namespace: string =
						functionToHelpWith.namespaceToken.stringValue;

					const name: string | undefined = functionToHelpWith.name;

					const udfDefinition: UserFunctionDefinition | undefined =
						this.tleInfo.scope.getUserFunctionDefinition(
							namespace,
							name,
						);

					functionMetadata = udfDefinition
						? UserFunctionMetadata.fromDefinition(udfDefinition)
						: undefined;
				} else {
					// Call to built-in function
					functionMetadata =
						AzureRMAssets.getFunctionMetadataFromName(
							functionToHelpWith.name,
						);
				}
				if (functionMetadata) {
					let currentArgumentIndex =
						this.getFunctionCallArgumentIndex();

					if (typeof currentArgumentIndex === "number") {
						const functionMetadataParameters: IFunctionParameterMetadata[] =
							functionMetadata.parameters;

						if (
							functionMetadataParameters.length > 0 &&
							functionMetadataParameters.length <=
								currentArgumentIndex &&
							functionMetadataParameters[
								functionMetadataParameters.length - 1
							].name.endsWith("...")
						) {
							currentArgumentIndex =
								functionMetadataParameters.length - 1;
						...
						return new TLE.FunctionSignatureHelp(
							currentArgumentIndex,
							functionMetadata,
						);
					}
	 ...
	 */
	public getFunctionCallArgumentIndex(
		functionCall?: TLE.FunctionCallValue,
	): number | undefined {
		const tleInfo = this.tleInfo;
		...
		if (tleInfo && tleValue) {
			...
			if (functionCall) {
				if (
					!functionCall.leftParenthesisToken ||
					tleInfo.tleCharacterIndex <=
						functionCall.leftParenthesisToken?.span.endIndex
				) {
					return -1;
				}
				...
				for (const commaToken of functionCall.commaTokens) {
					if (
						commaToken.span.startIndex < tleInfo.tleCharacterIndex
					) {
						++currentArgumentIndex;
					}
				}
	 ...
	 * is undefined).
	 */
	private static getFunctionCompletions(
		scope: TemplateScope,
		namespace: UserFunctionNamespaceDefinition | undefined,
		replaceSpan: Span,
	): Completion.Item[] {
		...
		if (namespace) {
			...
			matches = namespace.members.map((fd) =>
				UserFunctionMetadata.fromDefinition(fd),
			);
		} else {
			// Built-in functions
		...
		return matches.map((m) =>
			Completion.Item.fromFunctionMetadata(m, replaceSpan),
		);
	}
	 ...
	 */
	private static getNamespaceCompletions(
		scope: TemplateScope,
		replaceSpan: Span,
	): Completion.Item[] {
		return scope.namespaceDefinitions.map((m) =>
			Completion.Item.fromNamespaceDefinition(m, replaceSpan),
		);
	}
	...
	private getParameterCompletions(
		...
		const replaceSpanInfo: ITleReplaceSpanInfo | undefined =
			this.getParameterOrVariableNameReplaceInfo(
				tleValue,
				tleCharacterIndex,
			);
		...
		if (replaceSpanInfo) {
			for (const parameterDefinition of scope.parameterDefinitionsSource
				.parameterDefinitions) {
				parameterCompletions.push(
					Completion.Item.fromParameterDefinition(
						parameterDefinition,
						replaceSpanInfo.replaceSpan,
						replaceSpanInfo.includeRightParenthesisInCompletion,
						replaceSpanInfo.includeSingleQuotesInCompletion,
					),
				);
			}
		}
		...
		if (replaceSpanInfo) {
			for (const variableDefinition of scope.variableDefinitions) {
				variableCompletions.push(
					Completion.Item.fromVariableDefinition(
						variableDefinition,
						replaceSpanInfo.replaceSpan,
						replaceSpanInfo.includeRightParenthesisInCompletion,
						replaceSpanInfo.includeSingleQuotesInCompletion,
	...
	private getParameterOrVariableNameReplaceInfo(
		...
		if (tleValue instanceof TLE.StringValue) {
			...
			const functionValue: TLE.FunctionCallValue | undefined =
				TLE.asFunctionCallValue(tleValue.parent);
			...
			if (tleCharacterIndex <= stringStartIndex) {
				...
				tleCharacterIndex - stringStartIndex === 1 &&
				tleValue.unquotedValue.startsWith(`''`)
			) {
				...
				const rightParenthesisIndex: number = tleValue
					.toString()
					.indexOf(")");

				const rightSquareBracketIndex: number = tleValue
					.toString()
					.indexOf("]");

				if (rightParenthesisIndex >= 0) {
					// Cut off before the ending parenthesis
					tleReplaceSpan = new Span(
						stringStartIndex,
					...
					tleReplaceSpan = new Span(
						stringStartIndex,
						functionValue.rightParenthesisToken.span.afterEndIndex -
							stringStartIndex,
					);
					includeRightParenthesisInCompletion = true;
				} else {
			...
			if (tleValue.rightParenthesisToken) {
				replaceSpan = new Span(
					this.documentCharacterIndex,
					tleValue.rightParenthesisToken.span.startIndex -
						tleCharacterIndex +
						1,
				);
				includeRightParenthesisInCompletion = true;
