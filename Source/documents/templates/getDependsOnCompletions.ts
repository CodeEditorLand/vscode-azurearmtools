...
export function getDependsOnCompletions(
	...
	const currentResource = findClosestEnclosingResource(
	...
	const descendentsIncludingSelf: Set<IResourceInfo> =
		findDescendentsIncludingSelf(currentResource);
...
function getDependsOnCompletionsForResource(
	...
	if (resourceIdExpression) {
		...
		const friendlyTypeExpression = resource.getFriendlyTypeExpression({
			fullType: false,
		});
		...
		const item = new Completion.Item({
			...
			priority: isParent
				? Completion.CompletionPriority.high
				: Completion.CompletionPriority.normal,
		});
...
function findClosestEnclosingResource(
	...
	if (firstMatch) {
		...
		while (true) {
			const childrenContainingResource = deepestMatch.children.filter(
				(child) =>
