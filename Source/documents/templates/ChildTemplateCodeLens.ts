...
export class NestedTemplateCodeLens extends ChildTemplateCodeLens {
	...
	public static create(
		...
		const langServerLoadState: string | undefined =
			getLoadStateFromLanguageServerStatus();
...
export class LinkedTemplateCodeLens extends ChildTemplateCodeLens {
	private constructor(
		...
		this.command = {
			...
			command: linkedFileUri
				...
				: "",
			tooltip,
			arguments: linkedFileUri ? [linkedFileUri] : [],
		};
	}
	...
	public static create(
		...
		const firstLinkedTemplateRef = linkedTemplateReferences
			? linkedTemplateReferences[0]
			: undefined;
		...
		if (!fullValidationStatus.fullValidationEnabled) {
			...
			title +=
				" " +
				"(cannot validate - make sure all other validation errors have been fixed)";
		}
