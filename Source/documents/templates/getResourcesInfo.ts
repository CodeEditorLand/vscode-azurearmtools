...
export class ResourceInfo implements IResourceInfo {
	...
	public constructor(
		public readonly nameSegmentExpressions: string[],
		public readonly typeSegmentExpressions: string[],
		public readonly parent?: IResourceInfo,
	) {
		if (parent) {
			parent.children.push(this);
		}
	}
	public isDecoupledChild = false;

	public get shortNameExpression(): string {
		return this.nameSegmentExpressions[
			this.nameSegmentExpressions.length - 1
		];
	}
	...
	public get shortTypeExpression(): string | undefined {
		if (this.typeSegmentExpressions.length > 1) {
			...
			return this.typeSegmentExpressions[
				this.typeSegmentExpressions.length - 1
			];
		} else {
	...
	public getResourceIdExpression(): string | undefined {
		if (
			this.nameSegmentExpressions.length > 0 &&
	...
	public getFriendlyNameExpression({
		fullName,
	}: { fullName?: boolean }): string {
		return getFriendlyNameExpression({ resource: this, fullName });
	}

	public getFriendlyTypeExpression({
		fullType,
	}: { fullType?: boolean }): string {
		return getFriendlyTypeExpression({ resource: this, fullType });
	}

	public getFriendlyResourceLabel({
		fullName,
		fullType,
	}: { fullName?: boolean; fullType?: boolean }): string {
		return getFriendlyResourceLabel({ resource: this, fullName, fullType });
	}
}

export class JsonResourceInfo
	extends ResourceInfo
	implements IJsonResourceInfo
{
	public constructor(
		nameSegmentExpressions: string[],
		typeSegmentExpressions: string[],
	...
	public get copyBlockElement(): Json.ObjectValue | undefined {
		return this.resourceObject.getPropertyValue(templateKeys.copyLoop)
			?.asObjectValue;
	}
}
 ...
 */
function concatExpressionsWithSeparator(
	expressions: string[],
	unquotedLiteralSeparator?: string,
): string | undefined {
	...
	for (let i = 0; i < expressionsLength; ++i) {
		...
		if (isSingleQuoted(expression)) {
			if (unquotedLiteralSeparator && !isLastSegment) {
				// Add separator
				expression = `'${removeSingleQuotes(
					expression,
				)}${unquotedLiteralSeparator}'`;
			}
...
function getInfoFromResourcesArray(
	resourcesArray: Json.ArrayValue,
	parent: IJsonResourceInfo | undefined,
): IJsonResourceInfo[] {
	const results: IJsonResourceInfo[] = [];
...
function getInfoFromResourceObject(
	resourceObject: Json.ObjectValue,
	parent: IJsonResourceInfo | undefined,
	results: IJsonResourceInfo[],
	...
	if (resName && resType) {
		...
		const info: IJsonResourceInfo = new JsonResourceInfo(
			nameExpressions,
			typeExpressions,
			resourceObject,
			parent,
		...
		if (processChildren) {
			...
			// ]
			if (
				resType.unquotedValue.toLowerCase() ===
...
//     ->
//   [ "concat(variables('sqlServer')", "variables('firewallRuleName')" ]
export function splitResourceNameIntoSegments(
	nameUnquotedValue: string,
): string[] {
...
//   ->
// [ "'Microsoft.Network/networkSecurityGroups'", "'securityRules'" ]
function splitResourceTypeIntoSegments(
	typeNameUnquotedValue: string,
): string[] {
	...
	if (isJsonStringAnExpression(typeNameUnquotedValue)) {
		...
		let segmentsAsExpressions = splitExpressionIntoSegments(
			typeNameUnquotedValue,
		);
		...
		if (segmentsAsStrings.length >= 2) {
			...
			segmentsAsStrings = [
				`${segmentsAsStrings[0]}/${segmentsAsStrings[1]}`,
			].concat(segmentsAsStrings.slice(2));
		}
...
function splitExpressionIntoSegments(jsonString: string): string[] {
	...
	if (jsonString.includes("/")) {
		...
		if (parseResult.errors.length === 0 && expression) {
			...
			if (
				expression instanceof TLE.FunctionCallValue &&
				...
				if (argumentExpressions.every((v) => !!v)) {
					...
					for (const arg of argumentExpressions) {
						if (
							arg instanceof TLE.StringValue &&
							arg.unquotedValue.includes("/")
						) {
							const refactoredArg: string[] =
								splitStringAndKeepSeparators(
									arg.unquotedValue,
									"/",
								).map((s) => `'${s}'`);
...
function getFriendlyNameExpression({
	...
	const resourceObject =
		"resourceObject" in resource ? resource.resourceObject : undefined;

	let friendlyName: string;

	// Look for displayName tag first
	const tags = resourceObject?.getPropertyValue(
		templateKeys.tags,
	)?.asObjectValue;
	const displayName = tags?.getPropertyValue(templateKeys.displayNameTag)
