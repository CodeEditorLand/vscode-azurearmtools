...
import {
	prependLinkedTemplateScheme,
	removeLinkedTemplateScheme,
} from "../../../util/linkedTemplateScheme";
...
export async function onRequestOpenLinkedFile({
	...
	pathType,
}: IRequestOpenLinkedFileArgs): Promise<
	IRequestOpenLinkedFileResult | undefined
> {
	return await callWithTelemetryAndErrorHandling<IRequestOpenLinkedFileResult>(
		...
		async (context: IActionContext) => {
			...
			if (requestedLinkUri.scheme === documentSchemes.untitled) {
				...
				return {
					loadErrorMessage: "Link uri should be an absolute path",
				};
				...
				const loadErrorMessage = result.loadError
					? parseError(result.loadError).message
					: undefined;
