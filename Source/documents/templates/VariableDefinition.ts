...
export class TopLevelVariableDefinition extends VariableDefinition {
	private readonly _value: CachedValue<Json.Value | undefined> =
		new CachedValue<Json.Value | undefined>();
				...
				for (const copyElement of arrayValue.elements) {
					...
					if (loopVarObject) {
						...
						if (name && count && input) {
							...
							const array = new Json.ArrayValue(input.span, [
								input,
							]);
...
export class TopLevelCopyBlockVariableDefinition extends VariableDefinition {
	...
	public constructor(
		...
		// some value.  We'll create an array with a single element using the 'input' expression.
		this.value = input
			? new Json.ArrayValue(input.span, [input])
			: undefined;
	}
