...
export class InsertItem {
	...
	private async insertParameter(
		...
		const defaultValue = await this.ui.showInputBox({
		...
		const description = await this.ui.showInputBox({
	...
	private async insertInObjectHelper({
		...
		const templatePartStartPos = textEditor.document.positionAt(
			templatePart.span.startIndex,
		);
		const templatePartStartLine = textEditor.document.lineAt(
		...
		const templatePartIndentLevel = Math.max(
			0,
	...
	private async insertFunctionAsMembers(
		...
		const functionName = await this.ui.showInputBox({
	...
	private async insertFunction(
		...
		const functions = this.getTemplateArrayPart(
			templateTopLevel,
		...
		const members = namespace.getPropertyValue(
	...
	private async insertResource(
		...
		const resources = this.getTemplateArrayPart(
			templateTopLevel,
		...
		const newCursorPosition = this.getCursorPositionForInsertResource(
			textEditor,
			index,
			prepend,
		);
	...
	private getCursorPositionForInsertResource(
		...
		const prependRange = new vscode.Range(
	...
	private async getFunctionNamespace(): Promise<FunctionNameSpace> {
		const namespaceName = await this.ui.showInputBox({
	...
	private async insertText(
		...
		if (setCursor && text.lastIndexOf(insertCursorText) >= 0) {
			const insertedText = textEditor.document.getText(
				new vscode.Range(
					pos,
