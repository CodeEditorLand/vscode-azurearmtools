...
export class JsonOutlineProvider
	implements vscode.TreeDataProvider<IElementInfo>
{
		...
		if (vscode.window.visibleTextEditors.length > 0) {
			if (
				...
				if (!elementInfo) {
					...
					if (
						valueNode instanceof Json.ObjectValue &&
						elementInfo.current.collapsible
					) {
						// tslint:disable-next-line:one-variable-per-declaration
						for (
						...
						valueNode instanceof Json.ArrayValue &&
						elementInfo.current.collapsible
					) {
						// Array with objects
						// tslint:disable-next-line:one-variable-per-declaration
	...
	private getTreeNodeLabel(elementInfo: IElementInfo): string {
		...
		if (keyNode instanceof Json.ObjectValue) {
			...
			elementInfo.current.value.kind === Json.ValueKind.ArrayValue ||
			elementInfo.current.value.kind === Json.ValueKind.ObjectValue
		) {
			...
			return `${
				keyNode instanceof Json.StringValue
					? toFriendlyString(keyNode)
					: "?"
			}: ${toFriendlyString(valueNode)}`;
		}
	...
	private getElementInfo(
		...
		if (childElement instanceof Json.ObjectValue) {
			...
			if (
				childElement.value instanceof Json.ArrayValue &&
				childElement.value.elements.length > 0
			) {
				// Is the first element in the Array an Object
				if (
					childElement.value.elements[0].valueKind ===
					Json.ValueKind.ObjectValue
				) {
					collapsible = true;
				}
			} else if (
				childElement.value instanceof Json.ObjectValue &&
				childElement.value.properties.length > 0
			) {
				collapsible = true;
			}
		}
	...
	private getIconPath(elementInfo: IElementInfo): string | undefined {
		...
		if (elementInfo.current.level && elementInfo.current.level > 1) {
			...
			if (
				elementInfo.current.key.kind === Json.ValueKind.ObjectValue &&
				rootNode &&
				rootNode.toString().toUpperCase() ===
					"resources".toUpperCase() &&
				keyOrResourceNode instanceof Json.ObjectValue
			) {
				// tslint:disable-next-line:one-variable-per-declaration
				for (
			...
			if (
				rootNode &&
				rootNode.toString().toUpperCase() === "functions".toUpperCase()
			) {
